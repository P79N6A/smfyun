...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/wfba/orders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            <ul class="nav nav-tabs">
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/wfba/orders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/wfba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/wfba/orders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/wfba/orders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/wfba/orders?qid=<?=$result['qid']?>&type=fare">话费/充值</a></span>
          <span tag='hb' class='tag <?=$activetype == 'hb' ? 'tactive' : ''?>'><a href="/wfba/orders?qid=<?=$result['qid']?>&type=hb">红包</a></span>
          ...
          <div class="tab-pane active" id="orders<?=$result['status']?>">
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <th>金额</th>
                <th>备注</th>
                <th><?=$bid==2345?'核销时间':'时间'?></th>
                <th>品名</th>
                <th nowrap="">积分</th>
                <?php if($bid==1||$bid==2345):?>
                <th>核销备注</th>
              <?php endif;?>
                <th>操作</th>
              </tr>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/wfba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
                  ...
                  <?php endif;?>
                  <?php if($bid==2345&&$result['status'] != 0):?>
              ...
              <?php endif;?>
                  <td nowrap="">
                    <?php if ($result['status'] == 0||$bid==2345):?>
                    <a href="#" data-toggle="modal" data-target="#actionModel" data-openid="<?=$order->user->openid?>" data-name="<?=$order->item->name?>" data-id="<?=$order->id?>" data-linkman="<?=$order->name?>" data-tag="<?=$order->type?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->city.' '.$order->address?>" data-bid="<?=$order->bid?>" data-status="<?=$result['status']?>">
                      <span><?=$result['status'] == 0?'处理':'修改'?></span> <i class="fa fa-check-square"></i>
                    </a>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary"><?=$result['status'] == 0?'处理':'修改'?>该订单</button>
      </div>
    </div><!-- /.modal-content -->
    </form>
  </div><!-- /.modal-dialog -->
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['wfba']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['wfba']['shipcode']?>"></div>'

  $('#id').val(id)

  var modal = $(this)
  modal.find('.modal-title').text(name)
  if(bid!=2345||status==0){
    if(tag==3){
    ...
    //modal.find('form').attr('action', '/wfba/orders/ship/' + id + '?p=<?=$_GET['p']?>')
    }else if(tag==4){
    ...
    modal.find('.modal-body').html('<div class="form-group"><label for="remark">核销备注：</label><input name="remark" maxlength="6" type="text" value="<?=$_SESSION['wfba']['remark']?>"></div><input type="hidden" name="flag" value="0">')
  }
