  ...
  <body class="page-detail" data-page="detail" ontouchstart="">
    <div class="view js-page-view" id="page-view-detail">
        <div class="detail-header" id="detail-header">
              ...
              <div style="width: auto;"> <img src="<?=$config['cdn']?>/qwtwdb/images/item/<?=$item->id?>.v<?=$item->lastupdate?>.jpg" alt="<?=$item->name?>">
           ...
           <div id="pay-bar" class="pay-bar"> 单价：
            <span class="pay-total js-total-price">
             <b class="coins"><?=$item->score?></b><?=$config['scorename']?>（剩余：<?=$item->stock?>件）
           </span>
           <?php if ($dlimit==1):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:40%">已经达到兑换上限</a>
                <?php elseif($item->stock <= 0):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:35%">已换完</a>
                <?php elseif ($user2['score'] < $item->score):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:35%">您的<?=$config['scorename']?>不够</a>
                ...
                <?php elseif ($item->limit > 0 && $limit >= $item->limit):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:40%">您已经兑换过该奖品</a>
                <?php elseif ($user2['lock'] == 1):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:40%">您的账号已被锁定</a>
                <?php else:?>
                <a href="/qwtwdb/neworder/<?=$item->id?>" class="pay-btn" style="width:35%">立即兑换</a>
                <?php endif?>
