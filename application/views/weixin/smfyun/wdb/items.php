...
foreach ($items as $item):
    $item = (object)$item;
    //判断限购
    $limit = ORM::factory('qwt_wdborder')->where('qid', '=', $user2['id'])->where('iid', '=', $item->id)->count_all();
?>
                <div class="goods js-click">
                <a href="/qwtwdb/item/<?=$item->id?>">
                    <div class="media">
                        <img src="http://cdn.jfb.smfyun.com/qwtwdb/images/item/<?=$item->id?>.v<?=$item->lastupdate?>0.jpg" alt="<?=$item->name?>">
                    </div>
                    <div class="info">
                        <span class="name">
                            <?=$item->name?>
