...
<ul class="cd-gallery">
        ...
        <li>
            <div class="cd-single-item">
                <ul class="cd-slider-wrapper">
                    <li class="selected"><img src="<?=$config['cdn']?>/qwtdka/images/item/<?=$item->id?>.v<?=$item->lastupdate?>.jpg" alt="<?=$item->name?>"></li>
                </ul>
            </div> <!-- .cd-single-item -->
            <div class="cd-item-info">
                <b><a href="/qwtdka/neworder/<?=$item->id?>"><?=$item->name?></a></b>
                <?php if(isset($_GET['rank'])):?>
                    <em><?=$_GET['rank']?>等奖</em>
                <?php else:?>
    ...
    <?php endif?>
            ...
            <? elseif($item->type==5):?>
                ...
                <button type="submit" class="add-to-cart">确认兑换</button>
            <? elseif($item->type==6):?>
                <input style="display:none" id="memo" type="text" name="data[type]" maxlength="30" value='6'>
                <button type="submit" class="add-to-cart">确认兑换</button>
            <? else:?>
...
<script src="http://cdn.jfb.smfyun.com/qwtdka/plugins/citySelect/jquery.cityselect.js"></script>
