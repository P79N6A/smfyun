  ...
  <body class="page-detail" data-page="detail" ontouchstart="">
    <div class="view js-page-view" id="page-view-detail">
        <div class="detail-header" id="detail-header">
              ...
              <div style="width: auto;"> <img src="<?=$config['cdn']?>/qwtdka/images/item/<?=$item->id?>.v<?=$item->lastupdate?>.jpg" alt="<?=$item->name?>">

          </div>

           <div class="goods-info">
            <!-- <span class="type"><?=convert($item->type)?></span> -->
            <span class="name"><?=$item->name?></span> / 价值￥<?=$item->price?>
             </div>
           </div>
           ...
           <div id="pay-bar" class="pay-bar"> 单价：
            <span class="pay-total js-total-price">
             <b class="coins"><?=$item->score?></b><?=$config['score2']?>（剩余：<?=$item->stock?>件）
           </span>
           <?php if ($dlimit==1):?>
                ...
                <?php elseif ($user2['score'] < $item->score):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled" style="width:35%">您的<?=$config['score2']?>不够</a>
                ...
                <?php else:?>
                <a href="/qwtdka/neworder/<?=$item->id?>" class="pay-btn" style="width:35%">立即兑换</a>
                <?php endif?>
