...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dyba/orders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            <ul class="nav nav-tabs">
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/dyba/orders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/dyba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/dyba/orders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/dyba/orders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/dyba/orders?qid=<?=$result['qid']?>&type=fare">话费流量</a></span>
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/dyba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['dyba']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['dyba']['shipcode']?>"></div>'
  ...
  //modal.find('form').attr('action', '/dyba/orders/ship/' + id + '?p=<?=$_GET['p']?>')
  }else{
