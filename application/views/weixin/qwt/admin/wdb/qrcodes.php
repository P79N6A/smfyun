            ...
            <button class="am-btn  am-btn-default am-btn-success tpl-am-btn-success am-icon-search" type="submit"></button>
              ...
              <?php endif?>
                ...
                foreach ($result['qrcodes'] as $v):
                  $count2 = ORM::factory('qwt_wdbqrcode')->where('openid','!=','')->where('fuopenid','!=','')->where('bid', '=', $v->bid)->where('fopenid', '=', $v->openid)->count_all();
                  $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_wdbqrcodes WHERE fuopenid!='' and fopenid='$v->openid'")->execute()->as_array();
                  $tempid=array();
                  ...
                  $count3 = ORM::factory('qwt_wdbqrcode')->where('openid','!=','')->where('fuopenid','!=','')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->count_all();
                  $fuser='';
                  if($v->fopenid){
                      $fuser = ORM::factory('qwt_wdbqrcode')->where('openid','!=','')->where('fuopenid','!=','')->where('bid', '=', $v->bid)->where('openid', '=', $v->fopenid)->find();
                  }
                ...
                <tr>
                  ...
                  <td><?=$v->ticket ? '是' : '否'?></td>
              <?php if($config['status']==1):?>
                  <th><?=$v->area?$v->area:'用户未点击允许获取地理位置上报'?></th>
              <?php endif?>
                  <td><a href="/qwtwdba/qrcodes?id=<?=$fuser->id?>"><?=$fuser->nickname?></a></td>
                  <td nowrap="">
                    <a style="background-color:#fff;" class='edit am-btn am-btn-default am-btn-xs am-text-secondary' href="#myModal" data-toggle="modal" data-id="<?=$v->id?>" data-lock="<?=$v->lock?>" data-name="<?=$v->nickname?>"><span class="am-icon-pencil-square-o"></span> 修改
                    </a>
                  </td>
                </tr>
                <?php endforeach;?>
        ...
        <div class="tpl-content-wrapper">
          ...
          <div class="am-tabs tpl-index-tabs" data-am-tabs>
            ...
            <input class="form-control" id="fscore" name="form[score]" max="999" style="width:150px" type="number">
                                ...
                                <div class="am-form-group">
                                    <label for="user-name" class="am-u-sm-12 am-form-label">用户状态 <span class="tpl-form-line-small-title">User Status</span></label>
                                    <div class="am-u-sm-12">
                            <div class="actions">
