  ...
  <input id="datetimepicker" size="16" type="text" name="data[endtime]" size="16" value="<?=$_POST['data']['endtime']?>" readonly="" class="am-form-field" readonly>
      ...
      <?php if($result['action']=='add'):?>
        ...
        <?php if ($result['action'] == 'edit' && $result['item']['pic']):?>
                  <a href="/qwtwdba/images/item/<?=$result['item']['id']?>.v<?=$result['item']['lastupdate']?>.jpg" target="_blank">
                                            <div class="tpl-form-file-img">
                                            <img class="img-thumbnail" src="/qwtwdba/images/item/<?=$result['item']['id']?>.v<?=$result['item']['lastupdate']?>.jpg" width="100">
                                            </div>
                                            </a>
                                          <?php endif?>
...
function change(i){
  ...
  swal({
    ...
    function(){
      window.location.href = "/qwtwdba/items/edit/<?=$result['item']['id']?>?DELETE=1";
    })
  })
  $(function() {
