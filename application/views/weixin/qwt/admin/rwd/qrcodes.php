        ...
        <div class="tpl-content-wrapper">
            ...
            <button class="am-btn  am-btn-default am-btn-success tpl-am-btn-success am-icon-search" type="submit"></button>
              ...
              <tr>
                  ...
                  <th>性别</th>
                  <th>直接粉丝</th>
                  <th><a href="/qwtrwda/qrcodes?sort=jointime&amp;s=<?=$result['s']?>&amp;lock=<?=$result['lock']?>" title="按关注时间排序">关注时间</a></th>
                  <th>关注状态</th>
                  <th>是否有生成海报？</th>
                  <!-- 开启了可选参与地区开关 -->
              ...
              <?php endif?>
               ...
               <?php
                foreach ($result['qrcodes'] as $v):
                  $count2 = ORM::factory('qwt_rwdqrcode')->where('openid','!=','')->where('fuopenid','!=','')->where('bid', '=', $v->bid)->where('fopenid', '=', $v->openid)->count_all();
                  $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_rwdqrcodes WHERE fopenid='$v->openid'")->execute()->as_array();
                  $tempid=array();
                  ...
                  $count3 = ORM::factory('qwt_rwdqrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->count_all();
                  $fuser = ORM::factory('qwt_rwdqrcode')->where('bid', '=', $v->bid)->where('openid', '=', $v->fopenid)->find();
                  $task_name=ORM::factory('qwt_rwdrecord')->where('bid','=',$v->bid)->where('fqid','!=','')->where('fqid','=',$fuser->id)->where('qid','=',$v->id)->find()->task->name;
                  $count4 =ORM::factory('qwt_rwdorder')->where('bid','=',$v->bid)->where('qid','=',$v->id)->count_all();
                  $sql = DB::query(Database::SELECT,"SELECT COUNT(DISTINCT tid) as t_num FROM qwt_rwdorders WHERE `qid` = $v->id ");
                ...
                <tr>
                  ...
                  <td id="score1<?=$v->id?>"><a href="/qwtrwda/orders?qid=<?=$v->id?>" title="查看奖品明细"><?=$count4?></a></td>
                  <td><?=$sex[$v->sex]?></td>
                  <td><a href="/qwtrwda/qrcodes?fopenid=<?=$v->openid?>" title="查看推荐明细"><?=$count2?></a></td>
              ...
              <?php endif?>
                  <td><a href="/qwtrwda/qrcodes?id=<?=$fuser->id?>"><?=$fuser->nickname?></a></td>
                  <td><?=$task_name?></td>
