        ...
        <div class="tpl-content-wrapper">
          ...
          <?php endif?>
                                <div class="am-form-group">
                                    <label for="goal2" class="am-u-sm-12 am-form-label">分类名称</label>
                                    <div class="am-u-sm-12">
                    <input type="text" class="tpl-form-input titleinput" id="goal2" name="form[name]" value="<?=$type->name?>">
                                    </div>
                                </div>
                                <div class="am-form-group">
                                    <label for="goal2" class="am-u-sm-12 am-form-label">可以查看该分类的代理等级</label>
                                    <div class="am-u-sm-12">
                  <?php if ($lv[0]->id):?>
                    <?php foreach ($lv as $k => $v):?>
                        <?php
                        $has = @in_array($v->id, $auth);
                        // echo $has;
                        ?>
                        <label class="am-form-label" for="<?=$v->id?>"><?=$v->lv?></label><input type="checkbox" name="form[lv][<?=$k?>]" <?=$auth&&$has==false?'':"checked='checked'"?> value="<?=$v->id?>" id="<?=$v->id?>">
                    <?php endforeach?>
                  <?php endif?>
                                    </div>
                                </div>
