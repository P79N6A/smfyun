...
<form method="get" name="qrcodesform">
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                //   $count3 = ORM::factory('qwt_dkascore')->where('bid', '=', $v->bid)->where('qid', '=', $v->id)->where('type', 'IN', array(3,8))->count_all();
                //   $fuser = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $v->bid)->where('openid', '=', $v->fopenid)->find();
                  //$information=ORM::factory('qwt_dkaqrcode',array('id'=>$v->qid,'bid'=>$v->bid))->find();
                $information=ORM::factory('qwt_dkaqrcode')->where('id','=',$v->qid)->find();
                $fid = ORM::factory('qwt_dkadetail')->where('tid', '=', $v->id)->where('type', '=', 2)->find();
                $fuser = ORM::factory('qwt_dkaqrcode')->where('id', '=', $fid->qid)->find();
