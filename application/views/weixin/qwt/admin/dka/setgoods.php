...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/qwtdkaa/qrcodes">分销商品管理</a></li>
         ...
         <!--      <div class="box-tools">
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                {
                  $mon=ORM::factory('qwt_dkasetgood')->where('goodid','=',$v['num_iid'])->where('bid','=',$bid)->find();
                   if($mon->id)
                   {
                   	$money0=$mon->money0;
                   	$money1=$mon->money1;
                   ...
                   {
                   	$monn = ORM::factory('qwt_dkacfg')->getCfg($bid);
                   	$money0=$monn['money0'];
                   	$money1=$monn['money1'];
                   	$money2=$monn['money2'];
                   }
                   $goodid=$v['num_iid'];
                  $soldednum=DB::query(database::SELECT,"select sum(temp.num)as tonum from (SELECT qwt_dkaorder1s.* FROM `qwt_dkatrades`,qwt_dkaorder1s WHERE qwt_dkaorder1s.tid=qwt_dkatrades.tid and qwt_dkatrades.status!='TRADE_CLOSED' and qwt_dkatrades.status!='TRADE_CLOSED_BY_USER' and qwt_dkatrades.status!='NO_REFUND' and qwt_dkaorder1s.goodid=$goodid) as temp where temp.bid=$bid")->execute()->as_array();
                   	//var_dump($soldednum);
                ?>
