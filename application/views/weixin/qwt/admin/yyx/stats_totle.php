...
?>
            ...
            <div class="tpl-portlet-components">
              ...
              <li id="orders<?=$status?>" class="<?=$status== 1 ? 'am-active' : ''?>"><a href="/qwtyyxa/stats_totle?qid=1">按天统计</a></li>
              <li id="orders<?=$status?>" class="<?=$status == 2? 'am-active' : ''?>"><a href="/qwtyyxa/stats_totle/month?qid=2">按月统计</a></li>
              <li id="orders<?=$status?>" class="<?=$status == 3? 'am-active' : ''?>"><a href="/qwtyyxa/stats_totle/shaixuan?qid=3">按日期筛选</a></li>
                            </ul>
  ...
  <input name="data[over]" id="datetimepicker2" size="16" type="text" value="<?=$_GET['data']['over']?>" class="am-form-field" readonly>
            ...
            <button id="ssbtn" class="search-btn1 am-btn  am-btn-default am-btn-success am-icon-search" type="submit"></button>
                ...
                <div class="tpl-block">
                  ...
                  <tr>
                    <td><?=$newadd[$i]['time']?></td>
                    <td><?=$newadd[$i]['yz_sold']||$newadd[$i]['lr_sold']?$newadd[$i]['yz_sold']+$newadd[$i]['lr_sold']:0?></td>
                    <td><?=$newadd[$i]['yz_sold']?$newadd[$i]['yz_sold']:0?></td>
                    <td><?=$newadd[$i]['lr_sold']?$newadd[$i]['lr_sold']:0?></td>
                  </tr>
