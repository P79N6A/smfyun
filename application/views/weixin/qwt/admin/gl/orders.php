...
?>
          ...
          <form method="get" name="ordersform">
                            <ul class="am-nav am-nav-tabs" style="left:0;">
                                <li id="orders<?=$result['state']?>" class="<?=$result['state'] == 0 ? 'am-active' : ''?>"><a href="/qwtgla/orders">未处理订单</a></li>
                                <li id="orders<?=$result['state']?>" class="<?=$result['state'] == 1 ? 'am-active' : ''?>"><a href="/qwtgla/orders/done">已处理订单</a></li>
                            </ul>
                            </form>
          ...
          <div class="tab-pane active" id="orders<?=$result['state']?>">
            ...
            <form method="post" class="am-form">
                ...
                <tr>
                  ...
                  <td><?=convert1($order->type)?></td>
                  <th><?=$order->name?></th>
                  <td><?=$order->type==4?number_format((($order->code)/100),2).'元':$order->code?></td>
                  <td><?=cstatus($order->status)?><?=$order->tel||$order->type!=0?'':'（未填写收货信息）'?></td>
                  <td ><?=date('Y/m/d H:i:s ',$order->lastupdate)?></td>

                  <td><?=$order->shiptype?></td>
                  <td><?=$order->shipcode?></td>
                  <td><?=$order->type==0?$order->pay_money.'元':''?></td>
                  <td><?=$order->receive_name?></td>
                  <td><?=$order->tel?></td>
                  <td><?=$order->address?></td>
                    <?php if ($result['state'] == 0):?>
                  <td nowrap="">
                    <a class="edit am-btn am-btn-default am-btn-xs am-text-secondary" data-toggle="modal"  data-openid="<?=$order->openid?>" data-name="<?=$order->name?>" data-id="<?=$order->id?>" data-linkman="<?=$order->receive_name?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->address?>" style="background-color:#fff;">
                      <span class="am-icon-pencil-square-o"></span> 处理
                    </a>
                    <input type="hidden" name="id[]" value="<?=$order->id?>">
                    <?php if ($order->type==0):?>
                      <a class="edit2 am-btn am-btn-default am-btn-xs am-text-secondary" data-toggle="modal" data-id="<?=$order->id?>" data-name="<?=$order->name?>" data-linkman="<?=$order->receive_name?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->address?>" style="background-color:#fff;">
