...
?>
            <ol class="am-breadcrumb">
                ...
                <li><a href="#">新建活动</a></li>
                <li><a href="/qwtyyhba/orders">领取记录</a></li>
          ...
          <form method="get" name="ordersform">
                            <ul class="am-nav am-nav-tabs" style="left:0;">
                                <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'am-active' : ''?>"><a href="/qwtyyhba/orders?qid=<?=$result['qid']?>">未处理订单</a></li>
                                <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'am-active' : ''?>"><a href="/qwtyyhba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
                            </ul>
                            </form>
          ...
          <div class="tab-pane active" id="orders<?=$result['status']?>">
            ...
            <form method="post" class="am-form">
                ...
                <tr>
                  ...
                  <td>
                    <a href="/qwtyyhba/qrcodes?id=<?=$order->user->id?>"><?=$order->name?></a>
                  </td>
                  ...
                  <td><?=$order->item->type==1?$order->pay_money.'元':''?></td>
                  <td><?php if($order->item->type==1):?><?=$order->order_state==1?'已付款':'未付款'?><?php endif?></td>
                  <td><?=date('m-d H:i',$order->lastupdate)?></td>
                  <th><?=converta($order->state)?></th>
                  <td><?=$order->state==1?'':$order->log?></td>
