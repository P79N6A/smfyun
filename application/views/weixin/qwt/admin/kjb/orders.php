...
<?php
if ($result['qrcode']) $title = $result['qrcode']->nickname . '的订单明细 / ';
if ($result['state'] == 0) $title .= '未处理';
if ($result['state'] == 1) $title .= '已处理';
if ($result['state'] == 1){
  $activetype = 'done';
}
?>
            <ol class="am-breadcrumb">
                ...
                <li><a href="#">订单管理</a></li>
                <li><a href="/qwtkjba/orders">订单管理</a></li>
          ...
          <form method="get" name="ordersform">
                            <ul class="am-nav am-nav-tabs" style="left:0;">
                                <li id="orders<?=$result['state']?>" class="<?=$result['state'] == 0 ? 'am-active' : ''?>"><a href="/qwtkjba/orders?qid=<?=$result['qid']?>">未处理订单</a></li>
                                <li id="orders<?=$result['state']?>" class="<?=$result['state'] == 1 ? 'am-active' : ''?>"><a href="/qwtkjba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
                            </ul>
                            </form>
      ...
      <a href="<?=$_SERVER['PATH_INFO']?>?qid=<?=$result['qid']?>&amp;export=csv" class="am-btn am-btn-default am-btn-secondary"> <span class="am-icon-save"></span> <span class="downloadbtn">导出未处理订单 </span></a>
                            </div>
                        </div>
                        <?php if ($result['state']==0):?>
                        <div class="am-u-sm-12 am-u-md-3">
                            <div class="am-btn-toolbar">
                                    <a id="piliangfahuo" data-toggle="modal" data-target="#shipModel" class="csv am-btn am-btn-default am-btn-secondary"><span class="am-icon-shopping-cart"></span> 批量发货</a>
                            </div>
                        </div>
                    <?php endif?>
          <form method="get" name="ordersform">
          ...
          <div class="tab-pane active" id="orders<?=$result['state']?>">
                    <div class="am-g">
            ...
            <form method="post" class="am-form">
                ...
                <tr>
                  ...
                  <td>
                    <a href="/qwtkjba/cutlist/<?=$order->eid?>"><?=$order->user->nickname?></a>
                  </td>
                  <td><?=$order->order_state==1?'已付款':'未付款'?></td>
                  <td><?=$order->shiptype?></td>
                  <td><?=$order->shipcode?></td>
                  <td><?=$order->item->name?></td>
                  <td><?=round($order->item->old_price/100,2)?></td>
                  <td><?=ORM::factory('qwt_kjbcut')->where('bid','=',$bid)->where('eid','=',$order->eid)->count_all()?>/<?=$order->item->cut_num?></td>
                  <td><?=round($order->pay_money/100,2)?></td>
                  <td><?=$order->pay_time==0?date('Y-m-d H:i:s',$order->lastupdate):date('Y-m-d H:i:s',$order->pay_time)?></td>
                  <td><?=$order->name?></td>
                  <td><?=$order->tel?></td>
                  <td><?=$order->address?></td>
                    <?php if ($result['state'] == 0):?>
                  <td nowrap="">
                    <a class="edit am-btn am-btn-default am-btn-xs am-text-secondary" data-toggle="modal"  data-openid="<?=$order->user->openid?>" data-name="<?=$order->item_name?>" data-id="<?=$order->id?>" data-linkman="<?=$order->name?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->address?>" style="background-color:#fff;">
                      <span class="am-icon-pencil-square-o"></span> 处理
                    </a>
                    <input type="hidden" name="id[]" value="<?=$order->id?>">

                      <a class="edit2 am-btn am-btn-default am-btn-xs am-text-secondary" data-toggle="modal" data-id="<?=$order->id?>" data-name="<?=$order->item_name?>" data-linkman="<?=$order->name?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->address?>" style="background-color:#fff;">
              ...
              <?php if ($result['state'] == 0 && $order->id):?>
                    <div class="am-g">
