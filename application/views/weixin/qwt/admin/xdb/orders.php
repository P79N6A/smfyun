...
?>
            <ol class="am-breadcrumb">
                ...
                <li><a href="#">奖品设置</a></li>
                <li><a href="/qwtxdba/orders">兑换记录</a></li>
          ...
          <form method="get" name="ordersform">
                            <ul class="am-nav am-nav-tabs" style="left:0;">
                                <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'am-active' : ''?>"><a href="/qwtxdba/orders?qid=<?=$result['qid']?>">未处理订单</a></li>
                                <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'am-active' : ''?>"><a href="/qwtxdba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
                            </ul>
                            </form>
          ...
          <?php if ($result['status'] == 0 ):?>
                            <div class="actions">
                                <ul class="actions-btn">
                                    <a href="/qwtxdba/orders?qid=<?=$result['qid']?>&type=all"><li tag='total' class='<?=$activetype == 'total' ? 'red-on' : 'red'?>'>全部</li></a>
                                    <a href="/qwtxdba/orders?qid=<?=$result['qid']?>&type=object"><li tag='object' class='<?=$activetype == 'object' ? 'green-on' : 'green'?>'>实物</li></a>
                                    <a href="/qwtxdba/orders?qid=<?=$result['qid']?>&type=fare"><li tag='fare' class='<?=$activetype == 'fare' ? 'blue-on' : 'blue'?>'>话费/充值</li></a>
                                    <a href="/qwtxdba/orders?qid=<?=$result['qid']?>&type=hb"><li tag='hb' class='<?=$activetype == 'hb' ? 'purple-on' : 'purple'?>'>微信红包</li></a>
                                </ul>
                            </div>
          <?php endif?>
                        </div>
          ...
          <div class="tab-pane active" id="orders<?=$result['status']?>">
            ...
            <form method="post" class="am-form">
                ...
                <tr>
                  ...
                  <td>
                    <a href="/qwtxdba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
                  ...
                  <th><?=number_format($order->need_money/100,2)?></th>
                  <td><?=$order->memo?></td>
                      ...
                      <div class="am-u-sm-12">
                        <form method="post" class="am-form am-form-horizontal" name="qrcodesform">
                          ...
                          <div class="am-form-group chulibox type2">
                            ...
                            <div class="am-u-sm-9">
            <input class="form-control" name="shiptype" maxlength="6" style="width:150px" type="text" value="<?=$_SESSION['qwtxdba']['shiptype']?>">
                            </div>
                          </div>
                          <div class="am-form-group chulibox type2">
                            <label for="shiptype" class="am-u-sm-3 am-form-label">快递单号：</label>
                            <div class="am-u-sm-9">
            <input class="form-control" name="shipcode" maxlength="20" style="width:150px" type="text" value="<?=$_SESSION['qwtxdba']['shipcode']?>">
                            </div>
                          </div>
