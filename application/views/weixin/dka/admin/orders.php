...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dkaa/orders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            <ul class="nav nav-tabs">
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/dkaa/orders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/dkaa/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/dkaa/orders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/dkaa/orders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/dkaa/orders?qid=<?=$result['qid']?>&type=fare">话费/充值</a></span>
          <span tag='hb' class='tag <?=$activetype == 'hb' ? 'tactive' : ''?>'><a href="/dkaa/orders?qid=<?=$result['qid']?>&type=hb">红包</a></span>
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/dkaa/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['dkaa']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['dkaa']['shipcode']?>"></div>'
  ...
  //modal.find('form').attr('action', '/dkaa/orders/ship/' + id + '?p=<?=$_GET['p']?>')
  }else if(tag==4){
