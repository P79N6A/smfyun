...
<div class="top">
    <a class="btn btn-danger" href="/rwb/orders">查看我已经兑换的产品</a>
</div>
...
foreach ($items as $item):
    $item = (object)$item;
    //判断限购
    $limit = ORM::factory('rwb_order')->where('qid', '=', $user2['id'])->where('iid', '=', $item->id)->count_all();
?>

        <li>
            <div class="cd-single-item">
                <ul class="cd-slider-wrapper">
                    <li class="selected"><img src="http://cdn.jfb.smfyun.com/rwb/images/item/<?=$item->id?>.v<?=$item->lastupdate?>0.jpg" alt="<?=$item->name?>"></li>
                </ul>
            </div> <!-- .cd-single-item -->
            <div class="cd-item-info">
                <b><a href="/rwb/neworder/<?=$item->id?>"><?=$item->name?></a> (还有<?=$item->stock?>件)</b>
                <em>价值 <?=$item->price?> 元 / <?=$item->score?> <?=$scorename?></em>
            </div> <!-- cd-item-info -->
            ...
            <div class="cd-customization">
                ...
                <?php elseif ($user2['score'] < $item->score):?>
                    <button type="submit" class="add-to-cart disabled" disabled="">您的<?=$scorename?>不够</button>
                ...
                <?php else:?>
                    <form action="/rwb/neworder/<?=$item->id?>"><button type="submit" class="add-to-cart">立即兑换</button></form>
                <?php endif?>
