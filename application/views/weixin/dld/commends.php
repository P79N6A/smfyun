...
<div class="bd">
        <div class="weui_panel weui_panel_access">
            ...
            <img src="/dld/img/banner.png" style="width:100%;height:100%">
            ...
            <?php foreach ($result['commends'] as $v):?>
              ...
              if($v->type==1){//多规格
                if($user->group_id>0){//有分组
                  $sku_commends = ORM::factory('dld_smoney')->where('bid','=',$bid)->where('sid','=',$user->group_id)->where('item_id','=',$v->num_iid)->find_all();
                }else{
                  $sku_commends = ORM::factory('dld_goodsku')->where('bid','=',$bid)->where('item_id','=',$v->num_iid)->find_all();
                }
                foreach ($sku_commends as $key => $value) {
                  $sku = ORM::factory('dld_goodsku')->where('bid','=',$bid)->where('sku_id','=',$value->sku_id)->find();
              ...
              ?>
              <a href="/dld/shareopenid/<?=base64_encode($result['openid'])?>/<?=$v->id?>/<?=$bid?>" class="weui_media_box weui_media_appmsg" style="padding:10px;">
