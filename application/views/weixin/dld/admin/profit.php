...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dlda/qrcodes">销售利润结算账单</a></li>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover nowrap">
                ...
                foreach ($result['qrcodes'] as $v):
                    ...
                    $typearry[0]=5;
                    $typearry[1]=6;
                    $score=ORM::factory('dld_score')->where('bid','=',$v->bid)->where('qid','=',$v->id)->where('type','IN',$typearry)->find();
                    if($score->id){
                      $flag=1;
                    }else{
                      $flag=0;
                    ...
                    $monthjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthjs_pxmoney from dld_scores where bid=$v->bid and qid = $v->id and type in (1,5,6) and paydate < $nawtime   ")->execute()->as_array();
                ...
                <tr>
                  ...
                  <td><a href="/dlda/history_scores?qid=<?=$v->id?>"><?=$v->nickname?></td>
                  <td><?=$v->name?></td>
                  <td><?=$v->tel?></td>
                  <td><?=$v->alipay_name?></td>
                  <!-- <td><?=$month_tmoney?></td>
                  ...
                  <td><a href="/dlda/history_scores?flag=lr&qid=<?=$v->id?>"><?=number_format(-$monthyjs_pxmoney,2)?></td>
                  ...
                  <a href="#" data-toggle="modal" data-target="#actionModel" data-id="<?=$v->id?>" data-lv="<?=$v->lv?>" data-name="<?=$v->nickname?>" data-tel='<?=$v->tel?>' data-uname='<?=$v->name?>' data-money='<?=$monthjs_pxmoney?>' data-bz='<?=$v->bz?>'>
