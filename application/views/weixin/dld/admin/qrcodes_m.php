...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dlda/qrcodes">代理商设置</a></li>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover nowrap">
                ...
                foreach ($result['qrcodes'] as $v):
                  ...
                  //   $group_num=0;
                  // }
                  $group_num = ORM::factory('dld_qrcode')->where('lv','=',1)->where('bid','=',$v->bid)->where('fopenid','=',$v->openid)->count_all();
                  ...
                  $qr_num=ORM::factory('dld_qrcode')->where('bid','=',$v->bid)->where('fopenid','=',$v->openid)->where('lv','!=',1)->where('fopenid','!=','')->count_all();
                   ...
                   $month=date('Y-m',time());
                    ...
                    $nickname=ORM::factory('dld_qrcode')->where('bid','=',$v->bid)->where('openid','=',$v->fopenid)->where('lv','=',1)->find()->nickname;
                    if($nickname){
                      $fname = $nickname;
                    }
                    // echo $all_pxmoney.'<br>';
                ...
                <tr>
                  ...
                  <td><a href="/dld/memberpage/<?=$v->openid?>"><?=$v->nickname?></td>
                  <td id="lock<?=$v->id?>">
                  <?php
                  $fuser = ORM::factory('dld_qrcode')->where('bid','=',$v->bid)->where('openid','=',$v->fopenid)->find();
                  if ($v->lv == 1)
                  ...
                  <td><a href="/dlda/qrcodes_m?qid=<?=$v->id?>"><?=$group_num?></td>
                  <td><a href="/dlda/customers?qid=<?=$v->id?>"><?=$qr_num?></td>
                  <td><a href="/dlda/history_trades?flag=dayp&qid=<?=$v->id?>"><?=$day_pnum?$day_pnum:0?></td>
                  <td><a href="/dlda/history_trades?flag=monthp&qid=<?=$v->id?>"><?=$month_pnum?$month_pnum:0?></td>
                  <td><a href="/dlda/history_trades?flag=allp&qid=<?=$v->id?>"><?=$all_pnum?$all_pnum:0?></td>
                  <td><a href="/dlda/history_trades?flag=dayt&qid=<?=$v->id?>"><?=$day_tnum?></td>
                  <td><a href="/dlda/history_trades?flag=montht&qid=<?=$v->id?>"><?=$month_tnum?></td>
                  <td><a href="/dlda/history_trades?flag=allt&qid=<?=$v->id?>"><?=$all_tnum?></td>
                  <td><?=$month_tmoney?></td>
                  <td><?=$month_pmoney?></td>
