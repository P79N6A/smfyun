...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dlda/qrcodes">个人团队奖励结算账单</a></li>
              ...
              <table class="table table-hover nowrap">
                ...
                foreach ($result['qrcodes'] as $v):
                   $groups=ORM::factory('dld_group')->where('bid','=',$v->bid)->where('qid','=',$v->id)->find_all();
                    ...
                    $typearry[0]=2;
                    $typearry[1]=3;
                    $score=ORM::factory('dld_score')->where('bid','=',$v->bid)->where('qid','=',$v->id)->where('bz','=',$month)->where('type','IN',$typearry)->find();
                    if($score->id){
                      $flag=1;
                    }else{
                      $flag=0;
                    ...
                    //累计销售利润
                    //已结算团队奖励
                    $monthyjs_pmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthyjs_pmoney from dld_scores where bid=$v->bid and qid = $v->id and type IN (2,3) and FROM_UNIXTIME(`lastupdate`, '$monthtype')='$month' ")->execute()->as_array();
                ...
                <tr>
                  ...
                  <td><a href="/dlda/history_scores?flag=jl&qid=<?=$v->id?>"><?=number_format(-$monthyjs_pmoney,2)?></td>
                  <td><?=$fname?></td>
                  <td id="lock<?=$v->id?>">
                  <?php
                  if ($flag == 1)
