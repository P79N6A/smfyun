...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/qwtwfba/orders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            ...
            <ul class="nav nav-tabs">
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/qwtwfba/orders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/qwtwfba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/qwtwfba/orders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/qwtwfba/orders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/qwtwfba/orders?qid=<?=$result['qid']?>&type=fare">话费/充值</a></span>
        </div>
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/qwtwfba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
                ...
                <script type="text/javascript">
                    $('.edit').click(function(){
                      if($(this).data('addr')==' '){
                        ...
                        $('#id').val($(this).data('id'));
                        $(".openid").text($(this).data('openid'));
                        $('.addresstel').html('<strong>收货地址:<span class="addr">'+$(this).data('addr')+'</span></strong>');
                        $(".man").text($(this).data('linkman'));
                      }
                    })
                </script>
...
<form method="post" >
    <div class="col-md-9 img-modal">
      ...
      <span class='cp'><input  value="<?=$_SESSION['qwtwfba']['shiptype']?>" class="form-control" name="shiptype"></span>
      </p>
      <p class='cp'><strong>快递单号:</strong>
      <span class='cp'><input  value="<?=$_SESSION['qwtwfba']['shipcode']?>" class="form-control" name="shipcode"></span>
      </p>
    </div>
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['qwtwfba']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['qwtwfba']['shipcode']?>"></div>'
