...
<section class="wrapper" style="width:85%;float:right;">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/qwtwdba/qrcodes">用户明细</a></li>
    ...
    <div class="col-xs-16">
        <div class="box box-success">
            <div class="box-header">
              <h3 class="box-title" style="margin-left:10px"><?=$title?>：共 <?=$result['countall']?> 个用户</h3>
              <div class="box-tools" style="margin-left:10px">
                ...
                <div class="col-sm-14" style="margin-left:-14px">
                ...
                <tr>
                  ...
                  <th>头像</th>
                  <th>昵称</th>
                  <th><a href="/qwtwdba/qrcodes?sort=score&amp;s=<?=$result['s']?>&amp;lock=<?=$result['lock']?>" title="按积分排序">现有积分</a></th>
                  <th>兑换数量</th>
                  <th>性别</th>
                  <th>直接粉丝</th>
                  <th>间接粉丝</th>
                  <th><a href="/qwtwdba/qrcodes?sort=jointime&amp;s=<?=$result['s']?>&amp;lock=<?=$result['lock']?>" title="按关注时间排序">关注时间</a></th>
                  <th>是否有生成海报？</th>
                  <th>来源用户</th>
                  <th>操作</th>
                </tr>
                ...
                foreach ($result['qrcodes'] as $v):
                  $count2 = ORM::factory('qwt_wdbqrcode')->where('bid', '=', $v->bid)->where('fopenid', '=', $v->openid)->count_all();
                  $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_wdbqrcodes WHERE fopenid='$v->openid'")->execute()->as_array();
                  $tempid=array();
                  ...
                  $count3 = ORM::factory('qwt_wdbqrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->count_all();
                  $fuser='';
                  if($v->fopenid){
                      $fuser = ORM::factory('qwt_wdbqrcode')->where('bid', '=', $v->bid)->where('openid', '=', $v->fopenid)->find();
                  }
                  $count4 =ORM::factory('qwt_wdborder')->where('bid','=',$v->bid)->where('qid','=',$v->id)->count_all();
                ?>
                </thead>
                <tbody>
                <tr>
                  ...
                  $url = '/qwtwdb/index/'. $v->bid .'?url=score&cksum='. $cksum .'&openid='. base64_encode($v->openid);
                  ?>
                  <td id="score<?=$v->id?>"><a href="<?=$url?>" target="_blank" title="查看用户前台"><?=$v->score?></a></td>
                  <td id="score1<?=$v->id?>"><a href="/wdba/orders?qid=<?=$v->id?>" title="查看兑换明细"><?=$count4?></a></td>
                  <td><?=$sex[$v->sex]?></td>
                  <td><a href="/qwtwdba/qrcodes?fopenid=<?=$v->openid?>" title="查看推荐明细"><?=$count2?></a></td>
                  <td><a href="/qwtwdba/qrcodes?ffopenid=<?=$v->openid?>" title="查看推荐明细"><?=$count3?></a></td>
                  <td><?=date('Y-m-d H:i', $v->jointime)?></td>
                  <td><?=$v->ticket ? '是' : '否'?></td>
                  <td><a href="/qwtwdba/qrcodes?id=<?=$fuser->id?>"><?=$fuser->nickname?></a></td>
                  <td nowrap="">
                    <a class='edit' href='#myModal' data-id="<?=$v->id?>" data-lock="<?=$v->lock?>" data-name="<?=$v->nickname?>" data-toggle="modal" >
        ...
        <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div style="margin-left:-20px">
                    ...
                    <label class="checkbox-inline" style="margin-left:-15px"><input class='checkedit' name="form[lock]" id="flock4" value="4" type="radio"><span class="label label-info" style="font-size:14px">隐身用户</span></label></div> <p class="help-block">1、加入白名单后不会自动锁定<br>2、隐身用户不会出现在积分排行中</p>
            </div><input class='edithidden' name="form[id]" value='' type="hidden"></div>
