...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/myba/qrcodes">用户明细</a></li>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                <tbody><tr>
                  ...
                  <th>昵称</th>
                  <!-- <th>OpenID</th> -->
                  <th nowrap=""><a href="/myba/qrcodes?sort=money" title="按总收益排序">总收益</a></th>
                  <th nowrap=""><a href="/myba/qrcodes?sort=score" title="按收益排序">未提现收益</a></th>
                  <th nowrap=""><a href="/myba/qrcodes?sort=paid" title="按订单收入排序">订单收入</a></th>
                  <th nowrap=""><a href="/myba/qrcodes?sort=shscore" title="按现有积分排序">现有积分</a></th>
                  <th>性别</th>
                  <th>下线</th>
                  <th>二级</th>
                  ...
                  <th>三级</th>
                  <?endif;?>
                  <th><a href="/myba/qrcodes?sort=jointime" title="按关注时间排序">关注时间</a></th>
                  <th>有海报？</th>
                  <th>上线</th>
                  <th>操作</th>
                </tr>
                ...
                foreach ($result['qrcodes'] as $v):
                  ...
                  $firstchild=DB::query(Database::SELECT,"SELECT openid FROM myb_qrcodes WHERE fopenid='$v->openid'")->execute()->as_array();
                  $tempid=array();
                  ...
                  if($config['kaiguan_needpay']==1){
                      $tempdata = ORM::factory('myb_qrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->find_all();
                      // var_dump($tempdata);
                      $i=0;
                      $resid=array();
                  ...
                  $count3 = ORM::factory('myb_qrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->count_all();
                  $count4 = ORM::factory('myb_qrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$resid)->count_all();
                ...
                <tr>
                  ...
                  $url = '/myb/index/'. $v->bid .'?url=home&cksum='. $cksum .'&openid='. base64_encode($v->openid);
                  ?>
                  </td>
                  ...
                  <td><?=$sex[$v->sex]?></td>
                  <td><a href="/myba/qrcodes?fopenid=<?=$v->openid?>" title="查看下线"><?=$count2?></a></td>
                  <td><a href="/myba/qrcodes?ffopenid=<?=$v->openid?>" title="查看二级"><?=$count3?></a></td>
                  <?if($config['kaiguan_needpay']==1):?>
                  <td><a href="/myba/qrcodes?fffopenid=<?=$v->openid?>" title="查看三级"><?=$count4?></a></td>
                  <?endif?>
