...
<div id="rankpage">
    <section id="ranking">
        ...
        <section class="box block-item">
                  ...
                  <section class="col_1 cola">现积分</section>
                  <section class="col_1 cola">一级</section>
        ...
        $rank++;
        $ischild=ORM::factory('ytb_qrcode')->where('bid','=',$bid)->where('openid','=',$user->openid)->find();
        ...
        {
           $ischildd=ORM::factory('ytb_qrcode')->where('bid','=',$bid)->where('openid','=',$ischild->fopenid)->find();
        ...
        $count2 = ORM::factory('ytb_qrcode')->where('bid', '=', $bid)->where('fopenid', '=', $user->openid)->count_all();

        $firstchild=DB::query(Database::SELECT,"SELECT openid FROM ytb_qrcodes WHERE fopenid='$user->openid' and bid='$bid'")->execute()->as_array();
         ...
         {
              $tempdata = ORM::factory('ytb_qrcode')->where('bid', '=', $bid)->where('fopenid', 'IN',$tempid)->find_all()->as_array();
        ...
        $count4 = ORM::factory('ytb_qrcode')->where('bid', '=', $bid)->where('fopenid', 'IN',$tempiid)->count_all();
        $count3 = ORM::factory('ytb_qrcode')->where('bid', '=', $bid)->where('fopenid', 'IN',$tempid)->count_all();
        ?>
                <section class="box block-item" style='height: auto;line-height: 20px;'>

                  <section class="col_1"><?=$rank+($pagenum-1)*500?></section>
                  <!-- <section class="col_2"><img src="<?=$user->headimgurl?>" /></section> -->
                  <!-- <section class="col_1"><?//=($ischild)?'一级':'二级'?></section> -->
                  <section class="col_1"><?=$kind?></section>
                  <section class="col_1"><?=$user->nickname?></section>

                  <section class="col_1"><?=$user->score?></section>
