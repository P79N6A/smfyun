...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/ytba/shorders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="shordersform">
            <ul class="nav nav-tabs">
              <li id="shorders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/ytba/shorders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="shorders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/ytba/shorders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <!-- <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/ytba/shorders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/ytba/shorders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/ytba/shorders?qid=<?=$result['qid']?>&type=fare">优惠券</a></span>
          <span tag='hb' class='tag <?=$activetype == 'hb' ? 'tactive' : ''?>'><a href="/ytba/shorders?qid=<?=$result['qid']?>&type=hb">红包</a></span> -->
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/ytba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['ytba']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['ytba']['shipcode']?>"></div>'
  ...
  //modal.find('form').attr('action', '/ytba/shorders/ship/' + id + '?p=<?=$_GET['p']?>')
  }else if(tag==4){
