...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/ytba/qrcodes">分销商品管理</a></li>
    ...
    <div class="col-xs-12">
        <div class="box box-success">
         ...
         <!--      <div class="box-tools">
            ...
            <?php
            $config['kaiguan_needpay']=ORM::factory('ytb_cfg')->where('bid','=',$bid)->where('key','=','kaiguan_needpay')->find()->value;
            ?>
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                {
                  $mon=ORM::factory('ytb_setgood')->where('goodid','=',$v['num_iid'])->where('bid','=',$bid)->find();
                   if($mon->id)
                   {
                   	$money0=$mon->money0;
                   	$money1=$mon->money1;
                   ...
                   {
                   	$monn = ORM::factory('ytb_cfg')->getCfg($bid,1);
                   	$money0=$monn['money0'];
                   	$money1=$monn['money1'];
                   	$money2=$monn['money2'];
                    $money3=$monn['money3'];
                   ...
                   $goodid=$v['num_iid'];
                  $soldednum=DB::query(database::SELECT,"select sum(temp.num)as tonum from (SELECT ytb_orders.* FROM `ytb_trades`,ytb_orders WHERE ytb_orders.tid=ytb_trades.tid and ytb_trades.status!='TRADE_CLOSED' and ytb_trades.status!='TRADE_CLOSED_BY_USER' and ytb_trades.status!='NO_REFUND' and ytb_orders.goodid=$goodid) as temp where temp.bid=$bid")->execute()->as_array();
                   	//var_dump($soldednum);
                ?>
                ...
                <tr>
                  ...
                  <td nowrap="">
                    <a href="#" data-toggle="modal" data-target="#actionModel" data-num_id="<?=$v[num_iid]?>" data-price="<?=$v['price']?>" data-name="<?=$v['title']?>" data-money0="<?=$money0?>" data-money1="<?=$money1?>" data-money2="<?=$money2?>" data-money3="<?=$money3?>">
