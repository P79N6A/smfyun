...
function check($auth,$lv){
    $content = '';
    $len = strlen($auth);

    $content = strpos($auth,'a')===false?$content.'':$content.',0';
    $content = strpos($auth,'b')===false?$content.'':$content.',1';
    $content = strpos($auth,'c')===false?$content.'':$content.',2';
    $content = strpos($auth,'d')===false?$content.'':$content.',3';

    if(strpos($content,$lv)!==false){
      $flag = 0;
    }else{
      $flag = 1;
    }
...
$hasbuy = ORM::factory('ytb_trade')->where('qid','=',$user2['id'])->where('status','=','TRADE_BUYER_SIGNED')->count_all();
?>
<head>
    <meta charset="UTF-8">
  ...
  <body class="page-detail" data-page="detail" ontouchstart="">
    <div class="view js-page-view" id="page-view-detail">
        <div class="detail-header" id="detail-header">
              ...
              <div style="width: auto;"> <img src="http://<?=$_SERVER['HTTP_HOST']?>/ytb/images/item/<?=$item->id?>.v<?=$item->lastupdate?>0.jpg" alt="<?=$item->name?>">
           ...
           <div id="pay-bar" class="pay-bar"> 单价：
            <span class="pay-total js-total-price">
             <b class="coins"><?=$item->score?></b><?=$config['score_name']?>
           </span>
                <?php if ($hasbuy==0):?>
                    <a style="width:200px" href="javascript:;" class="pay-btn js-pay disabled">请您先购买完成一笔订单哦</a>
                <?php elseif (check($item->auth,$user2['lv'])==1):?>
                    <a style="width:200px" href="javascript:;" class="pay-btn js-pay disabled"><?=auth($item->auth,$config)?></a>
                <?php elseif ($user2['score'] < $item->score):?>
                    <a href="javascript:;" class="pay-btn js-pay disabled">您的<?=$config['score_name']?>不够</a>
                ...
                <?php else:?>
                <a href="/ytb/neworder/<?=$item->id?>" class="pay-btn">立即兑换</a>
                <?php endif?>
