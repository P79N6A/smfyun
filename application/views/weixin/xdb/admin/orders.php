...
?>
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/xdba/orders">兑换记录</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            <ul class="nav nav-tabs">
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 0 ? 'active' : ''?>"><a href="/xdba/orders?qid=<?=$result['qid']?>">未处理订单</a>
              </li>
              <li id="orders<?=$result['status']?>" class="<?=$result['status'] == 1 ? 'active' : ''?>"><a href="/xdba/orders/done?qid=<?=$result['qid']?>">已处理订单</a></li>
          ...
          <span tag='total' class='tag <?=$activetype == 'total' ? 'tactive' : ''?>'><a href="/xdba/orders?qid=<?=$result['qid']?>&type=all">全部</a></span>
          <span tag='object' class='tag <?=$activetype == 'object' ? 'tactive' : ''?>'><a href="/xdba/orders?qid=<?=$result['qid']?>&type=object">实物</a></span>
          <span tag='fare' class='tag <?=$activetype == 'fare' ? 'tactive' : ''?>'><a href="/xdba/orders?qid=<?=$result['qid']?>&type=fare">话费/充值</a></span>
          <span tag='hb' class='tag <?=$activetype == 'hb' ? 'tactive' : ''?>'><a href="/xdba/orders?qid=<?=$result['qid']?>&type=hb">红包</a></span>
            ...
            <table class="table table-striped">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <a href="/xdba/qrcodes?id=<?=$order->user->id?>"><?=$order->user->nickname?></a>
                  </td>
                  ...
                  <td><?=$order->memo?></td>
                  <?php if($result['status']!=0):?>
                  <td nowrap=""><?=$order->hdatetime?date('m-d H:i', $order->hdatetime):date('m-d H:i', $order->lastupdate)?></td>
                <?php endif;?>
                <?php if($result['status']==0):?>
                   <td nowrap=""></td>
                   <?php endif;?>
                  ...
                  <td nowrap="">
                    ...
                    <a href="#" data-toggle="modal" data-target="#actionModel" data-openid="<?=$order->user->openid?>" data-name="<?=$order->item->name?>" data-id="<?=$order->id?>" data-linkman="<?=$order->name?>" data-tag="<?=$order->type?>" data-tel="<?=$order->tel?>" data-addr="<?=$order->city.' '.$order->address?>" >
                      <span>核销</span> <i class="fa fa-check-square"></i>
                    </a>
              ...
              <button type="submit" class="btn btn-success pull-right">一键核销本页订单</button>
              <?php endif?>
              </form>
              </div>
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var form = '<div class="form-group"><label for="shiptype">快递公司：</label><input name="shiptype" maxlength="6" type="text" value="<?=$_SESSION['xdba']['shiptype']?>"></div>'
  form += '<div class="form-group"><label for="shiptype">快递单号：</label><input name="shipcode" maxlength="20" type="text" value="<?=$_SESSION['xdba']['shipcode']?>"></div>'
  var remark='<div class="form-group"><label for="remark">核销备注：</label><input name="remark" maxlength="6" type="text" value="<?=$_SESSION['xdba']['remark']?$_SESSION['xdba']['remark']:'西单大悦城'?>"></div><input type="hidden" name="flag" value="1">'
  $('#id').val(id)

  var modal = $(this)
  modal.find('.modal-title').text(name)
  if(tag==3){
    modal.find('.modal-body').html('<div class="form-group"><label>OpenID：</label>'+ button.data('openid') + '</div><div class="form-group"><label>收货人：</label>'+ man +'</div><div class="form-group"><label>手机号：</label>'+ tel + '</div>'+remark )
  //modal.find('form').attr('action', '/xdba/orders/ship/' + id + '?p=<?=$_GET['p']?>')
  }else if(tag==4){
    modal.find('.modal-body').html('<div class="form-group"><label>OpenID：</label>'+ button.data('openid')+remark  )
  //modal.find('form').attr('action', '/wdya/orders/ship/' + id + '?p=<?=$_GET['p']?>')
  }else{
    modal.find('.modal-body').html('<div class="form-group"><label>OpenID：</label>'+ button.data('openid') + '</div><div class="form-group"><label>收货人：</label>'+ man +'</div><div class="form-group"><label>收货地址：</label>'+ addr + '</div>' + form+remark )
  //modal.find('form').attr('action', '/xdba/orders/ship/' + id + '?p=<?=$_GET['p']?>')
  }
