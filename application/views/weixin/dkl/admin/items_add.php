...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dkla/items">奖品管理</a></li>
...
// });
      ...
      <div class="form-group">
        ...
        <label class="checkbox-inline">
            <input id='true' type="radio" name="data[type]" value="0" onclick="change(3)"<?=$type==0 ? ' checked="checked"' : ''?>>
          ...
          <label class="checkbox-inline">
            <input id='xn' type="radio" name="data[type]" value="2" onclick="change(2)"<?=$type==2 ? ' checked="checked"' : ''?>>
            <span class="label label-success" style="font-size:14px">有赞优惠券/优惠码</span>
          </label>
           <label class="checkbox-inline">
            <input type="radio" name="data[type]" value="6" onclick="change(7)"<?=$type==6 ? ' checked="checked"' : ''?>>
            <span class="label label-success" style="font-size:14px">有赞会员卡</span>
          </label>
          <label class="checkbox-inline">
            <input type="radio" name="data[type]" value="3" onclick="change(4)"<?=$type==3 ? ' checked="checked"' : ''?>>
            <span class="label label-success" style="font-size:14px">特权商品</span>
          </label>
          <label class="checkbox-inline">
          ...
          <label class="checkbox-inline">
            <input type="radio" name="data[type]" value="5" onclick="change(6)"<?=$type==5 ? ' checked="checked"' : ''?>>
      ...
      <div class="form-group">
        ...
        <?php if ($result['action'] == 'edit' && $result['item']['pic']):?>
          <div class="form-group"><img class="img-thumbnail" src="/dkla/images/item/<?=$result['item']['id']?>.v<?=$result['item']['lastupdate']?>.jpg" width="100"></div>
...
4、请商户按照上述要求操作，因为运营操作不当、兑换门槛过低导致的损失，与我方无关；</label>
      ...
      <div id="tab-content" class="form-group" <?=($type==0) ? ' style="display:none;"' : ''?>>
        <div class="tab-content" >
          ...
          <option <?=$_POST['data']['value1']==$wxcard['id']?"selected":""?> value="<?=$wxcard['id']?>"><?=$wxcard['title']?></option>
          <?php endforeach; ?>
           <?php endif;?>
          </select>

          <select name="yzcode"  class="form-control yzcode" <?=($type==2)?'':'style="display:none"'?>>
          <?php if($yzcoupons):?>
          ...
          <select name="yzgift"  <?=($type==5)?'':'style="display:none"'?> class="form-control yzgift" >
          <?php if($yzgifts):?>
          ...
          <select name="yzvip" <?=($type==6)?'':'style="display:none"'?> class="form-control yzvip" >
          <?php if($yzmembers):?>
          <?php foreach ($yzmembers as $yzmember):?>
          <option <?=$_POST['data']['value1']==$yzmember['card_url']?"selected":""?> value="<?=$yzmember['card_url']?>"><?=$yzmember['name']?></option>
           <?php endforeach; ?>
            <?php endif;?>
          </select>
          <input type="text" class="form-control wered" name="wered" placeholder="请输入红包金额" <?=($type==4)?'value='.$_POST['data']['value1']:'style="display:none"'?>><span class="form-control wered" <?=($type==4)?'':'style="display:none"'?>>元</span>
          <input type="text" class="form-control goodtag" name="goodtag" placeholder="请输入标签" <?=($type==3)?'value='.$_POST['data']['value1']:'style="display:none"'?>>
          </label>
          <input type="text" class="form-control goodlink" name="data[url]" placeholder="请输入链接" value="<?=$_POST['data']['url']?>" <?=($type==3)?'':'style="display:none"'?>>

        </div>
      </div>
      <!--微信红包-->
      <div id="hongbao-content" class="form-group" <?=($type==1||$type==2||$type==3||$type==5||$type==6||$type==7) ? ' style="display:none;"' : ''?>>
        <div class="tab-content" >
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="/dkla/items/edit/<?=$result['item']['id']?>?DELETE=1" class="btn btn-outline">确认删除</a>
      </div>
