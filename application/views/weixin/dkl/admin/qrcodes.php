...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/dkla/qrcodes">用户明细</a></li>
            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                <tbody><tr>
                  ...
                  <th>头像</th>
                  <th>昵称</th>
                  <th><a href="/dkla/qrcodes?sort=score&amp;s=<?=$result['s']?>&amp;lock=<?=$result['lock']?>" title="按积分排序">现有积分</a></th>
                  <th>兑换数量</th>
                  <th>性别</th>
                  <th>直接粉丝</th>
                  <th>间接粉丝</th>
                  <th><a href="/dkla/qrcodes?sort=jointime&amp;s=<?=$result['s']?>&amp;lock=<?=$result['lock']?>" title="按关注时间排序">关注时间</a></th>
                  <th>关注状态</th>
                  <th>是否有生成海报？</th>
                  <th>来源用户</th>
                  <th>操作</th>
                ...
                foreach ($result['qrcodes'] as $v):
                  $count2 = ORM::factory('dkl_qrcode')->where('bid', '=', $v->bid)->where('fopenid', '=', $v->openid)->count_all();
                  $firstchild=DB::query(Database::SELECT,"SELECT openid FROM dkl_qrcodes WHERE fopenid='$v->openid'")->execute()->as_array();
                  $tempid=array();
                  ...
                  $count3 = ORM::factory('dkl_qrcode')->where('bid', '=', $v->bid)->where('fopenid', 'IN',$tempid)->count_all();
                  $fuser = ORM::factory('dkl_qrcode')->where('bid', '=', $v->bid)->where('openid', '=', $v->fopenid)->find();
                  $count4 =ORM::factory('dkl_order')->where('bid','=',$v->bid)->where('qid','=',$v->id)->count_all();
                ?>
                ...
                <tr>
                  ...
                  $url = '/dkl/index/'. $v->bid .'?url=score&cksum='. $cksum .'&openid='. base64_encode($v->openid);
                  ?>
                  <td id="score<?=$v->id?>"><a href="<?=$url?>" target="_blank" title="查看用户前台"><?=$v->score?></a></td>
                  <td id="score1<?=$v->id?>"><a href="/dkla/orders?qid=<?=$v->id?>" title="查看兑换明细"><?=$count4?></a></td>
                  <td><?=$sex[$v->sex]?></td>
                  <td><a href="/dkla/qrcodes?fopenid=<?=$v->openid?>" title="查看推荐明细"><?=$count2?></a></td>
                  <td><a href="/dkla/qrcodes?ffopenid=<?=$v->openid?>" title="查看推荐明细"><?=$count3?></a></td>
                  ...
                  <td><?=$v->ticket ? '是' : '否'?></td>
                  <td><a href="/dkla/qrcodes?id=<?=$fuser->id?>"><?=$fuser->nickname?></a></td>
                  <td nowrap="">
