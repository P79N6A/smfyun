...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/fxba/qrcodes">分销商品管理</a></li>
    ...
    <div class="col-xs-12">
        <div class="box box-success">
         ...
         <!--      <div class="box-tools">
            ...
            <?php
            $config['kaiguan_needpay']=ORM::factory('fxb_cfg')->where('bid','=',$bid)->where('key','=','kaiguan_needpay')->find()->value;
            ?>
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                {
                  $mon=ORM::factory('fxb_setgood')->where('goodid','=',$v['num_iid'])->where('bid','=',$bid)->find();
                   if($mon->id)
                   {
                   	$money0=$mon->money0;
                   	$money1=$mon->money1;
                   ...
                   {
                   	$monn = ORM::factory('fxb_cfg')->getCfg($bid,1);
                   	$money0=$monn['money0'];
                   	$money1=$monn['money1'];
                   	$money2=$monn['money2'];
                    $money3=$monn['money3'];
                   ...
                   $goodid=$v['num_iid'];
                  $soldednum=DB::query(database::SELECT,"select sum(temp.num)as tonum from (SELECT fxb_orders.* FROM `fxb_trades`,fxb_orders WHERE fxb_orders.tid=fxb_trades.tid and fxb_trades.status!='TRADE_CLOSED' and fxb_trades.status!='TRADE_CLOSED_BY_USER' and fxb_trades.status!='NO_REFUND' and fxb_orders.goodid=$goodid) as temp where temp.bid=$bid")->execute()->as_array();
                   	//var_dump($soldednum);
                ?>
