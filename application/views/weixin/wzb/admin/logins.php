            ...
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                foreach ($result['logins'] as $login):
                  //按账号统计
                  $biz_name = ORM::factory('wzb_cfg')->where('bid', '=', $login->id)->where('key', '=', 'name')->find()->value;
                  $endname='';
                  $beginname='';
                  $aname=ORM::factory('wzb_admin')->where('id','=',$login->faid)->find()->name;
                  if($login->faid==$login->fadmin){
                    $endname="直销";
                  }else{
                    $beginname=ORM::factory('wzb_admin')->where('id','=',$login->fadmin)->find()->name.'所属';
                    $endname="销售";
                  }
                  $count_all = ORM::factory('wzb_qrcode')->where('bid', '=', $login->id)->count_all();
                ?>
                ...
                <tr>
                  ...
                  <td>
                  <?php
                  $sql = DB::query(Database::SELECT,"SELECT sum(data) as CT FROM wzb_lives where bid=$login->id");
