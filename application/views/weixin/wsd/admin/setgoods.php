...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="/wsda/qrcodes">分销商品管理</a></li>
  ...
  <div class="row">
    <div class="col-xs-12">
        <div class="box box-success">
         ...
         <div class="box-tools">
         <a href="/wsda/setgood1s?refresh=1" class="btn btn-success" style="position:absolute;right:270px;height:30px;line-height:18px;"><i class="fa fa-refresh"></i>刷新商品列表</a>
            ...
            <?php
            $config['kaiguan_needpay']=ORM::factory('wsd_cfg')->where('bid','=',$bid)->where('key','=','kaiguan_needpay')->find()->value;
            ?>
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                <?php foreach ($result['goods'] as $v){
                   $num_iid=$v->num_iid;
                  $soldednum=DB::query(database::SELECT,"select sum(temp.num)as tonum from (SELECT wsd_orders.* FROM `wsd_trades`,wsd_orders WHERE wsd_orders.tid=wsd_trades.tid and wsd_trades.status!='TRADE_CLOSED' and wsd_trades.status!='TRADE_CLOSED_BY_USER' and wsd_trades.status!='NO_REFUND' and wsd_orders.goodid=$num_iid) as temp where temp.bid=$bid")->execute()->as_array();
                   	//var_dump($soldednum);
                ?>
                <tr>
                  ...
                  <td style="border-top:2px solid #e5e5e5;" nowrap="">
                    <a href="#" data-toggle="modal" data-target="#actionModel" data-num_id="<?=$v->num_iid?>" data-price="<?=$v->price?>"  data-name="<?=$v->title?>" data-money="<?=$v->money?>"  data-status="<?=$v->status?>" data-type="<?=$v->type?>" data-is_profit="<?=$v->is_profit?>" >
                ...
                <?php
                  if ($v->type==1) {
                    $skus = ORM::factory('wsd_goodsku')->where('bid','=',$bid)->where('item_id','=',$v->num_iid)->find_all();
                    foreach ($skus as $sku){
                      echo '<tr>
                              <td></td>
                              <td>'.$sku->title.'</td>
...
<?php foreach ($result['goods'] as $v) {
    if($v->type==1){
      $skus2 = ORM::factory('wsd_goodsku')->where('bid','=',$bid)->where('item_id','=',$v->num_iid)->find_all();
      ...
      foreach ($skus2 as $key=>$sku2) {
        ...
        foreach ($result['suite'] as $k3=>$v3) {
          $sku = ORM::factory('wsd_goodsku')->where('id','=',$sku2->id)->find();
          $value = ORM::factory('wsd_smoney')->where('bid','=',$bid)->where('sid','=',$v3->id)->where('sku_id','=',$sku->sku_id)->find();
      ...
      echo "skus[\"".$v->num_iid."\"][0][\"suite\"]= Array();";
        foreach ($result['suite'] as $k3=>$v3) {
          $value = ORM::factory('wsd_smoney')->where('bid','=',$bid)->where('sid','=',$v3->id)->where('item_id','=',$v->num_iid)->find();
...
$('#actionModel').on('show.bs.modal', function (event) {
  ...
  var is_profit=button.data('is_profit');
  ...
  if(type == 1){
    ...
    // // console.log(skus[id][0]["title"]);
     ...
     form += '<div class="form-group"><label for="flock">商品是否仅计算利润不计入销量：</label><div class="radio"><label class="checkbox-inline"><input type="radio" name="form[is_profit]" id="status0" value="1" '+ (is_profit==1 ? ' checked' : '') +'><span class="label label-success" style="font-size:14px">是</span></label> <label class="checkbox-inline"><input type="radio" name="form[is_profit]" id="flock1" value="0"'+ (is_profit==0 ? ' checked' : '') +'><span class="label label-danger" style="font-size:14px">否</div></div>';
