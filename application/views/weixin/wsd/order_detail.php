...
<div class="tab-body list-all">
  <?php foreach ($orders as $k => $v):?>
    ...
    <div class="content-head">
      <div class="head-top">
        <div class="phonenumber"><img src="/wsd/img/lisense.png">收货人：<?=$v->receiver_name?>(手机号：<?=$v->receiver_mobile?>)</div>
    ...
    <div class="content-body">
      <?php
    $details = ORM::factory('wsd_tradedetail')->where('bid','=',$v->bid)->where('tid','=',$v->tid)->find_all();
    $count = ORM::factory('wsd_tradedetail')->where('bid','=',$v->bid)->where('tid','=',$v->tid)->count_all();
    ?>
  <?php if($count>0):?>
    <?php foreach ($details as $key => $value):?>
      <?php
      $item = ORM::factory('wsd_setgood')->where('bid','=',$v->bid)->where('num_iid','=',$value->iid)->find();
      $sku = ORM::factory('wsd_goodsku')->where('item_id','=',$value->iid)->where('sku_id','=',$value->skuid)->find();
      ?>
      <div class="goods-box">
        <div class="img-box">
          <img src="<?=$item->pic?>">
          ...
          <div class="price">
           总价：￥<?=$value->total_fee?>  数量：<?=$value->num?>    利润：￥<?=$value->money?>
          </div>
          <div class="name">
            <?=$item->title?><br>
            <?=$sku->title?><br>
  ...
  <?php endif?>
    ...
    <div class="content-lg">
      ...
      <div class="lgright">
        <a href="../trade_express/<?=$v->bid?>/<?=$v->tid?>"><span>查看物流</span></a>
      </div>
    </div>
  </div>
  <?php endforeach?>
