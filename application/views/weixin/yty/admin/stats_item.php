...
<?php
if ($status == 1) $title .= '按月统计';
if ($status == 2) $title .= '按年统计';
if ($status == 3) $title .= '按日期筛选';
?>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> 首页</a></li>
    <li><a href="/ytya/stats_item">概况</a></li>
      ...
      <div class="nav-tabs-custom">
          ...
          <form method="get" name="ordersform">
            <ul class="nav nav-tabs">
              <li id="orders<?=$status?>" class="<?=$status== 1 ? 'active' : ''?>"><a href="/ytya/stats_totle?qid=1">按月统计</a></li>
              <li id="orders<?=$status?>" class="<?=$status == 2? 'active' : ''?>"><a href="/ytya/stats_item/year?qid=2">按年统计</a></li>
              <li id="orders<?=$status?>" class="<?=$status == 3? 'active' : ''?>"><a href="/ytya/stats_item/shaixuan?qid=3">按日期筛选</a></li>
            </ul>
          </form>
              ...
              <?endif;?>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td id="" title="查看兑换明细"><?=$newadd[$i]['goodsnum']?></a></td>
                    <td><?=$newadd[$i]['stock']?></td>
                    <td><?=empty($newadd[$i]['commision'])?sprintf('%.2f',0):$newadd[$i]['commision']?></td>
                    <td><?=empty($newadd[$i]['trade_close'])?sprintf('%.2f',0):$newadd[$i]['trade_close']?></td>
                  </tr>
                  <?php
                  }?>
                  <? if($status != 3):?>
