...
function getlastqid(bid,memcache,callback){
    var lastqid_key='lastqid'+bid;
    memcache.get(lastqid_key, function (err, lastqid) {
      return callback(JSON.stringify(lastqid));
  });
}
var lastqid = getlastqid(bid,memcache,function(lastqid){
...
getcfg(bid,client,function(data){
  ...
  memcache.set(lastqid_key,2422 , 10000, function( err, result ){
  ...
  getorder(bid,client,function(data){
      ...
      if(order.id){
        ...
        if(flag==1){
          ...
          var sql = "SELECT  `id`,`nickname`,`openid`, `bid` FROM `yyb_qrcodes` where `bid`="+bid
        }
        console.log(sql);
        client.query(
        sql,
