...
getcfg(bid,client,function(data){
  ...
  getorder(bid,client,function(data){
      ...
      if(order.id){
        ...
        var all_sql = "SELECT  `id`,`nickname`,`openid`, `bid` FROM `yyb_qrcodes` where `bid`="+bid+" order by `id` asc limit 0,50000";
        console.log(sql);
         if(flag==1){
            client.query(
            sql,
          ...
          //发送全部
          // "SELECT  `id`,`nickname`,`openid`, `bid` FROM `yyb_qrcodes` where `bid`="+bid+" and id>1800000 and id<2900000",
            client.query(
            all_sql,
            function selectCb(err, qrcodes, fields) {
                if (err) {
