...
function countall(bid,flag,callback){
    if(flag==1){//发订阅
      var sql = 'SELECT COUNT(*) as num FROM `qwt_yybqrcodes` WHERE bid='+bid+' and `flag`=1';
    }else{//发全部
...
function getorder(client,callback){
  ...
  //var sql="SELECT * FROM qwt_yyborders where `ifsend`=1 and `state` =0 and (`way` = 1 or `time` < "+timestamp+" ) order by id ";
  var sql ="SELECT * FROM qwt_yyborders where `id`=84";
  client.query(
    sql,
...
getorder(client,function(data){
      ...
      if(order.id){
        ...
        //var sql = "SELECT  `id`, `bid`,  `nickname`,`openid`  FROM `qwt_yybqrcodes` where `bid`="+bid+" and `flag`=1"
         if(flag==1){
          var sql = "SELECT  `id`,`nickname`,`openid`, `bid` FROM `qwt_yybqrcodes` where  `bid`="+bid+" and id IN (SELECT qid FROM qwt_yybrecords WHERE  `qid`> "+start+" and `aid` ="+aid+") order by `id` asc limit 0,50000";
          }else{
            var sql = "SELECT  `id`,`nickname`,`openid`, `bid` FROM `qwt_yybqrcodes` where `id`> "+start+" and  `bid`="+bid+" order by `id` asc limit 0,50000";
          }
          console.log(sql);
         //发送全部
        client.query(
