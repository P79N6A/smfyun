<?php defined('SYSPATH') or die('No direct script access.');
    ...
    $this->baseurl = 'http://'. $_SERVER['HTTP_HOST'] .'/qwtrwd/';
    $this->cdnurl = 'http://cdn.jfb.smfyun.com/qwt/rwd/';
    $config = ORM::factory('qwt_rwdcfg')->getCfg($bid,1);
    $sname = ORM::factory('qwt_rwdcfg')->where('bid','=',$bid)->where('key','=','scorename')->find()->value;
    if($sname){
    ...
    $userinfo = $wx->getUserInfo($openid);
        if ($userinfo == false) {
            ...
            Kohana::$log->add("qwtrwd:$bid:WE", $wx->errCode.':'.$wx->errMsg);
        ...
        if(isset($_POST['poster'])){
            $openid = $_POST['openid'];
            Kohana::$log->add("qwtrwdopenid:", $openid);
        }
        ...
        if(isset($_POST['poster'])){
            ...
            $openid = $_POST['openid'];
            $userinfo = $wx->getUserInfo($openid);
            //Kohana::$log->add("qwtrwd:$bid:userinfo", print_r($userinfo, true));
        }
        ...
        if ($userinfo && $Event == 'CLICK' || $chunv2||$haibao==2) {
            ...
            // }
            Kohana::$log->add("qwtrwd:", '1111');
            $status = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'status')->find()->value;
            $userid = ORM::factory('qwt_rwdqrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->id;
            // if(!$userid){
            //     $msg['touser'] = $openid;
            //     $msg['msgtype'] = 'text';
            ...
            if($userid&&$status==1){//用户存在 并且地理位置打开了
                $count = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'count')->find()->value;
                $position = 0;
                $u_location = $qr_user->area;
                for ($i=1; $i <=$count ; $i++) {
                    $pro[$i] = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'pro'.$i)->find()->value;
                    $city[$i] = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'city'.$i)->find()->value;
                    $dis[$i] = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'dis'.$i)->find()->value;
         ...
         if(($position >0 && $status=='1')||$status=='0'||!$status||!$userid){
            $isvalue = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key','=','value_'.substr($EventKey,-2))->find()->value;
            ...
            //生成海报
                ...
                if(!$tid){
                    $msg['touser'] = $openid;
                    $msg['msgtype'] = 'text';
                    $msg['text']['content'] = '暂时没有有效的任务哦，请继续关注我们的任务信息，么么哒。';
                    $wx->sendCustomMessage($msg);
                    exit();
                }
                if(Model::factory('select_experience')->dopinion($bid,'rwd')){
                    ...
                    //exit;
                    // if ($txtReply2) {
                    //     Kohana::$log->add('$rwd', 'wwwww');
                    //     $textTpl = "<xml><ToUserName><![CDATA[%s]]></ToUserName><FromUserName><![CDATA[%s]]></FromUserName><CreateTime>%s</CreateTime><MsgType><![CDATA[%s]]></MsgType><Content><![CDATA[%s]]></Content><FuncFlag>0</FuncFlag></xml>";
                    //     Kohana::$log->add('$rwd001', 'aaa');
                    //     $pc = new WXBizMsgCrypt($this->token, $this->encodingAesKey, $this->appId);
                    //     $result2 = sprintf($textTpl, $fromUsername, $toUsername, $timeStamp, 'text',$txtReply2);
                    //     Kohana::$log->add('$rwd00', print_r($result2, true));
                    //     $encryptMsg = '';
                    //     $errCode = $pc->encryptMsg($result2, $timeStamp, $nonce, $encryptMsg);
                    //     if ($errCode == 0) {
                    //         if ($bid == 1)Kohana::$log->add('$rwd', print_r($encryptMsg, true));
                    //         Kohana::$log->add('$rwd11', print_r($encryptMsg, true));
                    //     } else {
                    //         Kohana::$log->add('$rwd22', print_r($errCode, true));
                    //     }
                    //     ob_flush()
                    //     echo $encryptMsg;
                    //     exit;
                    ...
                    //Kohana::$log->add('$rwd11', print_r($result, true));
                    Kohana::$log->add("msg:$bid:rwd", print_r($msg,true));
                    $wx_result = $wx->sendCustomMessage($msg);
            ...
            //积分明细
                ...
                // if (!file_exists(DOCROOT."qwt/rwd/$news_pic_file")) $news_pic_file = 'news_score.png';
            ...
            //兑换商城
                ...
                // if (!file_exists(DOCROOT."qwt/rwd/$news_pic_file")) $news_pic_file = 'news_order.jpg';
                // $news_pic = $this->cdnurl.$news_pic_file;
            ...
            //排行榜
                ...
                // if (!file_exists(DOCROOT."qwt/rwd/$news_pic_file")) $news_pic_file = 'news_top.png';
                // $news_pic = $this->cdnurl.$news_pic_file;

                // $newsReply[0]['Title'] = $msg['news']['articles'][0]['title'] = $config['score'].'排行榜';
                // $newsReply[0]['Url'] = $msg['news']['articles'][0]['url'] = $url;
                // $newsReply[0]['PicUrl'] = $msg['news']['articles'][0]['picurl'] = $news_pic;
                // Kohana::$log->add('$rwd:jfph', print_r($newsReply, true));
                ...
                //用户少的时候才回复 debug
                // if (ORM::factory('qwt_rwdqrcode')->where('bid', '=', $bid)->count_all() < 10)
                //$wx_result = $wx->sendCustomMessage($msg);
            }
            ...
            //$msg['text']['content'] = '符合要求'.$u_location.$p_location;
                ...
                $url = 'http://'. $_SERVER['HTTP_HOST'] .'/smfyun/check_location/'.$bid.'/'.$openid.'/rwd';
                $replyhref = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', 'replyhref')->find()->value;
        ...
        // //默认图文回复
        // if ($newsReply) {
        //     Kohana::$log->add('$rwd:tuwen', 'aaaa');
        //     echo $result=$wx->news($newsReply)->reply(array(), true);
        //     Kohana::$log->add('$rwd:tuwen', print_r($result, true));
        // }
