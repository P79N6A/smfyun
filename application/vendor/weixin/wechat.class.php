...
<Nonce><![CDATA[%s]]></Nonce>
  ...
  @curl_setopt($oCurl, CURLOPT_POSTFIELDS,$strPOST);
    ...
    protected function getCache($cachename){
        ...
        $mem = Cache::instance('memcache');
        if ($result = $mem->get($cachename)) return $result;
        return false;
    }
  ...
  //手动刷新access_token但是不存
 public function refresh_token($appid='',$appsecret=''){
   ...
   return $result;
	...
	public function checkAuth($appid='',$appsecret='',$token=''){
		...
		if ($result)
		{
   Kohana::$log->add("wechat_access_token:$appid",print_r($appid,true));
			$json = json_decode($result,true);
			if (!$json || isset($json['errcode'])) {
				$this->errCode = $json['errcode'];
				$this->errMsg = $json['errmsg'];
    Kohana::$log->add("wechat_access_token1:$appid",$this->errCode.$this->errMsg);
				return 'error:'.$json['errcode'].$json['errmsg'];
			}
 ...
 public function getxcx_qr($scene,$page){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array(
            'scene'=>$scene,
            'page'=>$page,
   ...
   'action_info'=>array('scene'=>$type?($type == 2?array('scene_str'=>$scene_id):array('scene_id'=>$scene_id)):array('scene_str'=>$scene_id))
	...
	public function getUserList($next_openid=''){
		if (!$this->access_token && !$this->checkAuth()) return 'appid或者appsecret不正确';
 ...
 public function getUserInfo_list($data){
  ...
  return 2;
	...
	public function sendXcxTpl($data){
		if (!$this->access_token && !$this->checkAuth()) return 2;
		$result = $this->http_post('https://api.weixin.qq.com/cgi-bin/message/wxopen/template/send?access_token='.$this->access_token,self::json_encode($data));
		if ($result)
		{
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * @return boolean|array
	 */
 public function sendTemplateMessage_xcx($data){
  ...
  return false;
	...
	public function sendTemplateMessage($data){
		...
		if (!$this->access_token && !$this->checkAuth()) return '微信参数不合法，请检查您的微信参数是否填写规范';
