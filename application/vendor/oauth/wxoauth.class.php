...
class Wxoauth
{
  ...
  public function __construct($bid,$options=array()){
      ...
      $this->access_token = $this->get_memcache('qwt.access_token'.$bid);
      ...
      if($this->access_token){
        //Kohana::$log->add($this->type.'access_token', print_r($this->access_token, true));
         ...
         Kohana::$log->add('qwtrefresh_res'.$bid, print_r($res, true));
         Kohana::$log->add('qwtrefresh'.$bid, print_r(substr($res['authorizer_refresh_token'],15), true));
         Kohana::$log->add('qwtaccesstoken2'.$bid, print_r($res['authorizer_access_token'], true));
  ...
  //设置授权信息
  public function authorizer_option_set($data){
    $result = $this->http_post("https://api.weixin.qq.com/cgi-bin/component/api_set_authorizer_option?component_access_token=".$this->component_access_token,self::json_encode($data));
    if ($result)
    {
      return $result;
      $json = json_decode($result,true);
  ...
  public function getUserInfo($openid){
    Kohana::$log->add("{$this->appid}:getUserInfo", print_r($openid, true));
    if (!$this->access_token ) return false;
    $result = $this->http_get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid);
    if ($result)
    {
      // Kohana::$log->add("$this->appod:$openid:'return'", print_r($json, true));
      $json = json_decode($result,true);
      if (isset($json['errcode'])) {
        $this->errCode = $json['errcode'];
        $this->errMsg = $json['errmsg'];
        //echo 1;
         Kohana::$log->add("{$this->appid}:{$openid}:getUserInfo_error", print_r($json, true));
      }
      return $json;
    }
    return false;
  ...
  */
  ...
  public function sendCustomMessage($data){
    Kohana::$log->add("{$this->appid}:sendCustomMessage", print_r($data, true));
    if (!$this->access_token ) {
      $err = '客服消息发送失败：access_token不存在，请重新授权';//发送失败
      $json['errmsg'] = $err;
      $json['errcode'] = '11111';
    }
    ...
    if ($result){
      ...
      if (!$json || !empty($json['errcode'])) {
        $this->errCode = $json['errcode'];
        $this->errMsg = $json['errmsg'];
        Kohana::$log->add("{$this->appid}:{$data['touser']}:sendCustomMessage_error", print_r($json, true));
        $err =  '客服消息发送失败：'.json_encode($json);//发送失败
      }else{
        return $json;//发送成功
      }
    }else{
      $err = '客服消息发送失败：通讯失败'.json_decode($result,true);//发送失败
      $json['errmsg'] = $err;
      $json['errcode'] = '11112';
    }
    if($err){
      ...
      $msg['text']['content'] = $err;
      $this->sendCustomMessage_err($msg);
      return $json;
    }
  }
  public function sendCustomMessage_err($data){
    Kohana::$log->add("{$this->appid}:sendCustomMessage_err", print_r($data, true));
    if (!$this->access_token ) return false;
    $result = $this->http_post(self::API_URL_PREFIX.self::CUSTOM_SEND_URL.'access_token='.$this->access_token,self::json_encode($data));
    if ($result){
      $json = json_decode($result,true);
      if (!$json || !empty($json['errcode'])) {
        $this->errCode = $json['errcode'];
        $this->errMsg = $json['errmsg'];
        Kohana::$log->add("{$this->appid}:{$data['touser']}:sendCustomMessage_err_error", print_r($json, true));
        return $json;
      }
      return $json;
    }
  ...
  public function getUserList($next_openid=''){
    // Kohana::$log->add("$this->appod:$next_openid", print_r($next_openid, true));
    if (!$this->access_token ) return false;
    $result = $this->http_get(self::API_URL_PREFIX.self::USER_GET_URL.'access_token='.$this->access_token.'&next_openid='.$next_openid);
    if ($result)
    {
      $json = json_decode($result,true);
      // Kohana::$log->add("$this->appod:$next_openid:'return'", print_r($json, true));
      if (isset($json['errcode'])) {
        $this->errCode = $json['errcode'];
        $this->errMsg = $json['errmsg'];
        return $json;
  ...
  public function getalltpl(){
    ...
    $result = @$this->http_post('https://api.weixin.qq.com/cgi-bin/template/get_all_private_template?access_token='.$this->access_token);
    if($result){
      $json = json_decode($result,true);
      return $json;
    }
  ...
  public function sendTemplateMessage($data){
    Kohana::$log->add("{$this->appid}:sendTemplateMessage", print_r($data, true));
    if (!$this->access_token) return 2;
    ...
    if($result){
      ...
      if (!$json || !empty($json['errcode'])) {
        $this->errCode = $json['errcode'];
        $this->errMsg = $json['errmsg'];
        Kohana::$log->add("{$this->appid}:{$data['touser']}:sendTemplateMessage_error", print_r($json, true));
        $err =  '模板消息发送失败：'.json_encode($json);//发送失败
      }else{
        return $json;
      }
    }else{
  ...
  public function sendTemplateMessage1($data){
        // Kohana::$log->add("$this->appod", print_r($data, true));
       Kohana::$log->add("{$this->appid}:sendTemplateMessage1", print_r($data, true));
        if (!$this->access_token) return false;
        ...
        if($result){
            $json = json_decode($result,true);
            // Kohana::$log->add("$this->appod:'return'", print_r($json, true));
            //Kohana::$log->add("json",print_r($json,true));
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                 Kohana::$log->add("{$this->appid}:{$data['touser']}:sendTemplateMessage1_error", print_r($json, true));
                $err =  '模板消息发送失败：'.json_encode($json);//发送失败
            }else{
              return $json;
            }
  ...
  public function getallforever($data){
    if (!$this->access_token ) return false;
    $result = $this->request_post('https://api.weixin.qq.com/cgi-bin/material/batchget_material?access_token='.$this->access_token,json_encode($data));
    if ($result)
    {
      $json = json_decode($result,true);
      if (!$json || !empty($json['errcode'])) {
  ...
  public function uploadForever($data, $type){
    if (!$this->access_token ) return false;
    //原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
  ...
  public function downjpg($media_id){
    if (!$this->access_token ) return false;
    //原先的上传多媒体文件接口使用 self::UPLOAD_MEDIA_URL 前缀
    $result = @$this->http_post('https://api.weixin.qq.com/cgi-bin/media/get?access_token='.$this->access_token.'&media_id='.$media_id);
    return $result;
    if ($result)
    {
      $json = json_decode($result,true);
...
<Nonce><![CDATA[%s]]></Nonce>
  ...
  private function http_post($url,$param,$post_file=false){
    ...
    // if($post_file){
    //   Kohana::$log->add('qwt_file_post', print_r($post_file, true));
    //   curl_setopt(ch, CURLOPT_POSTFIELDS, [
    //       'file' => new CURLFile(realpath($post_file)),
    //   ]);
    // }else{
