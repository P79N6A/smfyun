...
class Helper{
 ...
 public static function GenerateCode($time,$bid,$count=5000,$iid){
  ...
  // $times = ceil($count/100000);
    ...
    while ($d_value>0) {
      ...
      while ($i<$count) {
        ...
        // $tmp[$str] = 1;
        $i++;
        $values[] = "($bid, $str, $iid,$time)";
      }
      //统一插入
      $SQL = 'INSERT IGNORE INTO wxp_kls (`bid`,`code`,`iid`,`lastupdate`) VALUES '. join(',', $values);
      $now_count = $colum = DB::query(NULL,$SQL)->execute();
      //当前行数
      $SQL2 = "SELECT * from wxp_kls where `bid`= $bid and `lastupdate`= $time";
