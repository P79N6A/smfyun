...
class Wxoauth
{
  ...
  public function __construct($bid,$type,$appId,$options=array()){
      //必要参数 传入
      $this->bid = $bid;
      $this->type = $type;
      ...
      $this->appId = $appId;//三方 appId
      //$this->appsecret = isset($options['appsecret'])?$options['appsecret']:'';
      $this->debug = isset($options['debug'])?$options['debug']:false;
      $this->logcallback = isset($options['logcallback'])?$options['logcallback']:false;
      $this->access_token = $this->get_memcache($type.'.access_token'.$bid);

      $this->component_access_token = $component_access_token = $this->get_memcache('component_access_token'.$appId);//三方appid
      if($this->access_token){
        //Kohana::$log->add($this->type.'access_token', print_r($this->access_token, true));
         ...
         $user = ORM::factory($type.'_login')->where('id','=',$bid)->find();
         $post_data = array(
         ...
         Kohana::$log->add($this->type.'refresh'.$bid, print_r(substr($res['authorizer_refresh_token'],15), true));
         Kohana::$log->add($this->type.'accesstoken2'.$bid, print_r($res['authorizer_access_token'], true));
         Kohana::$log->add($this->type.'accesstoken2_end'.$bid, print_r($res, true));
         $user->expires_in = time()+7200;
         if($bid){
            $user->save();
         }
         $this->set_memcache($res['authorizer_access_token'],$type.'.access_token'.$bid,5400);
  ...
  */
  ...
  public function sendCustomMessage($data){
    ...
    /**
   * oauth 授权跳转接口
   * @param string $callback 回调URI snsapi_base
   * @return string
   */
  public function getOauthRedirect($appid,$callback,$state='',$scope='snsapi_userinfo'){
    return self::OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL.'appid='.$appid.'&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state;
  }
  ...
  public function sendTemplateMessage($data){
    ...
    if (!$this->access_token && !$this->checkAuth()) return 2;
