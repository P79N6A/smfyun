...
class Controller_yty extends Controller_Base {
    ...
    public function action_form($openid) {
        ...
        $bid=$this->bid;
        $tpl = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'tpl')->find()->id;
    ...
    public function action_home() {
        ...
        //代理审核
        $result['num2']=ORM::factory('yty_qrcode')->where('bid', '=', $this->bid)->where('lv','=',2)->where('fopenid','=',$userobj->openid)->count_all();
        //总收益
        ...
        //计算排名
         if($_POST['data']){
            ...
            if($stock->money*$fuser->agent->skus->scale/100<=$stock->fqrcode->agent->stock){
                ...
                $sqrcodes=ORM::factory('yty_sqrcode')->where('bid','=',$bid)->where('qid','=',$qrcode->id)->find_all();
    ...
    public function action_top() {
        ...
        $result['rank'] = ORM::factory('yty_qrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('paid', '>', $user->paid)->count_all()+1;
    ...
    private function tradeImport($trade, $bid, $client, $we, $config) {
        ...
        if ($yty_trade->id) {
            ...
            Kohana::$log->add('ytytrade', print_r($yty_trade->id, true));
            //退款订单删返利
            if ($trade['status'] == 'TRADE_CLOSED'||$trade['status'] == 'TRADE_CLOSED_BY_USER'||$trade['refund_state'] != 'NO_REFUND'){
                Kohana::$log->add('ytytrade1', print_r($yty_trade->id, true));
                if($yty_trade->flag==1){
                    ORM::factory('yty_score')->where('tid', '=', $yty_trade->id)->delete_all();
                    $fopenid=$yty_trade->fopenid;
                    $fuser=ORM::factory('yty_qrcode')->where('bid','=',$bid)->where('openid','=',$fopenid)->find();
                    $fuser->stocks->stockIn($fuser, $type=6,$yty_trade->money1,$yty_trade->qrcode->id,1);
                    if($config['money_arrived_tpl']){
                        $money1= $fuser->agent->stock-$yty_trade->money1;
                        $money2= $fuser->agent->stock;
                        $remark='您的客户'.$yty_trade->qrcode->nickname.'购买的'.$yty_trade->title.'已退货。退回给您'.$yty_trade->money1.'元进货额';
                       $this->stocksuccess($fuser->openid,$money1,$money2,$remark);
                    }else{
                        $msg['touser'] = $fuser->openid;
                        $msg['msgtype'] = 'text';
                        $msg['text']['content'] = '您的客户'.$yty_trade->qrcode->nickname.'购买的'.$yty_trade->title.'已退货。退回给您'.$yty_trade->money1.'元进货额';
                        $this->we->sendCustomMessage($msg);
                    }
                }
            }
        ...
        $yty_trade1 = ORM::factory('yty_trade')->where('tid', '=', $tid)->find();
        if($yty_trade1->id) return;
        $yty_trade->save();
