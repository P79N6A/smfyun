...
class Controller_User_Config_Dka extends Controller_User_Home {
    ...
    public function action_index(){
        $url="http://dkb.smfyun.com/dkaa/login";//登陆域名
        $intro="http://wap.koudaitong.com/v2/feature/nbo89d92";
        $view=View::factory("user/config/dka");
        ...
        $dka=ORM::factory("dka");
        $result=$dka->where("user",'=',$name)->find();
        ...
        {
            $view->set("user",$dka->user)->set("pass",$dka->pass);
        }
        else
        {
            $dka->user=$name;
            $dka->pass=$pass;
            $dka->name=$from;
            $dka->lastupdate=$update;
            $dka->expiretime=$exprie;
            $dka->admin=$admin;
            $dka->logins=$logins;
            $dka->lastlogin=$login;
            $dka->save();
