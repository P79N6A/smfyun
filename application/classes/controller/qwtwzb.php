...
use OSS\OssClient;
class Controller_qwtwzb extends Controller_Base {
    ...
    public function before() {
        ...
        // if (Request::instance()->action == 'sub') return;
        ...
        if (!$_GET['openid']) {
            if (!$_SESSION['qwtwzb']['bid']) die('页面已过期。请重新点击相应菜单');
            if (!$_SESSION['qwtwzb']['openid']) die('Access Deined..请重新点击相应菜单');
        }

        $biz = ORM::factory('qwt_login')->where('id','=',$_SESSION['qwtwzb']['bid'])->find();
        // if ($biz->expiretime && strtotime($biz->expiretime) < time()) die('您的账号已过期');
        $this->config = $_SESSION['qwtwzb']['config'];
        $this->openid = $_SESSION['qwtwzb']['openid'];
        $this->bid = $_SESSION['qwtwzb']['bid'];
        $this->uid = $_SESSION['qwtwzb']['uid'];
        $this->sid = $_SESSION['qwtwzb']['sid'];
        $this->access_token = $_SESSION['qwtwzb']['access_token'];


        if ($_GET['debug']) print_r($_SESSION['qwtwzb']);
    ...
    public function action_stream_data($bid){
        ...
        $live = ORM::factory('qwt_wzblive')->where('bid','=',$bid)->where('end_time','=',0)->find();//找出未开播的
    ...
    public function action_isplay($bid){
        ...
        // echo $result['sid'];
        $url = 'http://jfb.dev.smfyun.com/qwtwzb/aliyun?online=1';
    ...
    public function action_send_live_comment(){
        ...
        if($_POST['login']){//comment
            ...
            $user = ORM::factory('qwt_wzbqrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            $mem = Cache::instance('memcache');
            // echo $openid;
            ...
            if($user->id){
                ...
                for ($i=0; $imgs[$i] ; $i++) {
                    $img[$i] = ORM::factory('qwt_wzbqrcode')->where('id','=',$imgs[$i])->find()->headimgurl;
                }
        ...
        if($_POST['scid']){//comment
            ...
            if($_POST['scid']=='admin'){
                ...
                $user->headimgurl = $shop->shoplogo;
                $user->nickname = $shop->shopname;
                $user->sex = 1;
            }else{
                $user = ORM::factory('qwt_wzbqrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            }
        ...
        if($_POST['buy']==1){
            ...
            // $client = stream_socket_client('tcp://127.0.0.1:5678', $errno, $errmsg, 1);
            // 推送的数据，包含uid字段，表示是给这个uid推送
            $cfg = ORM::factory('qwt_wzbcfg')->where('bid','=',$bid)->where('key','=','buynum')->find();
            ...
            $user = ORM::factory('qwt_wzbqrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            if($user->nickname){
                $cfg2 = ORM::factory('qwt_wzbcfg')->where('bid','=',$bid)->where('key','=','buyname')->find();
                $cfg2->bid = $bid;
                $cfg2->key = 'buyname';
    ...
    public function action_index_oauth($bid, $url='live') {
       $_SESSION =& Session::instance()->as_array();
       $bid = ORM::factory('qwt_login')->where('id','=',$bid)->find()->id;
       if(!$bid){
        die('不合法');
       }
       $config = ORM::factory('qwt_wzbcfg')->getCfg($bid,1);
        if ($config) {
            ...
            $userobj = ORM::factory('qwt_wzbqrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            ...
            $shop = ORM::factory('qwt_login')->where('id','=',$bid)->find();
            $_SESSION['qwtwzb']['sid'] = $shop->shopid;
            $_SESSION['qwtwzb']['config'] = $config;
            $_SESSION['qwtwzb']['openid'] = $openid;
            $_SESSION['qwtwzb']['bid'] = $bid;
            $_SESSION['qwtwzb']['uid'] = $userobj->id;
            // if (!$_GET['callback']) {
            ...
            //     $userinfo['lv'] = 0;
            // }
            // if (!$openid) $_SESSION['qwtwzb'] = NULL;
    ...
    public function action_sub(){
        ...
        // echo $this->openid;
        // exit;
        $user = ORM::factory('qwt_wzbqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find();
        $user->sub = 1;
        if($_POST['cancel']==1){
            $user->sub = 0;
        }
    ...
    public function action_SnapshotInfo(){
        ...
        if($_GET['bid']) {
            // $sid = ORM::factory('qwt_login')->where('id','=',$_GET['bid'])->find()->shopid;
    ...
    public function action_jianhuang($bid){
        ...
        // echo $result['sid'];
        $url = 'http://jfb.dev.smfyun.com/qwtwzb/SnapshotInfo?bid='.$bid;
        // var_dump($this->request_post($url, $post_data));
        $res = $this->request_post($url, $post_data);
        ...
        if($oss_shot['LiveStreamSnapshotInfoList']['LiveStreamSnapshotInfo'][0]['OssObject']){
            ...
            // echo $result['sid'];
            $url = 'http://jfb.dev.smfyun.com/qwtwzb/oss';
            // var_dump($this->request_post($url, $post_data));
    ...
    public function action_download_view(){
        ...
        self::before();
        $view = "weixin/smfyun/wzb/download_view";
        $this->template->content = View::factory($view);
    }

    public function action_download(){
        $file = DOCROOT.'qwt/wzb/apk/qwtsmfyun.apk';
    ...
    public function action_live_status($bid){
        ...
        if($_GET['action']=='start'){
            ...
            $post_data = array();
            // echo $result['sid'];
            $url = 'http://jfb.dev.smfyun.com/qwtwzb/isplay/'.$bid;
            // var_dump($this->request_post($url, $post_data));
            $res = $this->request_post($url, $post_data);
            if($res=='false'){//没有直播
                if(ORM::factory('qwt_wzblive')->where('bid','=',$bid)->where('end_time','=',0)->count_all()>=1){
                    // echo 'hasstart';
                    $live = ORM::factory('qwt_wzblive')->where('bid','=',$bid)->where('end_time','=',0)->find();
                    $live->end_time = time();
                    $live->save();
                    //终止旧的 新建新的
                    $new_live = ORM::factory('qwt_wzblive');
                    $new_live->bid = $bid;
        ...
        if($_GET['action']=='stop'&&$_GET['lid']){
            $live = ORM::factory('qwt_wzblive')->where('id','=',$_GET['lid'])->find();
            $live->end_time = time();
            $live->save();
            exit;
        }
    ...
    public function action_aliyun($bid=1){
        kohana::$log->add('qwtwzbdebug:aliyun1',time());
        ...
        ob_flush();
        echo json_encode($response);
        kohana::$log->add('qwtwzbdebug:aliyun2',time());
        exit;
        // echo $response;
    }
    ...
    public function action_zan(){
        $live = ORM::factory('qwt_wzblive')->where('bid','=',$_POST['bid'])->where('end_time','=',0)->find();
        if($live->id){
    ...
    public function action_live(){
        $_SESSION =& Session::instance()->as_array();
        if($_GET['debug']==1){
            $_SESSION['qwtwzb']['openid'] = 'oDt2QjtTeio8l0dBl28SQGhcHSH4';
            $_SESSION['qwtwzb']['bid'] = 6;
            $_SESSION['qwtwzb']['uid'] = 1008;
            $_SESSION['qwtwzb']['sid'] = 936565;
        }
        if(!$_SESSION['qwtwzb']['bid']){
            // Request::instance()->redirect('/wzb/index_oauth/'.$_GET['bid'].'/live');
            $url = 'http://'.$_SERVER['HTTP_HOST'].'/smfyun/user_snsapi_userinfo/'.$_GET['bid'].'/wzb/live';
            header("Location: $url");
            exit;
        }
        $biz = ORM::factory('qwt_login')->where('id','=',$_SESSION['qwtwzb']['bid'])->find();
        ...
        $view = "weixin/smfyun/wzb/live";
        $this->template = 'tpl/blank';
        require_once Kohana::find_file("vendor/youzan","YZTokenClient");
        self::before();

        $result['tpl'] = ORM::factory('qwt_wzbcfg')->where('bid', '=', $this->bid)->where('key', '=', 'wzbtpl')->find()->id;
        $result['tplhead'] = ORM::factory('qwt_wzbcfg')->where('bid', '=', $this->bid)->where('key', '=', 'wzbtplhead')->find()->id;
        $result['logo'] = $result['tplhead']?"/qwtwzb/images/cfg/".$result['tplhead'].".v".time().".jpg":$biz->shoplogo;
        $result['poster'] = $result['tpl']?"/qwtwzb/images/cfg/".$result['tpl'].".v".time().".jpg":$biz->shoplogo;
        $result['name'] = $this->config['name']?$this->config['name']:$biz->shopname;
        $result['openid'] = $this->openid;
        $user = ORM::factory('qwt_wzbqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find();
        $result['sid'] = $this->sid;
        $result['bid'] = $this->bid;
        $goods = ORM::factory('qwt_wzbsetgood')->where('bid','=',$this->bid)->where('status','=',1)->order_by('priority', 'DESC')->find_all();
        $post_data = array(
          'bid' =>$result['bid']
        );
        // echo $result['sid'];
        $url = 'http://jfb.dev.smfyun.com/qwtwzb/aliyun?num=1';
        // var_dump($this->request_post($url, $post_data));
        $res = $this->request_post($url, $post_data);
        ...
        $cfg = ORM::factory('qwt_wzbcfg')->where('bid','=',$this->bid)->where('key','=','buynum')->find();
        $cfg2 = ORM::factory('qwt_wzbcfg')->where('bid','=',$this->bid)->where('key','=','buyname')->find();
        ...
        $wsimg = ORM::factory('qwt_wzbcfg')->where('bid', '=', $this->bid)->where('key', '=', 'wzbtplshare')->find()->id;
        $result['wsimg'] = $wsimg?"http://".$_SERVER['HTTP_HOST']."/qwtwzb/images/cfg/".$wsimg.".v".time().".jpg":$shop->shoplogo;
        if($_POST['subaction']){
        ...
        $live = ORM::factory('qwt_wzblive')->where('bid','=',$this->bid)->where('end_time','=',0)->find();
        if($live->id){
            $live->pv = $live->pv+1;
        ...
        $result['goods_count'] = ORM::factory('qwt_wzbsetgood')->where('bid','=',$this->bid)->where('status','=',1)->order_by('priority', 'DESC')->count_all();
        $result['buynum'] = $cfg->value;
        $result['buyname'] = $cfg2->value;
        //转盘
        $access_token = ORM::factory('qwt_login')->where('id', '=', $this->bid)->find()->yzaccess_token;
        if($access_token){
            require_once Kohana::find_file("vendor/youzan","YZTokenClient");
        ...
        $data1 = ORM::factory('qwt_wzblottery')->where('bid','=',$this->bid)->where('item','=',1)->find();
        if ($data1->type == 1) {
            $content['type1'] = $data1->other.'积分';
        }elseif ($data1->type == 2) {
            if(is_array($coupons)&&$coupons['response']['coupons']){
            ...
            if(is_array($gifts)&&$gifts['response']['presents']){
        ...
        $data2 = ORM::factory('qwt_wzblottery')->where('bid','=',$this->bid)->where('item','=',2)->find();
        if ($data2->type == 1) {
        ...
        $data3 = ORM::factory('qwt_wzblottery')->where('bid','=',$this->bid)->where('item','=',3)->find();
        if ($data3->type == 1) {
        ...
        $data4 = ORM::factory('qwt_wzblottery')->where('bid','=',$this->bid)->where('item','=',4)->find();
        if ($data4->type == 1) {
        ...
        $lswitch = ORM::factory('qwt_wzbcfg')->where('bid','=',$this->bid)->where('key','=','switch')->find()->value;
        // echo $_SESSION['qwtwzb']['sid'];
        // exit;
        $this->template->content = View::factory($view)
                        ->bind('result', $result)
                        ->bind('goods', $goods)
    ...
    public function action_sendsub(){
        if($_GET['bid']){
            ...
            $shop = ORM::factory('qwt_login')->where('id','=',$bid)->find();
            $users = ORM::factory('qwt_wzbqrcode')->where('bid','=',$bid)->where('sub','=',1)->find_all();
            // $name = $this->config['name']?$this->config['name']:$shop->shopname;
            // $url = 'http://'.$_SERVER['HTTP_HOST'].'/wzb/index_oauth/'.$bid;
            $url = 'http://'.$_SERVER['HTTP_HOST'].'/smfyun/user_snsapi_base/'.$bid.'/wzb/live';
            if($this->config['starttpl']){
                foreach ($users as $k => $v) {
                    // echo json_encode($this->sendstarttpl($v->openid,$url));
                    $result = $this->sendstarttpl($v->openid,$url);
                    if($result['errcode']>0){
                        Kohana::$log->add('qwt_wzb_sub_tpl_fail_tplid_'.$bid, $this->config['starttpl']);
                        Kohana::$log->add('qwt_wzb_sub_tpl_fail_openid_'.$bid, $v->openid);
                        Kohana::$log->add('qwt_wzb_sub_tpl_fail_err_'.$bid, print_r($result,true));
                    }
                }
            }else{
                die('模板消息未设置！');
    ...
    private function sendstarttpl($openid, $url) {
        Kohana::$log->add('qwt_wzb_sub_tpl_fail_tplid2_'.$bid, $this->config['starttpl']);
    ...
    public function action_push_order(){
        ...
        if($type=='TRADE'){
            ...
            $this->bid=$bid;
            $this->config = $config = ORM::factory('qwt_wzbcfg')->getCfg($bid);
    ...
    private function tradeImport($trade, $bid, $client, $config) {
        ...
        $wzb_trade = ORM::factory('qwt_wzbtrade')->where('tid', '=', $tid)->find();
        ...
        $wzb_trade1 = ORM::factory('qwt_wzbtrade')->where('tid', '=', $tid)->find();
        if($wzb_trade1->id) return;
        $wzb_trade->save();
        $group_id = ORM::factory('qwt_login')->where('id','=',$bid)->find()->shopid;
        // $client = stream_socket_client('tcp://127.0.0.1:5678', $errno, $errmsg, 1);
        // 推送的数据，包含uid字段，表示是给这个uid推送
    ...
    public function action_sweepstakes(){//大转盘
        ...
        if($_POST['sweepstakes']==1){
            ...
            $live = ORM::factory('qwt_wzblive')->where('bid','=',$bid)->where('start_time','>',0)->order_by('id','desc')->find();
            $config = ORM::factory('qwt_wzbcfg')->getCfg($bid,1);
            $start = $live->start_time;
            $end = strtotime(date('Y-m-d',strtotime('+1 day')));
            $_has = ORM::factory('qwt_wzbsweepstake')->where('bid','=',$bid)->where('qid','=',$user->id)->where('lastupdate','<',$end)->where('lastupdate','>',$start)->count_all();
            if($config['switch']==1){
                // echo 1;
                if($_has>$config['times']){
                    ...
                    if($rand<=$config['probability']){
                        ...
                        $prize1 = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',1)->find();
                        $prize2 = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',2)->find();
                        $prize3 = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',3)->find();
                        $prize4 = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',4)->find();
                        if($prize1->num==0&&$prize2->num==0&&$prize3->num==0&&$prize4->num==0){
                            ...
                            $rand_prize = rand(1,4);
                            // $rand_prize =3;
                            $prize = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',$rand_prize)->find();
                            while ($prize->num==0) {
                                $rand_prize = rand(1,4);
                                $prize = ORM::factory('qwt_wzblottery')->where('bid','=',$bid)->where('item','=',$rand_prize)->find();
                            }
                            if($prize->type==3){//中了红包
                                ...
                                if(!isset($user_id)){
                                    ...
                                    if($result['state'] == 1){
                                        $wzb_sweepstake = ORM::factory('qwt_wzbsweepstake');
    ...
    public function addpoint($bid,$user_id,$points){
        require_once Kohana::find_file("vendor/youzan","YZTokenClient");
        $this->access_token = ORM::factory('qwt_login')->where('id','=',$bid)->find()->yzaccess_token;
        $client = new YZTokenClient($this->access_token);
    ...
    private function hongbao($config, $openid, $money, $bid){
        $this->bid = $bid;
        if (!$wx) {
            ...
            $options['encodingaeskey'] = $this->encodingAesKey;
            $options['appid'] = ORM::factory('qwt_login')->where('id','=',$this->bid)->find()->appid;
            if(!$this->bid) Kohana::$log->add('qwtwzbbid:', 'hongbao');//写入日志，可以删除
            $wx = new Wxoauth($this->bid,$options);
        }
        ...
        if ($bid === $this->debugbid) Kohana::$log->add('qwt:weixin:hongbaopost', print_r($data, true));//写入日志，可以删除
    ...
    private function curl_post_ssl($url, $vars, $second=30, $aHeader=array(), $bid) {
        $ch = curl_init();
        ...
        Kohana::$log->add('qwt_file_cert_wzb:',print_r($cert_file, true));
