...
//分销宝后台
class Controller_ytya extends Controller_Base {
    ...
    public function before() {
        ...
        $this->access_token=ORM::factory('yty_login')->where('id', '=', $this->bid)->find()->access_token;
        //未登录
    ...
    public function after() {
        if ($this->bid) {
            $todo['users'] = ORM::factory('yty_qrcode')->where('bid', '=', $this->bid)->count_all();
            $todo['tickets'] = ORM::factory('yty_qrcode')->where('bid', '=', $this->bid)->where('ticket', '<>', '')->count_all();

            //$todo['items'] = ORM::factory('yty_order')->where('bid', '=', $this->bid)->where('status', '=', 0)->count_all();
    ...
    public function action_callback(){
        ...
        if(isset($result->access_token))
        {
            ...
            $sid = $value['id'];
            $name = $value['name'];
            $usershop = ORM::factory('yty_login')->where('id','=',$this->bid)->find();
    ...
    public function action_home() {
        ...
        if ($_POST['cfg']) {
            ...
            if (file_exists($cert_file)) $cfg->setCfg($bid, 'yty_file_cert', '', file_get_contents($cert_file));
            if (file_exists($key_file)) $cfg->setCfg($bid, 'yty_file_key', '', file_get_contents($key_file));
        ...
        if ($_POST['text']) {
            ...
            if (!$result['err3']) {
                ...
                $tplhead = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'tplhead')->find();
                if ($tplhead) {
                    $tplhead->lastupdate = time();
                    $tplhead->save();
                }

                $tpl = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'tpl')->find();
                if ($tpl) {
                    $tpl->lastupdate = time();
                    $tpl->save();
                }
        ...
        $result['tpl'] = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'tpl')->find()->id;
        $result['tplhead'] = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'tplhead')->find()->id;
    ...
    public function action_skus($action='', $id=0) {
        ...
        $bid = $this->bid;
        $config = ORM::factory('yty_cfg')->getCfg($bid);

        $result['skus'] = ORM::factory('yty_sku')->where('bid', '=', $bid)->order_by('id', 'DESC')->find_all();
    ...
    public function action_skus_add() {
        ...
        if ($_POST['data']) {
            ...
            $sku = ORM::factory('yty_sku');
            $sku->values($_POST['data']);
            $num=ORM::factory('yty_sku')->where('bid','=',$bid)->count_all();
            $sku->order=$num+1;
            $sku->bid = $bid;
    ...
    public function action_qrcode_p(){
        $bid=$this->bid;
        $config=ORM::factory('yty_cfg')->getCfg($bid,1);
        $qrcodes=ORM::factory('yty_qrcode')->where('bid','=',$bid)->where('lv','!=',1);
        $qrcodes->reset(FALSE);
    ...
    public function action_qrcodes_m($action='', $id=0) {
        $bid = $this->bid;
        $this->config = ORM::factory('yty_cfg')->getCfg($bid,1);
        $config=$this->config;
        ...
        if ($_POST['form']['id']) {
            $id = $_POST['form']['id'];
            $qrcode_edit = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('id', '=', $id)->find();
            if ($qrcode_edit->id) {
                if (isset($_POST['form']['lv'])) {
                    $qrcode_edit->lv = (int)$_POST['form']['lv'];
                    $agent=ORM::factory('yty_agent')->where('bid','=',$bid)->where('openid','=',$qrcode_edit->openid)->find();
                ...
                if ($_POST['form']['status']){
                    ...
                    if($status==1){
                        $fuser=ORM::factory('yty_qrcode')->where('bid','=',$bid)->where('openid','=',$qrcode_edit->fopenid)->find();
                        if($fuser->s==1){
                            $sku=ORM::factory('yty_sku')->where('bid','=',$bid)->where('order','=',1)->find();
                            $qrcode_edit->s=1;
                            $agent->sid=$sku->id;
                        ...
                        $sqrcode=ORM::factory('yty_sqrcode');
                        $sqrcode->bid=$bid;
        ...
        $qrcode = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where_open()->where('lv','=',1)->or_where('lv','=',3)->where_close();
        ...
        if ($_GET['fopenid']) {//下线
            ...
            $result['fuser'] = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('openid', '=', $result['fopenid'])->find();
    ...
    public function action_qrcodes($action='', $id=0) {
            ...
            if ($qrcode_edit->id) {
                if (isset($_POST['form'])) {
                    ...
                    if((int)$_POST['form']['lv']==1){
                        ...
                        $front_user = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('id', '<', $qrcode_edit->id)->order_by('id','desc')->find();
        ...
        $qrcode = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('lv','=',2);
        $qrcode = $qrcode->reset(FALSE);
    ...
    public function action_stock(){
        ...
        $stocks=ORM::factory('yty_stock')->where('bid','=',$bid)->where('flag','=',0)->where('type','=',1);
        $stocks->reset(false);
    ...
    public function action_stock_history(){
        $bid=$this->bid;
        $config = ORM::factory('yty_cfg')->getCfg($bid,1);
        $stocks=ORM::factory('yty_stock')->where('bid','=',$bid)->where('flag','=',1)->where_open()->where('type','=',1)->or_where('type','=',0)->where_close();
        $stocks->reset(false);
        if($_GET['id']){
    ...
    public function action_logins_add() {
        ...
        if ($_POST['data']) {
            $login = ORM::factory('yty_login');
            $login->values($_POST['data']);
            if (!$_POST['data']['name'] || !$_POST['data']['user']) $result['error'] = '请填写完整后再提交';
            if (ORM::factory('yty_login')->where('user', '=', $_POST['data']['user'])->count_all() > 0) $result['error'] = '该登录名已经存在';
    ...
    public function action_logins_edit($id) {
        ...
        $login = ORM::factory('yty_login', $id);
        ...
        if ($_POST['data']) {
            $login->values($_POST['data']);
            if (!$_POST['data']['name'] || !$_POST['data']['user']) $result['error'] = '请填写完整后再提交';
            if (ORM::factory('yty_login')->where('user', '=', $_POST['data']['user'])->where('id', '<>', $id)->count_all() > 0)
    ...
    public function action_login() {
        ...
        if ($_POST['username'] && $_POST['password']) {
            $biz = ORM::factory('yty_login')->where('user', '=', $_POST['username'])->where('pass', '=', $_POST['password'])->find();
            ...
            if ($biz->id) {
                ...
                if ($biz->expiretime && strtotime($biz->expiretime) < time()) {
                    ...
                    $_SESSION['ytya']['config'] = ORM::factory('yty_cfg')->getCfg($biz->id);
    ...
    public function action_stats_totle($action=''){
        ...
        if($_GET['qid']==3||$action=='shaixuan'){
            ...
            if($_GET['data']['begin']!=null&&$_GET['data']['over']!=null)//搜索
            {
               ...
               else
               {
                ...
                //新增用户
                $fans=DB::query(Database::SELECT,"SELECT count(openid) as fansnum from yty_qrcodes where bid=$this->bid and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' and lv !=1  ")->execute()->as_array();
                $newadd[0]['fansnum']=$fans[0]['fansnum'];
                //新增经销商
                $user=DB::query(Database::SELECT,"SELECT count(openid) as usernum from yty_qrcodes where bid=$this->bid and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' and lv =1 ")->execute()->as_array();
                $newadd[0]['user']=$user[0]['usernum'];
                //进货额
                $stock=DB::query(Database::SELECT,"SELECT SUM(money) as stock from yty_stocks where bid=$this->bid and FROM_UNIXTIME(`lastupdate`, '$daytype')>='$begin' and FROM_UNIXTIME(`lastupdate`, '$daytype')<='$over' and fqid is null ")->execute()->as_array();
                $newadd[0]['stock']=$stock[0]['stock'];
                //有赞订单数，有赞订单数、有赞商品交易数量、有赞成交金额
                $tradesdata=DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from yty_trades where bid=$this->bid and left(pay_time,$length) >='$begin' and left(pay_time,$length) <='$over'")->execute()->as_array();
                $newadd[0]['tradesnum']=$tradesdata[0]['tradesnum'];
                $newadd[0]['goodsnum']=$tradesdata[0]['goodnum'];
                $newadd[0]['payment']=$tradesdata[0]['payment'];
                //所有佣金 已结算的佣金、待结算的佣金
                $commision=DB::query(Database::SELECT,"SELECT SUM(score) AS paymoney from yty_scores where bid=$this->bid and FROM_UNIXTIME(`lastupdate`, '$daytype')>='$begin' and FROM_UNIXTIME('lastupdate','$daytype')<='$over' and type =4")->execute()->as_array();
        ...
        else
        {
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_scores` where bid=$this->bid and type =4 UNION SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_stocks` where bid=$this->bid and fqid is NUll UNION SELECT FROM_UNIXTIME(`jointime`, '$daytype')as time FROM `yty_qrcodes` where bid=$this->bid UNION select left(pay_time,$length) from yty_trades where bid=$this->bid ORDER BY `time` DESC ")->execute()->as_array();
            //$pagesize=2;
            $num=count($days);
            $page = max($_GET['page'], 1);
            $offset = ($this->pagesize * ($page - 1));
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_scores` where bid=$this->bid and type =4 UNION SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_stocks` where bid=$this->bid and fqid is NUll UNION SELECT FROM_UNIXTIME(`jointime`, '$daytype')as time FROM `yty_qrcodes` where bid=$this->bid UNION select left(pay_time,$length) from yty_trades where bid=$this->bid ORDER BY `time` DESC limit $this->pagesize offset $offset")->execute()->as_array();
            $newadd=array();
            for($i=0;$days[$i];$i++)
            {
                $time=$days[$i]['time'];
                $newadd[$i]['time']=$time;
                //新增用户
                $fans=DB::query(Database::SELECT,"SELECT count(openid) as fansnum from yty_qrcodes where bid=$this->bid and FROM_UNIXTIME(`jointime`, '$daytype')='$time' and lv != 1")->execute()->as_array();
                $newadd[$i]['fansnum']=$fans[0]['fansnum'];
                //新增经销商
                $user=DB::query(Database::SELECT,"SELECT count(openid) as usernum from yty_qrcodes where bid=$this->bid and FROM_UNIXTIME(`jointime`, '$daytype')='$time' and lv = 1")->execute()->as_array();
                $newadd[$i]['user']=$user[0]['usernum'];
                //进货额
                $stock=DB::query(Database::SELECT,"SELECT SUM(money) as stock from yty_stocks where bid=$this->bid and  FROM_UNIXTIME(`lastupdate`, '$daytype') ='$time' and fqid is NULL ")->execute()->as_array();
                $newadd[$i]['stock']=$stock[0]['stock'];
                //有赞订单数，有赞订单数、有赞商品交易数量、有赞成交金额
                $tradesdata=DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from yty_trades where bid=$this->bid and left(pay_time,$length) LIKE '$time'")->execute()->as_array();
                $newadd[$i]['tradesnum']=$tradesdata[0]['tradesnum'];
                $newadd[$i]['goodsnum']=$tradesdata[0]['goodnum'];
                $newadd[$i]['payment']=$tradesdata[0]['payment'];
                //所有佣金 已结算的佣金、待结算的佣金
                $commision=DB::query(Database::SELECT,"SELECT SUM(score) AS paymoney from yty_scores where bid=$this->bid and FROM_UNIXTIME(`lastupdate`, '$daytype')='$time' and type =4")->execute()->as_array();
               // var_dump($commision);
                $newadd[$i]['commision']=$commision[0]['paymoney'];
            }
        }
        $duringdata=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`jointime`, '%Y-%m-%d')as time FROM `yty_qrcodes` where bid=$this->bid UNION select left(pay_time,10) from yty_trades where bid=$this->bid ORDER BY `time` DESC ")->execute()->as_array();
    ...
    public function action_stats_item($action=''){
        ...
        if($_GET['qid']==3||$action=='shaixuan'){
            ...
            if($_GET['data']['begin']!=null&&$_GET['data']['over']!=null)//搜索
            {
               ...
               else
               {
                ...
                $newadd[0]['stock']=$stock[0]['stock'];
                //退款金额
                $trade_close=DB::query(Database::SELECT,"SELECT SUM(payment) as payment from yty_trades where bid=$this->bid and left(pay_time,$length) >='$begin' and left(pay_time,$length) <='$over' and status ='TRADE_CLOSED' ")->execute()->as_array();
        ...
        else
        {
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_scores` where bid=$this->bid and type =4 UNION SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_stocks` where bid=$this->bid and fqid is NUll UNION select left(pay_time,$length) from yty_trades where bid=$this->bid ORDER BY `time` DESC ")->execute()->as_array();
            //$pagesize=2;
            $num=count($days);
            $page = max($_GET['page'], 1);
            $offset = ($this->pagesize * ($page - 1));
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_scores` where bid=$this->bid and type =4 UNION SELECT FROM_UNIXTIME(`lastupdate`, '$daytype')as time FROM `yty_stocks` where bid=$this->bid and fqid is NUll UNION select left(pay_time,$length) from yty_trades where bid=$this->bid ORDER BY `time` DESC limit $this->pagesize offset $offset")->execute()->as_array();
            $newadd=array();
            for($i=0;$days[$i];$i++)
            {
                $time=$days[$i]['time'];
                ...
                $newadd[$i]['stock']=$stock[0]['stock'];
                //退款金额
                $trade_close=DB::query(Database::SELECT,"SELECT SUM(payment) as payment from yty_trades where bid=$this->bid and left(pay_time,$length) LIKE '$time' and status ='TRADE_CLOSED' ")->execute()->as_array();
    ...
    public function action_money(){
        $bid=$this->bid;
        $has=DB::query(Database::SELECT,"SELECT SUM(score) as money_has from yty_scores where bid = $bid and type =4 ")->execute()->as_array();
        $money_has=$has[0]['money_has'];
        $having=DB::query(Database::SELECT,"SELECT SUM(score) as money_having from yty_scores where bid = $bid and type =2 ")->execute()->as_array();
    ...
    public function action_history_trades()
    {
        ...
        $bid = $this->bid;
        $this->config = ORM::factory('yty_cfg')->getCfg($bid);
        $config=$this->config;
        if ($_POST['action']) {
            $action = $_POST['action'];
            $id = $_POST['id'];
        ...
        if($_POST['form']){
            ...
            if($status==1){
                ...
                if($trade->flag==1){
                    ...
                    ORM::factory('yty_score')->where('tid', '=', $trade->id)->delete_all();
                }
            }
        }
        $trade = ORM::factory('yty_trade')->where('bid', '=', $bid);
        $trade = $trade->reset(FALSE);
        if ($_GET['id']){
            $openid=ORM::factory('yty_qrcode')->where('id','=',$_GET['id'])->find()->openid;
        ...
        if ($_GET['s']) {
            ...
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $openids=DB::query(Database::SELECT,"select openid from yty_qrcodes where nickname like '$s'  and bid=$this->bid")->execute()->as_array();
    ...
    public function action_history_withdrawals()
    {
        $bid = $this->bid;
        $config = ORM::factory('yty_cfg')->getCfg($bid);
        $outmoney=ORM::factory('yty_score')->where('bid',"=",$bid)->where('score','<',0);
        $outmoney = $outmoney->reset(FALSE);

        if ($_GET['s']) {
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $qid=DB::query(Database::SELECT,"select id from yty_qrcodes where nickname like '$s'  and bid=$this->bid")->execute()->as_array();
            if(count($qid)>0)
    ...
    public function action_num(){
            ...
            $tradeid=ORM::factory('yty_trade')->order_by('id','ASC')->find_all();
            $i=$j=1;
            foreach ($tradeid as $k)
             {  $i++;
                $goodd=ORM::factory('yty_order')->where('tid',"=",$k->tid)->find();
                if(!$goodd->id)
                {
                    $j++;
                    $tempbid=$k->bid;
                    $tempconfig = ORM::factory('yty_cfg')->getCfg($tempbid);
                    $this->access_token = ORM::factory('yty_login')->where('id','=',$tempbid)->find()->access_token;
                    ...
                    {
                        $good=ORM::factory('yty_order')->where('goodid',"=",$result['response']['trade']['orders'][$j]['num_iid'])->where('tid',"=",$k->tid)->find();
                        if(!$good->id)
                        {
                        $good->bid=$tempbid;
                        $good->tid=$k->tid;
    ...
    public function action_numtest($tid)
    {
            ...
            require_once Kohana::find_file("vendor/kdt","YZTokenClient");
            echo $tid;
            $bid=ORM::factory('yty_trade')->where('tid','=',$tid)->find()->bid;
    ...
    {
        //$goods=ORM::factory('yty_order')->where('bid','=',$this->bid)->find_all();
        $or = 'id';
        if ($_GET['sort']) $or = $_GET['sort'];


        //$goods=DB::query(database::SELECT,"SELECT DISTINCT goodid,title, sum(num) AS tonum,count(id) as totle,sum(price) as toprice  FROM `yty_orders` WHERE bid=$this->bid group BY goodid order by $or DESC")->execute()->as_array();
        $goods=DB::query(database::SELECT,"select DISTINCT (temp.goodid) as goodid,temp.title,sum(temp.price)as toprice,sum(temp.num)as tonum,count(temp.id)as totle from (SELECT yty_orders.* FROM `yty_trades`,yty_orders WHERE yty_orders.tid=yty_trades.tid and yty_trades.status!='TRADE_CLOSED' and yty_trades.status!='TRADE_CLOSED_BY_USER' and yty_trades.status!='NO_REFUND') as temp where temp.bid=$this->bid GROUP by temp.goodid ORDER by $or desc ")->execute()->as_array();

         if ($_GET['s']) {
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $goods=DB::query(database::SELECT,"select DISTINCT (temp.goodid) as goodid,temp.title,sum(temp.price)as toprice,sum(temp.num)as tonum,count(temp.id)as totle from (SELECT yty_orders.* FROM `yty_trades`,yty_orders WHERE yty_orders.tid=yty_trades.tid and yty_trades.status!='TRADE_CLOSED' and yty_trades.status!='TRADE_CLOSED_BY_USER' and yty_trades.status!='NO_REFUND') as temp where temp.bid=$this->bid and temp.title like '$s' GROUP by temp.goodid ORDER by $or desc ")->execute()->as_array();
    ...
    public function action_setgoods()
    {
        ...
        require_once Kohana::find_file("vendor/kdt","YZTokenClient");
        $this->access_token = ORM::factory('yty_login')->where('id','=',$bid)->find()->access_token;
        $tempconfig=ORM::factory('yty_cfg')->getCfg($this->bid);
        if($this->access_token)
        {
            ...
            for($k=0;$k<$a;$k++){
                ...
                for($i=0;$results['response']['items'][$i];$i++){
                    ...
                    $url=$res['detail_url'];
                    $num=$res['num'];
                    $num_num = ORM::factory('yty_setgood')->where('goodid', '=', $num_iid)->count_all();
                    if($num_num==0 && $num_iid!=""){
                        $sql = DB::query(Database::INSERT,"INSERT INTO `yty_setgoods` (`bid`,`goodid`,`title`,`price`, `picurl`,`url`,`status`,`state`) VALUES ($bid,$num_iid,'$name' ,$price,'$pic','$url',0,1)");
                        $sql->execute();
                    }else{
                        $sql = DB::query(Database::UPDATE,"UPDATE `yty_setgoods` SET `bid` = $bid ,`goodid` = $num_iid,`title` ='$name',`price`=$price, `picurl`='$pic',`url`='$url' ,`state` = 1 where `goodid` = $num_iid ");
                        $sql->execute();
                    }
                }
            }
            $sql = DB::query(Database::DELETE,"DELETE FROM `yty_setgoods` where `state` =0 and `bid` = $bid ");
            $sql->execute();
            $sql = DB::query(Database::UPDATE,"UPDATE `yty_setgoods` SET `state` =0 where `bid` = $bid");
            $sql->execute();
        }
    ...
    public function action_setgood1s(){
        $bid = $this->bid;
        $config = ORM::factory('yty_cfg')->getCfg($bid, 1);
        $setgoods = ORM::factory('yty_setgood')->where('bid','=',$bid);
        if ($_GET['s']) {
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
        ...
        if ($_POST['form']['num_iid']) {
            ...
            $setgoods = ORM::factory('yty_setgood')->where('bid', '=', $bid)->where('goodid','=',$goodid)->find();
