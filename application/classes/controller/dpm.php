...
class Controller_Dpm extends Controller_Base{
    ...
    public function before() {
        ...
        set_time_limit(0);
        $this->bid = $_SESSION['dpm']['bid'];
        $_SESSION['dpm']['bid'] = $this->bid;
        //未登录
        if (!$this->bid) {
            header('location:/dpm/login');
            exit;
        }
    }
    ...
    public function action_test2(){
        // $bid=2;
        // $commision=DB::query(Database::SELECT,"SELECT SUM(payment) AS paymoney from yyx_orders where bid=$bid  and unix_timestamp(`pay_time`)>=1479657600 and unix_timestamp(`pay_time`)<=1479744000  ")->execute()->as_array();
        // echo "<pre>";
        // var_dump($commision);
        // echo "</pre>";
        // exit;
        ...
        // set_time_limit(0);
        // $result =ORM::factory('yyx_order')->where('bid','=',1)->find_all();
        // foreach ($result as $a) {
        //     $id=$a->id;
        //     $time=strtotime($a->pay_time);
        //     $order=ORM::factory('yyx_order')->where('id','=',$id)->find();
    ...
    public function action_test3(){
        ...
        //     $newadd[$i]['time']=$time;
        //     echo $time.'<br>';
        //     $commision=DB::query(Database::SELECT,"SELECT SUM(payment) AS paymoney from yyx_orders where bid=$bid and FROM_UNIXTIME(`update_time`,'$daytype')='$time' ")->execute()->as_array();
    ...
    public function action_test8(){
        ...
        for($i=0;$count_time[$i];$i++){
            ...
            $newadd[$i]['time']=$time;
            echo $time.'<br>';
            $commision=DB::query(Database::SELECT,"SELECT SUM(payment) AS paymoney from yyx_orders where bid=$bid and FROM_UNIXTIME(`pay_time`,'$daytype')='$time' ")->execute()->as_array();
            $newadd[$i]['commision']=$commision[0]['paymoney'];
        }
        echo "<pre>";
        var_dump($newadd);
    ...
    public function action_test4(){
        //$aaa = ORM::factory('yyx_order')->where('bid','=',$bid)->select(array('SUM("payment")', 'done'))->order_by('done','desc')->find_all();//累计交易额
        $result['hot_items'] = DB::query(Database::SELECT,"select title,zongfen from (select title,iid,sum(payment) as zongfen from yyx_orders where `bid` = 2 group by iid) a order by a.zongfen desc")->execute()->as_array();
        echo "<pre>";
        var_dump($result);
        echo "<pre>";
    ...
    public function action_test5(){
        $result['hot_items'] = DB::query(Database::SELECT,"select receiver_city,zongfen from (select receiver_city,count(id) as zongfen from yyx_orders where `bid` = 2 group by receiver_city) a order by a.zongfen desc")->execute()->as_array();
        echo "<pre>";
        var_dump($result);
        echo "<pre>";
    }
    public function action_map(){//删除商户 token
        ...
        $result['done_goal'] = ORM::factory('yyx_order')->where('status','!=','TRADE_CLOSED')->where('bid','=',$bid)->where('pay_time','>=',mktime(0,0,0,date('m'),1,date('Y')))->select(array('SUM("payment")', 'done'))->find()->done+ORM::factory('yyx_cfg')->where('bid','=',$bid)->where('key','=','goal1')->find()->value;//本月
        $result['undone_goal'] = $result['sx_goal']-$result['done_goal'];
        // 今日昨日交易
        $result['today_done'] = ORM::factory('yyx_order')->where('status','!=','TRADE_CLOSED')->where('bid','=',$bid)->where('pay_time','>=',mktime(0,0,0,date('m'),date('d'),date('Y')))->select(array('SUM("payment")', 'done'))->find()->done+ORM::factory('yyx_cfg')->where('bid','=',$bid)->where('key','=','goal2')->find()->value;//今日

        $result['yestoday_done'] = ORM::factory('yyx_order')->where('bid','=',$bid)->where('pay_time','>=',mktime(0,0,0,date('m'),date('d')-1,date('Y')))->where('pay_time','<=',mktime(0,0,0,date('m'),date('d'),date('Y')))->select(array('SUM("payment")', 'done'))->find()->done+ORM::factory('yyx_cfg')->where('bid','=',$bid)->where('key','=','goal3')->find()->value;//昨日
        ...
        //热销商品，按销量排行
        $result['hot_items'] = DB::query(Database::SELECT,"SELECT lv,title,iid,zongfen from (select yyx_items.lv,yyx_orders.iid,yyx_orders.title,sum(yyx_orders.num) as zongfen from yyx_orders inner join yyx_items on yyx_orders.iid=yyx_items.id where yyx_orders.bid=$bid and yyx_orders.status !='TRADE_CLOSED'  group by iid) a order by (a.zongfen+a.lv) desc limit 0,10")->execute()->as_array();
        foreach ($result['hot_items'] as $k => $hot_items) {
            $oldcustomer=DB::query(Database::SELECT,"SELECT sum(a.oldnum) as oldctn from (SELECT sum(num) as oldnum from yyx_orders where qid!=0 and bid = $bid and iid = '{$hot_items['iid']}' group by qid having count(qid) > 1) a")->execute()->as_array()[0]['oldctn'];
        ...
        //热销商品，按销售额排行
        $result['hot_items_num'] = DB::query(Database::SELECT,"SELECT lv,title,zongfen from (SELECT yyx_items.lv,yyx_orders.iid,yyx_orders.title,sum(yyx_orders.payment) as zongfen from yyx_orders inner join yyx_items on yyx_orders.iid=yyx_items.id where yyx_orders.bid=$bid and yyx_orders.status !='TRADE_CLOSED' group by iid) a order by a.zongfen desc limit 0,10")->execute()->as_array();
        foreach ($result['hot_items_num'] as $k => $hot_items_num) {
            $result['hot_items_num'][$k]['title']=$this->sub_enter2($result['hot_items_num'][$k]['title']);
        }
        //热销城市排行
        $result['hot_area'] = DB::query(Database::SELECT,"SELECT receiver_city,zongfen from (select receiver_city,count(id) as zongfen from yyx_orders where `bid` = $bid and status !='TRADE_CLOSED' group by receiver_city) a order by a.zongfen desc limit 0,10")->execute()->as_array();
        //新老会员
        ...
        // exit;
        //地理位置
        $result['orders'] = DB::query(Database::SELECT,"SELECT * FROM (SELECT * FROM yyx_orders where receiver_city!='' and bid=$bid and status!='TRADE_CLOSED' ORDER BY pay_time DESC , has_show ASC) BIAOMING GROUP BY receiver_city ORDER BY pay_time desc LIMIT 40")->execute()->as_array();
        ...
        if($_GET['t']=='d2'){
            ...
            $date=date('Y-m-d',time());
            //热销商品，按销量排行
            $result['hot_items'] = DB::query(Database::SELECT,"select lv,title,iid,zongfen from (select yyx_items.lv,yyx_orders.iid,yyx_orders.title,sum(yyx_orders.num) as zongfen from yyx_orders inner join yyx_items on yyx_orders.iid=yyx_items.id where yyx_orders.bid=$bid and yyx_orders.status !='TRADE_CLOSED' and FROM_UNIXTIME(yyx_orders.pay_time,'$daytype')='$date'  group by iid) a order by (a.zongfen+a.lv) desc limit 0,10")->execute()->as_array();
            foreach ($result['hot_items'] as $k => $hot_items) {
                $oldcustomer=DB::query(Database::SELECT,"SELECT a.oldnum as oldctn from(SELECT sum(num) as oldnum from yyx_orders where qid!=0 and FROM_UNIXTIME(pay_time,'$daytype')='$date' and bid = $bid and iid = '{$hot_items['iid']}' group by qid having count(qid) > 1) a")->execute()->as_array()[0]['oldnum'];
                $result['hot_items'][$k]['oldCTN']=$oldcustomer;
                $result['hot_items'][$k]['newCTN']=$hot_items['zongfen']-$oldcustomer;
            ...
            //热销商品，按销售额排行
            $result['hot_items_num'] = DB::query(Database::SELECT,"select lv,title,zongfen from (select yyx_items.lv,yyx_orders.iid,yyx_orders.title,sum(yyx_orders.payment) as zongfen from yyx_orders inner join yyx_items on yyx_orders.iid=yyx_items.id where yyx_orders.bid=$bid and yyx_orders.status !='TRADE_CLOSED' and FROM_UNIXTIME(yyx_orders.pay_time,'$daytype')='$date' group by iid) a order by a.zongfen desc limit 0,10")->execute()->as_array();
            $a = 9;
            for ($i=0; $i <=$a ; $i++) {
                if($result['hot_items_num'][$i]['title']){
                   $arr[$i][0] = $i+1;
            ...
            //热销城市排行
            $result['hot_area'] = DB::query(Database::SELECT,"select receiver_city,zongfen from (select receiver_city,count(id) as zongfen from yyx_orders where `bid` = $bid and status !='TRADE_CLOSED' and FROM_UNIXTIME(yyx_orders.pay_time,'$daytype')='$date' group by receiver_city) a order by a.zongfen desc limit 0,10")->execute()->as_array();
            $a = 9;
            for ($i=0; $i <=$a ; $i++) {
           ...
           //热销商品，按销量排行
            $result['hot_items'] = DB::query(Database::SELECT,"select lv,title,iid,zongfen from (select yyx_items.lv,yyx_orders.iid,yyx_orders.title,sum(yyx_orders.num) as zongfen from yyx_orders inner join yyx_items on yyx_orders.iid=yyx_items.id where yyx_orders.bid=$bid and yyx_orders.status !='TRADE_CLOSED' and FROM_UNIXTIME(yyx_orders.pay_time,'$daytype')='$date' group by iid) a order by (a.zongfen+a.lv) desc limit 0,10")->execute()->as_array();
            foreach ($result['hot_items'] as $k => $hot_items) {
                $oldcustomer=DB::query(Database::SELECT,"SELECT sum(a.oldnum) as oldctn from (SELECT sum(num) as oldnum from yyx_orders where qid!=0 and FROM_UNIXTIME(pay_time,'$daytype')='$date' and bid = $bid and iid = '{$hot_items['iid']}' group by qid having count(qid) > 1) a")->execute()->as_array()[0]['oldnum'];
                $result['hot_items'][$k]['oldCTN']=$oldcustomer;
                $result['hot_items'][$k]['newCTN']=$hot_items['zongfen']-$oldcustomer;
            ...
            $result['hot_num'] = array('热销商品金额'=>$arr);
            //热销城市排行
            $result['hot_area'] = DB::query(Database::SELECT,"select receiver_city,zongfen from (select receiver_city,count(id) as zongfen from yyx_orders where `bid` = $bid and yyx_orders.status !='TRADE_CLOSED' and FROM_UNIXTIME(yyx_orders.pay_time,'$daytype')='$date' group by receiver_city) a order by a.zongfen desc limit 0,10")->execute()->as_array();
            $a = 9;
            for ($i=0; $i <=$a ; $i++) {
                if($result['hot_area'][$i]['receiver_city']){
                    $result['hot_area'][$i]['receiver_city'] = mb_substr($result['hot_area'][$i]['receiver_city'],0,3,'utf-8');
    ...
    public function action_smfyun(){//删除商户 token
        ...
        foreach ($result['hot_items'] as $k => $hot_items) {
            $oldcustomer=DB::query(Database::SELECT,"SELECT sum(a.oldnum) as oldctn from (SELECT sum(num) as oldnum from yyx_orders where qid!=0 and bid = $bid and iid = '{$hot_items['iid']}' group by qid having count(qid) > 1) a")->execute()->as_array()[0]['oldctn'];
            //$oldcustomer=DB::query(Database::SELECT,"SELECT sum(num) as oldnum from yyx_orders where qid!=0 and bid = $bid and iid = '{$hot_items['iid']}' group by qid having count(qid) > 1")->execute()->as_array();
