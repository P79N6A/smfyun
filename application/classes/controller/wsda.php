...
//分销宝后台
class Controller_wsda extends Controller_Base {

    public $template = 'weixin/wsd/tpl/fatpl';
    public $pagesize = 10;
    public $access_token;
    public $config;
    public $bid;
    ...
    public function before() {
        ...
        $this->bid = $_SESSION['wsda']['bid'];
        $this->config = $_SESSION['wsda']['config'];
        $this->access_token=ORM::factory('wsd_login')->where('id', '=', $this->bid)->find()->access_token;
        //未登录
        if (Request::instance()->action != 'login' && !$this->bid) {
            // header('location:/wsda/login');
            header('location:/wsda/login?from='.Request::instance()->action);
            exit;
        }
    }
    ...
    public function after() {
        if ($this->bid) {
            $todo['users'] = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->count_all();
            $todo['tickets'] = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('ticket', '<>', '')->count_all();

            //$todo['items'] = ORM::factory('wsd_order')->where('bid', '=', $this->bid)->where('status', '=', 0)->count_all();
    ...
    public function action_delete_money(){
        $bid = $this->bid;
        $title = '尛鲜生|爆汁爱媛果冻橙（四川丹棱）';
        $orders = ORM::factory('wsd_trade')->where('bid','=',$bid)->where('title','=',$title)->where('pay_time','<','2017-10-27 12:00:00')->find_all();
        echo ORM::factory('wsd_trade')->where('bid','=',$bid)->where('title','=',$title)->where('pay_time','<','2017-10-27 12:00:00')->count_all();
        // exit;
        foreach ($orders as $k => $v) {
            $v->money1 = 0.00;
            $score = ORM::factory('wsd_score')->where('bid','=',$bid)->where('tid','=',$v->id)->find();
            $score->score = 0.00;
            $v->save();
            $score->save();
        }
    ...
    public function action_test(){
        //$bid=4;
        //$title='秒杀|爆汁爱媛38号果冻橙（四川丹棱），果汁爆满，入口即化，京东生鲜包邮';
        // $access_token='ca645187b82d3f87bc209c6a1adbfc93';
        // require Kohana::find_file("vendor","kdt/YZTokenClient");
        // $orders=ORM::factory('wsd_trade')->where('bid','=',$bid)->where('title','=',$title)->find_all();
        ...
        //     // $score->save();
        //     echo $score->score.'<br>';
        //     echo "tid:".$order->tid.'<br>';
        //     echo "订单金额:".$order->money.'<br>';
        //     echo "订单收益:".$order->money1.'<br>';
        //     echo "-----------------------<br>";
        // }
    ...
    // public function action_test1($status){
    //     switch ($status) {
    ...
    public function action_oauth(){
        ...
        Request::instance()->redirect('https://open.youzan.com/oauth/authorize?client_id=c84f1b66c949fc9725&response_type=code&state=teststate&redirect_uri=http://'.$_SERVER["HTTP_HOST"].'/wsda/callback');
    }
    //回调获取 商户信息
    public function action_callback(){
        ...
        $data=array(
            ...
            "code"=>$code,
            "redirect_uri"=>'http://'.$_SERVER["HTTP_HOST"].'/wsda/callback'
        );
        $ch=curl_init();
        ...
        if(isset($result->access_token))
        {
            ...
            $sid = $value['id'];
            $name = $value['name'];
            $usershop = ORM::factory('wsd_login')->where('id','=',$this->bid)->find();
            ...
            $usershop->save();
            echo "<script>alert('授权成功');location.href='".URL::site("wsda/home")."';</script>";
        }
        //Request::instance()->redirect('wsda/home');
    }
    public function action_home() {
        require_once Kohana::find_file('vendor', 'weixin/wechat.class');

        $bid = $this->bid;
        $config = ORM::factory('wsd_cfg')->getCfg($bid, 1);

        //密码修改
        if ($_POST['password'] && $_POST['newpassword']) {
            $biz = ORM::factory('wsd_login', $bid);
        ...
        $cert_file = DOCROOT."wsd/tmp/$bid/cert.{$config['appsecret']}.pem";
        $key_file = DOCROOT."wsd/tmp/$bid/key.{$config['appsecret']}.pem";
        $result['cert_file_exists'] = file_exists($cert_file);
        $result['key_file_exists'] = file_exists($key_file);
        //店铺地址设置
        if ($_POST['shopurl']) {
            $cfg = ORM::factory('wsd_cfg');
        ...
        if ($_POST['cfg']) {
            ...
            if (file_exists($cert_file)) $cfg->setCfg($bid, 'wsd_file_cert', '', file_get_contents($cert_file));
            if (file_exists($key_file)) $cfg->setCfg($bid, 'wsd_file_key', '', file_get_contents($key_file));
        ...
        $access_token = ORM::factory('wsd_login')->where('id', '=', $bid)->find()->access_token;

        if(!$access_token){
            $oauth=1;
        }
        $this->template->title = '首页';
        $this->template->content = View::factory('weixin/wsd/admin/home')
            ->bind('result', $result)
            ->bind('oauth',$oauth)
            ->bind('config', $config);
    }
    public function action_skus($action='', $id=0) {
        ...
        $bid = $this->bid;
        $config = ORM::factory('wsd_cfg')->getCfg($bid);
        $result['skus'] = ORM::factory('wsd_sku')->where('bid', '=', $bid)->order_by('lv', 'ASC')->find_all();
        // if($_POST){
        //     echo "<pre>";
        //     var_dump($_POST);
        //     echo "</pre>";
        ...
        // }
        if ($_POST['text']) {
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_self',$_POST['text']['self']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_direct',$_POST['text']['direct']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_group',$_POST['text']['group']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_dirctcus',$_POST['text']['dirctcus']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_customer',$_POST['text']['customer']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_dirctorder',$_POST['text']['dirctorder']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_selforder',$_POST['text']['selforder']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'text_order',$_POST['text']['order']);
        }
        if($_POST['quest']){
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'buy_money',$_POST['quest']['buy']);
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'buy_url',$_POST['quest']['buy_url']);
        }
        if($_POST['calcu']){
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'money_type',$_POST['calcu']['type']);
        }
        if($_POST['ivcode']){
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'code',$_POST['ivcode']);
        }
        if($_POST['date']){
            $ok=ORM::factory('wsd_cfg')->setCfg($bid,'date',$_POST['date']);
        }
        $config = ORM::factory('wsd_cfg')->getCfg($bid,1);
        if ($_POST['menu']) {
            ORM::factory('wsd_sku')->where('bid', '=', $bid)->delete_all();
            $menu=$_POST['menu'];
            $level=($menu['count']);
            for ($i=1; $i <=$level ; $i++) {
                $j=$i*2-1;
                $k=$i*2;
                $skus= ORM::factory('wsd_sku');
                $skus->bid = $this->bid;
                $skus->lv = $i;
                $skus->scale = $menu['value_c'.$i.'_wsd'];
                $skus->money1 = $menu['key_c'.$j.'_wsd'];
                $skus->money2 = $menu['key_c'.$k.'_wsd'];
                $skus->save();
            }
            Request::instance()->redirect('wsda/skus');


        }

        $this->template->title = '代理设置';
        $this->template->content = View::factory('weixin/wsd/admin/skus')
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_skus_add() {
        ...
        $this->template->content = View::factory('weixin/wsd/admin/skus_add')
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_group($action='', $id=0) {
        ...
        $result['group'] = ORM::factory('wsd_suite')->where('bid', '=', $bid)->order_by('id', 'DESC')->find_all();

        $this->template->title = '分销商分组';
        $this->template->content = View::factory('weixin/wsd/admin/group')
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_group_add() {
        ...
        if ($_POST['data']) {
            ...
            if (!$result['error']) {
                ...
                Request::instance()->redirect('wsda/group');
            }
        }

        $result['action'] = 'add';
        $result['title'] = $this->template->title = '添加';
        $this->template->content = View::factory('weixin/wsd/admin/group_add')
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_group_edit($id) {
        ...
        $group = ORM::factory('wsd_suite', $id);
        if (!$group || $group->bid != $bid) die('404 Not Found!');

        if ($_GET['DELETE'] == 1) {
            $sum = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('group_id','=',$id)->count_all();
            if($sum>0){
                die('该分组下有分销商不允许删除！');
            }
            $group->delete();
    ...
    public function action_qrcodes_detail($id){
        ...
        $this->template->content = View::factory('weixin/wsd/admin/qrcodes_detail')
            ->bind('result', $result);
    }
    //审核管理
    public function action_qrcodes_m($action='', $id=0) {
        ...
        $alls = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','=',1)->find_all();
        //修改用户
        if ($_POST['form']['id']) {

            $id = $_POST['form']['id'];
            $qrcode_edit = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('id', '=', $id)->find();
            if ($qrcode_edit->id) {
                if (isset($_POST['form']['lv'])) {
                    $qrcode_edit->lv = (int)$_POST['form']['lv'];
                    $qrcode_edit->tel = $_POST['form']['tel'];
                    if($qrcode_edit->lv==3){//删除该代理商
                        $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $qrcode_edit->fopenid)->find();
                        //找出直属下线
                        $childs = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('fopenid', '=', $qrcode_edit->openid)->find_all();
                        //修改他们的上级为上级的上级
                        ...
                        $new_fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('id', '=', $_POST['form']['fuser'])->find();
                        if(!$new_fuser->id){//不修改关系
                            ...
                            while($loop_qrcode->fopenid){
                                ...
                                $loop_qrcode = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $loop_qrcode->fopenid)->find();
                            ...
                            foreach ($bottom_arr as $k => $v) {
                                if($v){
                                    $now_user = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('id', '=', $v)->find();
                                    $now_fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $now_user->fopenid)->find();
                            ...
                            // 组问题最后考虑了
                            // 未修改之前  当前修改用户存在的组
                            $now_user_g = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$qrcode_edit->id)->order_by('id','DESC')->find();
                            ...
                            foreach ($bottom_arr as $k => $v) {//先顺着轮只记录fgid
                                $old_group = ORM::factory('wsd_group')->where('bid','=',$bid)->where('id','=',$v)->order_by('id','DESC')->find();
                                $new_add_g = ORM::factory('wsd_group');
                                $new_add_g->bid=$bid;
                                $new_add_g->qid=$old_group->qid;
                                if($old_group->qid == $qrcode_edit->id){//如果是自己 就新增上级
                                    // $new_add_g->fqid=ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$new_fuser->id)->order_by('id','DESC')->find()->id;
                            ...
                            $new_fuser_g = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$new_fuser->id)->order_by('id','DESC')->find();
                            foreach ($new_add_arr as $k => $v) {//再轮一遍
                                // echo $k.'::'.$v.'<br>';
                                $new_add_g = ORM::factory('wsd_group')->where('bid','=',$bid)->where('id','=',$v)->order_by('id','DESC')->find();
                                $old_g = ORM::factory('wsd_group')->where('bid','=',$bid)->where('id','=',$k)->find();
                            ...
                            //找出当前用户最新的组
                            $now_new_user = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$qrcode_edit->id)->order_by('id','DESC')->find();
                            // 给新的上级每个加bottom
                            ...
                            while ($loop_group->id) {
                                ...
                                $loop_group->save();
                                $loop_group = ORM::factory('wsd_group')->where('bid','=',$bid)->where('id','=',$loop_group->fgid)->order_by('id','DESC')->find();
                            }
                            // exit;
                        }
                    }
        ...
        $qrcode = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('lv','=',1);
        ...
        if ($_GET['export']=='xls') {
            ...
            foreach($qrcodes as $k => $v){
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('P'.$num, date('Y-m-d H:i:s',$v->jointime));
            }
        ...
        if($_GET['qid']){
            $openid=ORM::factory('wsd_qrcode')->where('id','=',$_GET['qid'])->find()->openid;
        ...
        if ($_GET['fopenid']) {//下线
            ...
            $result['fuser'] = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $result['fopenid'])->find();
        ...
        if ($_GET['sort']=='fans_num'){
            $qrcodes = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where_open()->where('lv','=',1)->or_where('lv','=',3)->where_close()->find_all();
            foreach ($qrcodes as $k => $v) {
                $num = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('fopenid','=',$v->openid)->where('subscribe','=',1)->count_all();
                if($v->fans_num!=$num){
                    $v->fans_num = $num;
                    $v->save();
                }
        ...
        //分组
        $suite= ORM::factory('wsd_suite')->where('bid','=',$bid)->find_all();
        //分页
        ...
        ))->render('weixin/wsd/admin/pages');

        $result['qrcodes'] = $qrcode->order_by($result['sort'], 'DESC')->limit($this->pagesize)->offset($offset)->find_all();
        $group = ORM::factory('wsd_group')->where('bid','=',$bid)->find_all();
        $gnum = ORM::factory('wsd_group')->where('bid','=',$bid)->count_all();
        $this->template->title = '代理列表';
        $this->template->content = View::factory('weixin/wsd/admin/qrcodes_m')
            ->bind('pages', $pages)
            ->bind('group', $group)
            ->bind('gnum', $gnum)
            ->bind('result', $result)
    ...
    public function action_qrcodes($action='', $id=0) {
            ...
            if ($qrcode_edit->id) {
                if (isset($_POST['form']['lv'])) {
                    ...
                    if((int)$_POST['form']['lv']==1){
                        //给予编号
                        $front_user = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('id', '<', $qrcode_edit->id)->order_by('id','desc')->find();
        ...
        $qrcode = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where_open()->where('lv','=',2)->or_where('lv','=',4)->where_close();
        $qrcode = $qrcode->reset(FALSE);
        ...
        $this->template->content = View::factory('weixin/wsd/admin/qrcodes')
            ->bind('pages', $pages)
            ->bind('group', $group)
            ->bind('gnum', $gnum)
            ->bind('result', $result)
    ...
    public function action_logins($action='', $id=0) {
       ...
       if ($_GET['s']) {
        ...
        $this->template->content = View::factory('weixin/wsd/admin/logins')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_logins_add() {
        if ($_SESSION['wsda']['admin'] < 2) Request::instance()->redirect('wsda/home');

        $bid = $this->bid;

        if ($_POST['data']) {
            $login = ORM::factory('wsd_login');
            $login->values($_POST['data']);
            if (!$_POST['data']['name'] || !$_POST['data']['user']) $result['error'] = '请填写完整后再提交';
            if (ORM::factory('wsd_login')->where('user', '=', $_POST['data']['user'])->count_all() > 0) $result['error'] = '该登录名已经存在';

            if (!$result['error']) {
                $login->pass = Text::random(NULL, 6);
                if ($_POST['pass']) $login->pass = $_POST['pass'];
                $login->save();
                Request::instance()->redirect('wsda/logins');
            }
        }
        ...
        $this->template->content = View::factory('weixin/wsd/admin/logins_add')
            ->bind('result', $result)
            ->bind('config', $config);
    }
    ...
    public function action_logins_edit($id) {
        ...
        $login = ORM::factory('wsd_login', $id);
        ...
        if ($_GET['DELETE'] == 1) {
            //$login->delete();
            Request::instance()->redirect('wsda/items');
        }

        if ($_POST['data']) {
            $login->values($_POST['data']);
            if (!$_POST['data']['name'] || !$_POST['data']['user']) $result['error'] = '请填写完整后再提交';
            if (ORM::factory('wsd_login')->where('user', '=', $_POST['data']['user'])->where('id', '<>', $id)->count_all() > 0)
    ...
    public function action_login() {
        $this->template = 'weixin/wsd/tpl/login';
        $this->before();
        ...
        if ($_POST['username'] && $_POST['password']) {
            $biz = ORM::factory('wsd_login')->where('user', '=', $_POST['username'])->where('pass', '=', $_POST['password'])->find();
            ...
            if ($biz->id) {
                ...
                if ($biz->expiretime && (strtotime($biz->expiretime)+86400) < time()) {
                    ...
                    $_SESSION['wsda']['bid'] = $biz->id;
                    $_SESSION['wsda']['user'] = $_POST['username'];
                    $_SESSION['wsda']['admin'] = $biz->admin; //超管
                    $_SESSION['wsda']['config'] = ORM::factory('wsd_cfg')->getCfg($biz->id);
        ...
        if ($_SESSION['wsda']['bid']) {
            if (!$_GET['from']) $_GET['from'] = 'home';
            header('location:/wsda/'.$_GET['from']);
            exit;
        }
    }
    ...
    public function action_export_data_users(){
        ...
        if($_POST['data']['begin']!=NULL&&$_POST['data']['over']!=NULL){
           ...
           else
           {
            ...
            $users = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','=',1)->find_all();
            ...
            foreach ($users as $k => $v) {
                ...
                $newadd[$k]['goodsnum'] = 0;
                $newadd[$k]['payment'] = 0;
                $fans=DB::query(Database::SELECT,"select count(openid) as fansnum from wsd_qrcodes where bid=$this->bid and fopenid='$v->openid' and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' ")->execute()->as_array();
                $newadd[$k]['fansnum']=$fans[0]['fansnum'];
                //有赞订单数、有赞商品交易数量、有赞成交金额
                $tradesdata=DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from wsd_trades where bid=$this->bid and fopenid='$v->openid' and left(pay_time,$length) >='$begin' and left(pay_time,$length) <='$over'")->execute()->as_array();
    ...
    public function action_export_data_groups(){
        ...
        if($_POST['data']['begin']!=NULL&&$_POST['data']['over']!=NULL){
           ...
           else
           {
            ...
            $groups = ORM::factory('wsd_group')->where('bid','=',$bid)->find_all();
            ...
            foreach ($groups as $k => $v) {
                $group_users = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('group_id','=',$v->id)->find_all();
                $group_num = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('group_id','=',$v->id)->count_all();
                $addcount[$k] = 0;
                $addtradesnum[$k] = 0;
                $addgoodnum[$k] = 0;
                $addpayment[$k] = 0;
                foreach ($group_users as $g => $u) {
                    //新增用户
                    $fans=DB::query(Database::SELECT,"select count(openid) as fansnum from wsd_qrcodes where bid=$this->bid and fopenid='$u->openid' and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' ")->execute()->as_array();
                    $addcount[$k] = $addcount[$k] + $fans[0]['fansnum'];
                    //有赞订单数，有赞订单数、有赞商品交易数量、有赞成交金额
                    $tradesdata =DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from wsd_trades where bid=$this->bid and fopenid='$u->openid' and left(pay_time,$length) >='$begin' and left(pay_time,$length) <='$over'")->execute()->as_array();
    ...
    public function diff_date($date1, $date2){
     ...
     public function action_stats_totle($action=''){
        ...
        if($_GET['qid']==3||$action=='shaixuan'){
            ...
            if($_GET['data']['begin']!=null&&$_GET['data']['over']!=null){
               ...
               if(strtotime($begin)==strtotime($over)){
                ...
                //新增代理数
                $fans=DB::query(Database::SELECT,"SELECT count(id) as dlnum from wsd_qrcodes where bid=$this->bid and lv=1 and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' ")->execute()->as_array();
                $newadd[0]['dlnum']=$fans[0]['dlnum'];
                //新增客户数
                $ticket=DB::query(Database::SELECT,"SELECT count(id) as khnum from wsd_qrcodes where bid=$this->bid and lv !=1 and fopenid !='' and FROM_UNIXTIME(`jointime`, '$daytype')>='$begin' and FROM_UNIXTIME(`jointime`, '$daytype')<='$over' ")->execute()->as_array();
                $newadd[0]['khnum']=$ticket[0]['khnum'];

                //有赞订单数，有赞订单数、有赞商品交易数量、有赞成交金额
                $tradesdata=DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from wsd_trades where bid=$this->bid and left(pay_time,$length) >='$begin' and left(pay_time,$length) <='$over'")->execute()->as_array();
                $newadd[0]['tradesnum']=$tradesdata[0]['tradesnum'];
                $newadd[0]['goodsnum']=$tradesdata[0]['goodnum'];
                $newadd[0]['payment']=$tradesdata[0]['payment'];

                //所有佣金 已结算的佣金、待结算的佣金
                $commision=DB::query(Database::SELECT,"SELECT SUM(score) AS paymoney from wsd_scores where bid=$this->bid and score > 0 and FROM_UNIXTIME(`lastupdate`, '$daytype')>='$begin' and FROM_UNIXTIME('lastupdate','$daytype')<='$over' and tid !=0")->execute()->as_array();
        ...
        else
        {
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`jointime`, '$daytype')as time FROM `wsd_qrcodes` where bid=$this->bid UNION select left(pay_time,$length) from wsd_trades where bid=$this->bid ORDER BY `time` DESC ")->execute()->as_array();
            //$pagesize=2;
            $num=count($days);
            $page = max($_GET['page'], 1);
            $offset = ($this->pagesize * ($page - 1));
            ...
            $days=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`jointime`, '$daytype')as time FROM `wsd_qrcodes` where bid=$this->bid UNION select left(pay_time,$length) from wsd_trades where bid=$this->bid ORDER BY `time` DESC limit $this->pagesize offset $offset")->execute()->as_array();
            $newadd=array();
            for($i=0;$days[$i];$i++)
            {
                ...
                $newadd[$i]['time']=$time;
                //新增用户
                $fans=DB::query(Database::SELECT,"SELECT count(id) as dlnum from wsd_qrcodes where bid=$this->bid and lv =1 and FROM_UNIXTIME(`jointime`, '$daytype')='$time'")->execute()->as_array();
                $newadd[$i]['dlnum']=$fans[0]['dlnum'];

                //产生海报数
                $ticket=DB::query(Database::SELECT,"SELECT count(id) as khnum from wsd_qrcodes where bid=$this->bid and lv !=1 and fopenid !='' and FROM_UNIXTIME(`jointime`, '$daytype')='$time' ")->execute()->as_array();
                $newadd[$i]['khnum']=$ticket[0]['khnum'];

                //有赞订单数，有赞订单数、有赞商品交易数量、有赞成交金额
                $tradesdata=DB::query(Database::SELECT,"SELECT COUNT(id) AS tradesnum,SUM(NUM) as goodnum,SUM(payment) as payment from wsd_trades where bid=$this->bid and left(pay_time,$length) LIKE '$time'")->execute()->as_array();
                $newadd[$i]['tradesnum']=$tradesdata[0]['tradesnum'];
                $newadd[$i]['goodsnum']=$tradesdata[0]['goodnum'];
                $newadd[$i]['payment']=$tradesdata[0]['payment'];

                //所有佣金 已结算的佣金、待结算的佣金
                $commision=DB::query(Database::SELECT,"SELECT SUM(score) AS paymoney from wsd_scores where bid=$this->bid and score>0 and FROM_UNIXTIME(`lastupdate`, '$daytype')='$time' and tid !=0")->execute()->as_array();
               // var_dump($commision);
                $newadd[$i]['commision']=$commision[0]['paymoney'];
            }
        }
        $duringdata=DB::query(Database::SELECT,"SELECT FROM_UNIXTIME(`jointime`, '%Y-%m-%d')as time FROM `wsd_qrcodes` where bid=$this->bid UNION select left(pay_time,10) from wsd_trades where bid=$this->bid ORDER BY `time` DESC ")->execute()->as_array();
        ...
        $this->template->content = View::factory('weixin/wsd/admin/stats_totle')
        ->bind('newadd',$newadd)
        ->bind('status',$status)
        ->bind('pages', $pages)
    ...
    public function action_history_trades()
    {
        ...
        $trade = ORM::factory('wsd_trade')->where('bid', '=', $bid);
        ...
        if ($_GET['export']=='xls') {
            ...
            foreach($orders as $k => $v){
                $fuser=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$v->fopenid)->find();
                switch ($v->status) {
                    case 'WAIT_SELLER_SEND_GOODS':
                        $status='已付款';
                        break;
        ...
        if($_GET['qid']||$_GET['flag']){
            $openid=ORM::factory('wsd_qrcode')->where('id','=',$_GET['qid'])->find()->openid;
            $groups=ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$_GET['qid'])->find_all();
            $child_group=array();
            foreach ($groups as $group) {
                $bottom=$group->bottom;
                if($bottom){
        ...
        if ($_GET['s']) {
            ...
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $openids=DB::query(Database::SELECT,"select openid from wsd_qrcodes where nickname like '$s'  and bid=$this->bid")->execute()->as_array();
        ...
        $this->template->content = View::factory('weixin/wsd/admin/history_trades')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config);
    ...
    public function action_history_withdrawals()
    {
        $bid = $this->bid;
        $config = ORM::factory('wsd_cfg')->getCfg($bid);
        $outmoney=ORM::factory('wsd_score')->where('bid',"=",$bid)->where('score','<',0);
        $outmoney = $outmoney->reset(FALSE);

        if ($_GET['s']) {
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $qid=DB::query(Database::SELECT,"select id from wsd_qrcodes where nickname like '$s'  and bid=$this->bid")->execute()->as_array();
        ...
        $this->template->content = View::factory('weixin/wsd/admin/history_withdrawals')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config);
        }
    ...
    public function action_num()
    {
            ...
            $tradeid=ORM::factory('wsd_trade')->order_by('id','ASC')->find_all();
            $i=$j=1;
            foreach ($tradeid as $k)
             {  $i++;
                $goodd=ORM::factory('wsd_order')->where('tid',"=",$k->tid)->find();
                if(!$goodd->id)
                {
                    $j++;
                    $tempbid=$k->bid;
                    $tempconfig = ORM::factory('wsd_cfg')->getCfg($tempbid);
                    $this->access_token = ORM::factory('wsd_login')->where('id','=',$tempbid)->find()->access_token;
                    ...
                    {
                        $good=ORM::factory('wsd_order')->where('goodid',"=",$result['response']['trade']['orders'][$j]['num_iid'])->where('tid',"=",$k->tid)->find();
                        if(!$good->id)
                        {
                        $good->bid=$tempbid;
                        $good->tid=$k->tid;
    ...
    public function action_numtest($tid)
    {
            ...
            require_once Kohana::find_file("vendor/kdt","YZTokenClient");
            echo $tid;
            $bid=ORM::factory('wsd_trade')->where('tid','=',$tid)->find()->bid;

            $this->access_token = ORM::factory('wsd_login')->where('id','=',$bid)->find()->access_token;
            $tempbid=$bid;
            $tempconfig = ORM::factory('wsd_cfg')->getCfg($tempbid);
    ...
    {
        //$goods=ORM::factory('wsd_order')->where('bid','=',$this->bid)->find_all();
        $or = 'id';
        if ($_GET['sort']) $or = $_GET['sort'];


        //$goods=DB::query(database::SELECT,"SELECT DISTINCT goodid,title, sum(num) AS tonum,count(id) as totle,sum(price) as toprice  FROM `wsd_orders` WHERE bid=$this->bid group BY goodid order by $or DESC")->execute()->as_array();
        $goods=DB::query(database::SELECT,"select DISTINCT (temp.goodid) as goodid,temp.title,sum(temp.price)as toprice,sum(temp.num)as tonum,count(temp.id)as totle from (SELECT wsd_orders.* FROM `wsd_trades`,wsd_orders WHERE wsd_orders.tid=wsd_trades.tid and wsd_trades.status!='TRADE_CLOSED' and wsd_trades.status!='TRADE_CLOSED_BY_USER' and wsd_trades.status!='NO_REFUND') as temp where temp.bid=$this->bid GROUP by temp.goodid ORDER by $or desc ")->execute()->as_array();

         if ($_GET['s']) {
            $result['s'] = $_GET['s'];
            $s = '%'.trim($_GET['s'].'%');
            $goods=DB::query(database::SELECT,"select DISTINCT (temp.goodid) as goodid,temp.title,sum(temp.price)as toprice,sum(temp.num)as tonum,count(temp.id)as totle from (SELECT wsd_orders.* FROM `wsd_trades`,wsd_orders WHERE wsd_orders.tid=wsd_trades.tid and wsd_trades.status!='TRADE_CLOSED' and wsd_trades.status!='TRADE_CLOSED_BY_USER' and wsd_trades.status!='NO_REFUND') as temp where temp.bid=$this->bid and temp.title like '$s' GROUP by temp.goodid ORDER by $or desc ")->execute()->as_array();
        ...
        $this->template->content = View::factory('weixin/wsd/admin/stats_goods')
        ->bind('goods',$goods)
        ->bind('pages', $pages)
        ->bind('result',$result)
        ->bind('or',$or);
    ...
    public function action_setgood1s(){
        ...
        require_once Kohana::find_file("vendor/kdt","YZTokenClient");
        $this->access_token = ORM::factory('wsd_login')->where('id','=',$bid)->find()->access_token;
        $tempconfig=ORM::factory('wsd_cfg')->getCfg($this->bid);
        if($this->access_token){
            ...
            if(isset($total_result['response']['count'])){
                $item_num=ORM::factory('wsd_setgood')->where('bid','=',$bid)->count_all();
                // echo $total."<br>";
                // echo $item_num."<br>";
                        ...
                        for($i=0;$results['response']['items'][$i];$i++){
                            ...
                            if($skus){
                                ...
                                foreach ($skus as $sku) {
                                    ...
                                    $num=$sku['quantity'];
                                    // echo $sku_id."<br>";
                                    $sku_num = ORM::factory('wsd_goodsku')->where('sku_id', '=', $sku_id)->count_all();
                                    // echo $sku_num.'<br>';
                                    if($sku_num==0 && $sku_id){
                                        // echo "上面<br>";
                                        $sql = DB::query(Database::INSERT,"INSERT INTO `wsd_goodskus` (`bid`,`item_id`,`title`,`sku_id`, `price`,`status`,`state`,`num`) VALUES ($bid,$item_id,'$title' ,$sku_id,$price,0,1,$num)");
                                        $sql->execute();
                                    }else{
                                        // echo "下面<br>";
                                        $sql = DB::query(Database::UPDATE,"UPDATE `wsd_goodskus` SET `bid` = $bid ,`item_id` = $item_id,`title` ='$title',`sku_id`=$sku_id, `price`=$price,`state` = 1 , `num`= $num where `sku_id` = $sku_id ");
                                        $sql->execute();
                                    }
                                }
                            }
                            ...
                            $url=$item['detail_url'];
                            $num=$item['quantity'];
                            $num_num = ORM::factory('wsd_setgood')->where('num_iid', '=', $num_iid)->count_all();
                            if($num_num==0 && $num_iid){
                                $sql = DB::query(Database::INSERT,"INSERT INTO `wsd_setgoods` (`bid`,`num_iid`,`title`,`price`, `pic`,`url`,`status`,`state`,`type`,`num`) VALUES ($bid,$num_iid,'$name' ,$price,'$pic','$url',0,1,$type,$num)");
                                $sql->execute();
                            }else{
                                $sql = DB::query(Database::UPDATE,"UPDATE `wsd_setgoods` SET `bid` = $bid ,`num_iid` = $num_iid,`title` ='$name',`price`=$price, `pic`='$pic',`url`='$url' ,`num` = $num ,`state` = 1 , `type` =$type where `num_iid` = $num_iid ");
                                $sql->execute();
                            }
                        }
                    }
                    $sql = DB::query(Database::DELETE,"DELETE FROM `wsd_goodskus` where `state` =0 and `bid` = $bid ");
                    $sql->execute();
                    $sql = DB::query(Database::UPDATE,"UPDATE `wsd_goodskus` SET `state` =0 where `bid` = $bid");
                    $sql->execute();
                    $sql = DB::query(Database::DELETE,"DELETE FROM `wsd_setgoods` where `state` =0 and `bid` = $bid ");
                    $sql->execute();
                    $sql = DB::query(Database::UPDATE,"UPDATE `wsd_setgoods` SET `state` =0 where `bid` = $bid");
                    $sql->execute();
                }
            }
        }
        Request::instance()->redirect('wsda/setgoods');
    }
    public function action_setgoods(){
        $bid = $this->bid;
        $config = ORM::factory('wsd_cfg')->getCfg($bid, 1);
        $goods = ORM::factory('wsd_setgood')->where('bid','=',$bid);
        ...
        if ($_POST['form']['num_iid']) {
            ...
            if($_POST['form']['smoney']){
                foreach ($_POST['form']['smoney'] as $k => $v) {
                    ...
                    // exit;
                    $sku = ORM::factory('wsd_goodsku')->where('id','=',$v['skuid'])->find();
                    if($sku->sku_id){

                    }else{
                        $sku->sku_id = 0;
                    }
                    $smoney = ORM::factory('wsd_smoney')->where('bid','=',$bid)->where('sid','=',$v['suiteid'])->where('sku_id','=',$sku->sku_id)->where('item_id','=',$_POST['form']['num_iid'])->find();
                    $smoney->bid = $bid;
            ...
            if($_POST['form']['type']==1){
                ...
                foreach ($sku as $k => $v) {
                    //echo $k."<br>";
                   $setsku=ORM::factory('wsd_goodsku')->where('id','=',$k)->find();
                   $setsku->money=$v;
                   $setsku->save();
                }
            }
            $good = ORM::factory('wsd_setgood')->where('bid', '=', $bid)->where('num_iid','=',$goodid)->find();
            if(isset($_POST['form']['status'])){
                $good->status=$_POST['form']['status'];
            }
            if(isset($_POST['form']['is_profit'])){
                $good->is_profit=$_POST['form']['is_profit'];
            }
            if($_POST['form']['type']!=1){
        ...
        $result['suite'] = ORM::factory('wsd_suite')->where('bid','=',$bid)->find_all();
      //   //require_once kohana::find_file('vendor',"kdt/KdtApiClient");
      //   require_once Kohana::find_file("vendor/kdt","YZTokenClient");
      //   $this->access_token = ORM::factory('wsd_login')->where('id','=',$bid)->find()->access_token;
      //   $tempconfig=ORM::factory('wsd_cfg')->getCfg($this->bid);
    ...
    $this->template->content=View::factory('weixin/wsd/admin/setgoods')
    ->bind('result',$result)
    ->bind('pages',$pages)
    ->bind('bid',$this->bid);
    ...
    public function action_customers(){
        $bid = $this->bid;
        $customers=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','!=',1)->where('fopenid','!=','');
        ...
        if ($_GET['export']=='xls') {
            ...
            foreach($customer1s as $k => $v){
                $allnum=ORM::factory('wsd_trade')->where('bid','=',$bid)->where('deletedd','=',0)->where('openid','=',$v->openid)->count_all();
                $allmoney=DB::query(Database::SELECT,"SELECT SUM(payment) as allmoney from wsd_trades where bid=$bid and deletedd = 0 and `openid` = '$v->openid' ")->execute()->as_array();
                    $allmoney=$allmoney[0]['allmoney'];
                $fname=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$v->fopenid)->where('lv','=',1)->find()->nickname;
                $num=$k+2;
                $objPHPExcel->setActiveSheetIndex(0)
                             //Excel的第A列，uid是你查出数组的键值，下面以此类推
                            ->setCellValue('A'.$num, $v->nickname)
        ...
        $this->template->content=View::factory('weixin//wsd/admin/customers')
            ->bind('pages',$pages)
            ->bind('result',$result)
            ->bind('bid',$this->bid);
    }
    public function action_calculate(){
        ...
        if($_POST['select']){
            ...
            foreach ($_POST['select'] as $key => $value) {
                ...
                $money=$qmt[2];
                $type=3;
                $qrcode_edit = ORM::factory('wsd_qrcode')->where('id', '=', $qid)->find();
                if ($money){
                    ORM::factory('wsd_score')->scoreOut($qrcode_edit, $type, $money,'','',$time);
                }
                // echo '<pre>';
                // var_dump($qmt);
                // exit();
        ...
        $qrcode = $qrcode->reset(FALSE);
         if ($_GET['export']=='xls') {
            ...
            foreach($qrcode1s as $k => $v){
                ...
                $nawtime=time();
                $monthtype='%Y-%m';
                $score=ORM::factory('wsd_score')->where('bid','=',$v->bid)->where('qid','=',$v->id)->where('bz','=',$month)->find();
                if($score->id){
                  $flag=1;
                }else{
                  $flag=0;
                ...
                //累计销售利润
                $fname=ORM::factory('wsd_qrcode')->where('bid','=',$v->bid)->where('openid','=',$v->fopenid)->where('lv','=',1)->find()->nickname;
                $score=ORM::factory('wsd_score')->where('bid','=',$v->bid)->where('qid','=',$v->id)->where('bz','=',$_GET['data']['begin'])->find();
                if($score->id){
                  $flag='已结算';
                }else{
                  $flag='未结算';
        ...
        $result['qrcodes'] = $qrcode->order_by('monthjs_pmoney', 'DESC')->limit($this->pagesize)->offset($offset)->find_all();
        $this->template->content=View::factory('weixin/wsd/admin/calculate')
            ->bind('pages',$pages)
            ->bind('month',$month)
            ->bind('config',$config)
            ->bind('result',$result)
    ...
    public function action_profit(){
        ...
        if($_POST['select']){
            foreach ($_POST['select'] as $key => $value) {
                ...
                $qrcode_edit = ORM::factory('wsd_qrcode')->where('id', '=', $qid)->find();
                if ($money){
                    ORM::factory('wsd_score')->scoreOut($qrcode_edit, $type, $money,'','','');
                }
                $month_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as month_pxmoney from wsd_scores where bid=$bid and qid = $qrcode_edit->id and score > 0  ")->execute()->as_array();
                $month_pxmoney=$month_pxmoney[0]['month_pxmoney'];
                $qrcode_edit->all_money=$month_pxmoney;
                $monthyjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthyjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode_edit->id and type IN (5,6) ")->execute()->as_array();
                $monthyjs_pxmoney=$monthyjs_pxmoney[0]['monthyjs_pxmoney'];
                $qrcode_edit->yjs_money=-$monthyjs_pxmoney;
                $monthdjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthdjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode_edit->id and paydate >= $nawtime and score > 0 ")->execute()->as_array();
                $monthdjs_pxmoney=$monthdjs_pxmoney[0]['monthdjs_pxmoney'];
                $qrcode_edit->djs_money=$monthdjs_pxmoney;
                $monthjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode_edit->id and type in (1,5,6) and paydate < $nawtime   ")->execute()->as_array();
                $monthjs_pxmoney=$monthjs_pxmoney[0]['monthjs_pxmoney'];
                $qrcode_edit->kjs_money=$monthjs_pxmoney;
                $qrcode_edit->save();
            }
        }
        //修改用户
        ...
        if($_GET['refresh']==1){
           $qrcodes1= ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('lv','=',1)->find_all();
            foreach ($qrcodes1 as $key => $qrcode1) {
                $month_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as month_pxmoney from wsd_scores where bid=$bid and qid = $qrcode1->id and score > 0  ")->execute()->as_array();
                $month_pxmoney=$month_pxmoney[0]['month_pxmoney'];
                $qrcode1->all_money=$month_pxmoney;
                $monthyjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthyjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode1->id and type IN (5,6) ")->execute()->as_array();
                $monthyjs_pxmoney=$monthyjs_pxmoney[0]['monthyjs_pxmoney'];
                $qrcode1->yjs_money=-$monthyjs_pxmoney;
                $monthdjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthdjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode1->id and paydate >= $nawtime and score > 0 ")->execute()->as_array();
                $monthdjs_pxmoney=$monthdjs_pxmoney[0]['monthdjs_pxmoney'];
                $qrcode1->djs_money=$monthdjs_pxmoney;
                $monthjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthjs_pxmoney from wsd_scores where bid=$bid and qid = $qrcode1->id and type in (1,5,6) and paydate < $nawtime   ")->execute()->as_array();
                $monthjs_pxmoney=$monthjs_pxmoney[0]['monthjs_pxmoney'];
                $qrcode1->kjs_money=$monthjs_pxmoney;
                $qrcode1->save();
            }
            Request::instance()->redirect('/wsda/profit');
        ...
        $qrcode = $qrcode->reset(FALSE);
         if ($_GET['export']=='xls') {
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                        ...
                        ->setCellValue('F'.$num, '已结算销售利润')
                        ->setCellValue('G'.$num, '待结算销售利润')
                        ->setCellValue('H'.$num, '目前可结算销售利润')
            ...
            foreach($qrcode1s as $k => $v){
                ...
                // $month_pxmoney=$month_pxmoney[0]['month_pxmoney'];
                // $monthjs_pxmoney=DB::query(Database::SELECT,"SELECT SUM(score) as monthjs_pxmoney from wsd_scores where bid=$v->bid and  type in (1,5,6) and qid = $v->id and paydate < $nawtime ")->execute()->as_array();
                // $monthjs_pxmoney=$monthjs_pxmoney[0]['monthjs_pxmoney'];
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                            ...
                            ->setCellValue('C'.$num, $v->tel)
                            ->setCellValue('D'.$num, $v->alipay_name)
                            ->setCellValue('E'.$num, number_format($v->all_money,2))
                            ->setCellValue('F'.$num, number_format($v->yjs_money,2))
                            ->setCellValue('G'.$num, number_format($v->djs_money,2))
                            ->setCellValue('H'.$num, number_format($v->kjs_money,2))
        ...
        $result['qrcodes'] = $qrcode->order_by('kjs_money', 'DESC')->limit($this->pagesize)->offset($offset)->find_all();
        $this->template->content=View::factory('weixin/wsd/admin/profit')
            ->bind('pages',$pages)
            ->bind('month',$month)
            ->bind('config',$config)
            ->bind('result',$result)
    ...
    public function action_delete($id){
        $score = ORM::factory('wsd_score')->where('id','=',$id)->find();
        $qid = $score->qid;
        $score->delete();
        Request::instance()->redirect('wsda/history_scores?qid='.$qid);
    }
    public function action_history_scores(){
        ...
        $type[2]=5;
        $type[3]=6;
        $scores = ORM::factory('wsd_score')->where('bid', '=', $bid)->where('type','IN',$type);
        if($_GET['qid']){
        ...
        if($_GET['s']['type']){
            ...
            if($_GET['s']['text']){
                $s = '%'.trim($_GET['s']['text'].'%');
                $user = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where_open()->where('nickname', 'like', $s)->or_where('alipay_name', 'like', $s)->or_where('tel', 'like', $s)->where_close()->find_all();
                $user_arr[0] = 0;//qid
                foreach ($user as $k => $v) {
                    $k++;
                    $user_arr[$k] = $v->id;//qid
        ...
        $this->template->content = View::factory('weixin/wsd/admin/history_scores')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config);
    ...
    private function sendsuccess($openid, $nickname,  $remark='恭喜您成功获得资格，赶紧点击菜单【生成海报】吧') {
        ...
        //Kohana::$log->add("weixin_wsd:$bid:tplmsg", print_r($openid, true));
         //Kohana::$log->add("weixin_wsd:$bid:tplmsg", print_r($tplmsg, true));
        $result = $this->we->sendTemplateMessage($tplmsg);
        Kohana::$log->add("weixin_wsd:tpl", print_r($result, true));
        return $result;
    }
    private function sendMoney1($userobj, $money,$time) {
        ...
        // Kohana::$log->add('weixin_wsd:hongbaopost', print_r($data, true));
        ...
        // Kohana::$log->add('weixin_wsd:hongbaoresult', print_r($result, true));
        return $result;
    }
    ...
    private function curl_post_ssl($url, $vars, $second=30, $aHeader=array(), $bid=0) {
        ...
        //证书分布式异步更新
        $file_cert = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'wsd_file_cert')->find();
        $file_key = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'wsd_file_key')->find();
        ...
        // Kohana::$log->add("weixin_wsd:$bid:curl_post_ssl:cert_file", $cert_file);
