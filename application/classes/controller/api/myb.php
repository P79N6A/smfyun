...
Class Controller_Api_myb extends Controller_API {

    var $token = 'meiyebao';

    var $FromUserName;
    var $Keyword;
    var $appId = 'wx5e00a61d465c412e';
    var $appSecret = '8c8f2b05fa512f3b5b561641eada14f9';
    var $encodingAesKey = 'aCR3CJKZszCBi8DELhIPmJzjA6MFh8lqU5zOWdShQXQ';
    var $baseurl = 'http://jfb.smfyun.com/myb/';
    var $cdnurl = 'http://cdn.jfb.smfyun.com/myb/';
    var $cdnurl2 = 'http://jfb.dev.smfyun.com/myb/';
    var $scorename;
    ...
    public function action_get($appid='')
    {
        ...
        if (!is_numeric($appid)) $bid = ORM::factory('myb_login')->where('user', '=', $appid)->find()->id;
    ...
    {
        $this->baseurl = 'http://'. $_SERVER['HTTP_HOST'] .'/myb/';
        Kohana::$log->add('myb','aaaaaaaaaa');
        $postStr = file_get_contents("php://input");
        Kohana::$log->add('myb', print_r($postStr, true));
        // Kohana::$log->add('$myb', print_r($appid, true));
        set_time_limit(15);
        Database::$default = "wdy";
        ...
        //username->bid
        //if (!is_numeric($bid))
        $biz = ORM::factory('myb_login')->where('user', '=', $appid)->find();
        $bid = $biz->id;
        $debug_bid = $bid;
        ...
        if(!$bid) {
            Kohana::$log->add('mybbid:', 'api'.$appid);//写入日志，可以删除
        }
        $we = new wechat($config);

        // Kohana::$log->add("myb", print_r($options,true));
        $we->getRev();
        ...
        $sname = ORM::factory('myb_cfg')->where('bid','=',$bid)->where('key','=','scorename')->find()->value;
        if($sname){
        ...
        if ($userinfo == false) {
            ...
            if ($we->errCode != 45009) {
                $mem = Cache::instance('memcache');
                $cachename1 ='myb.access_token'.$bid;
        ...
        $model_q = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        //扫码事件 || 扫码关注
        $scene_id = $bid;
         Kohana::$log->add("Event", print_r($Event, true));
         Kohana::$log->add("EventKey", print_r($EventKey, true));
         Kohana::$log->add("scene_id", print_r($scene_id, true));
        Kohana::$log->add("scene_id2", print_r($we->getRevSceneId(), true));
        if ($userinfo && ($Event == 'SCAN' && $EventKey == $scene_id) || ($we->getRevSceneId() == $scene_id)) {
            ...
            //珂心如意
            $fuser = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();//上一级
            if($fuser->fopenid){
            ...
            if ($model_q->id && !$model_q->fopenid && $fopenid && $fopenid != $openid && $ffopenid!=$openid&&$model_q->lock!=1&&$fuser->lock!=1) {
                ...
                //首次关注积分
                if (ORM::factory('myb_shscore')->where('qid', '=', $model_q->id)->where('type', '=', 1)->count_all() == 0) {
                    if ($config['goal0'] > 0) {
                ...
                if ($model_q->id > $fuser->id) {
                    ...
                    if ($config['risk_level1'] > 0 && $config['risk_level2'] > 0) {
                        //直接用户
                        $count2 = ORM::factory('myb_qrcode', $fuser->id)->scores->where('type', '=', 2)->count_all();
                        //用是否生成海报判断真实下线
                        $count3 = ORM::factory('myb_qrcode')->where('bid', '=', $fuser->bid)->where('fopenid', '=', $fuser->openid)->where('ticket', '<>', '')->count_all();
                        // //解除锁定
                    ...
                    //上一级推荐人 积分增加处理
                    //珂心如意
                    $ffuser = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenidFromQrcode)->find();

                    if ($ffuser->fopenid) {
                        //返利给 $ffuser 的上线 $ffuser->fopenid;
                        $fffuser = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $ffuser->fopenid)->find();//上上级
                        if ($fffuser && $config['goal2'] > 0&&$fffuser->lock!=1) {


                            $fffuser = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $ffuser->fopenid)->find();//上上级
                            $goal2_result = ORM::factory('myb_shscore')->scoreIn($fffuser, 3, $config['goal2'], $model_q->id);
                        }
            ...
            //已经有上级就直接取来
                ...
                //$fuser = ORM::factory('myb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $model_q->fopenid)->find();
            ...
            //自定义回复文本消息
            //读取数据库： $replySet = ORM::factory('myb_cfg')->where('bid', '=', $bid)->where('key', '=', 'text_goal3')->find()->value;
            ...
            if ($model_q->id && $model_q->fopenid && $fopenid){
                $fnickname = ORM::factory('myb_qrcode')->where('bid','=',$bid)->where('openid','=',$model_q->fopenid)->find()->nickname;
        ...
        Kohana::$log->add('chunv2', print_r($chunv2,true));
        //菜单点击事件
        if(isset($_GET['bname'])){
            $Event = 'CLICK';
        ...
        if ($userinfo && $Event == 'CLICK' || $chunv2) {
         ...
         if(($position >0 && $status=='1')||$status=='0'||!$status){
            ob_flush();
            $isvalue = ORM::factory('myb_cfg')->where('bid', '=', $bid)->where('key','=','value_'.substr($EventKey,-2))->find()->value;
            ...
            //生成海报
                ...
                //图片合成
                //模板
                $imgtpl = DOCROOT."myb/tmp/tpl.$bid.jpg";
                $tmpdir = '/dev/shm/';
                ...
                //默认头像
                $tplhead = ORM::factory('myb_cfg')->where('bid', '=', $bid)->where('key', '=', 'tplhead')->find();
                $default_head_file = DOCROOT."myb/tmp/head.$bid.jpg";
                ...
                //有海报缓存直接发送
                $tpl_key = 'myb:tpl:'.$openid.':'.$tpl->lastupdate;
                ...
                if ($uploadresult['media_id'] && !$newticket) {
                    ...
                    if (!$remote_head && $default_head_file) {
                        ...
                        Kohana::$log->add("myb:$bid:head4:", $remote_head);
                    }
                ...
                //海报发送前提醒消息
                $txtReply2 = $msg['text']['content'] = '海报有效期到 '. date('Y-m-d H:i', $model_q->lastupdate+$ticket_lifetime) .' 过期后请点击「生成海报」按钮重新获取哦！';
            ...
            //积分明细
                ...
                if (!file_exists(DOCROOT."myb/$news_pic_file")) $news_pic_file = 'news_score.png';
            ...
            /*//积分明细
                ...
                // if (!file_exists(DOCROOT."myb/$news_pic_file")) $news_pic_file = 'imgtpl/'.'$bid'.'/score1.jpg';
            ...
            //兑换商城
                ...
                if (!file_exists(DOCROOT."myb/$news_pic_file")) $news_pic_file = 'news_order.jpg';
                $news_pic = $this->cdnurl.$news_pic_file;
            ...
            //排行榜
                ...
                if (!file_exists(DOCROOT."myb/$news_pic_file")) $news_pic_file = 'news_top.png';
                $news_pic = $this->cdnurl.$news_pic_file;
                ...
                //用户少的时候才回复 debug
                // if (ORM::factory('myb_qrcode')->where('bid', '=', $bid)->count_all() < 10)
                //$we_result = $we->sendCustomMessage($msg);
            }
            ...
            //$msg['text']['content'] = '符合要求'.$u_location.$p_location;
                ...
                $replyfront = ORM::factory('myb_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyfront')->find()->value;
                $replyend = ORM::factory('myb_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyend')->find()->value;
        ...
        if ($txtReply) {
            ...
            // Kohana::$log->add('$mybtext',print_r($txtresult,true));

            $result = $we->text($txtReply)->reply(array(),true);
            // ob_clean();
            echo $result;
            // Kohana::$log->add('$myb',print_r($result,true));
            // echo $result;
        }
