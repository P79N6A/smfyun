...
Class Controller_Api_Wdy extends Controller_API {
    ...
    public function action_post($bname=1, $debug=0)
    {
        ...
        if ($biz->expiretime && (strtotime($biz->expiretime)+86400) < time()) {
            $die = 1;
            $txtReply = '您的积分宝服务号有赞版插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
        }
        ...
        if($die == 1){
            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = '您的积分宝服务号有赞版插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
                   ...
                   // Kohana::$log->add("weixin2:aaaa",print_r($qg,true));
                    if(!$qg){
                        ...
                        if($userobj0->score>=$config['goal0']){
                            ...
                            if($config['switch']==1){
                                $this->rsync($bid,$userobj0->openid,$access_token,-$config['goal0'],$sid,'取消关注扣积分1');
                            }
                        }
                        ...
                        if($userobj1->score>=$config['goal']){
                           $sid=$userobj1->scores->scoreOut($userobj1, 10,$config['goal']);
                            if($config['switch']==1){
                                $this->rsync($bid,$userobj1->openid,$access_token,-$config['goal'],$sid,'取消关注扣积分2');
                            } 
                        }
                        ...
                        if($userobj3){
                            ...
                            if($userobj2->score>=$config['goal2']){
                                ...
                                if($config['switch']==1){
                                    $this->rsync($bid,$userobj2->openid,$access_token,-$config['goal2'],$sid,'取消关注扣积分3');
                                }
                            }
                        }
                    }
                ...
                if ($chuserinfo['subscribe'] === 0) {
                    ...
                    if(!$qg){
                        ...
                        if($userobj0->score>=$config['goal0']){
                           $sid=$userobj0->scores->scoreOut($userobj0, 9,$config['goal0']);
                            if($config['switch']==1){
                                $this->rsync($bid,$userobj0->openid,$access_token,-$config['goal0'],$sid,'取消关注扣积分4');
                            } 
                        }
                        $userobj1 = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
                        if($userobj1->score>=$config['goal2']){
                            $sid=$userobj1->scores->scoreOut($userobj1, 10,$config['goal2']);
                            if($config['switch']==1){
                                $this->rsync($bid,$userobj1->openid,$access_token,-$config['goal2'],$sid,'取消关注扣积分5');
                            }
                        }
                        $fopenid1 =ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $b->openid)->find()->fopenid;
                        $userobj2 = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenid1)->find();
                        if($userobj2->score>=$config['goal']){
                           $sid=$userobj2->scores->scoreOut($userobj2, 10,$config['goal']);
                            if($config['switch']==1){
                                $this->rsync($bid,$userobj2->openid,$access_token,-$config['goal'],$sid,'取消关注扣积分6');
                            } 
                        } 
                    }
                }
        ...
        if($Event=='subscribe'){
            Kohana::$log->add("wdy{$bid}subscribe",print_r($openid, true));
            Kohana::$log->add("wdy{$bid}subscribe",print_r(time(), true));
            $subscribe=ORM::factory('wdy_subscribe')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            $subscribe->bid=$bid;
        ...
        if ($userinfo && ($Event == 'SCAN' && $EventKey == $scene_id) || ($we->getRevSceneId() == $scene_id)) {
            ...
            if ($model_flag==1&&$model_q->id && !$model_q->fopenid && $fopenid && $fopenid != $openid && $ffopenid!=$openid&&$model_q->lock!=1&&$fuser->lock!=1) {
                ...
                if($subscribe->id&&$subscribe->creattime<=time()-60){
                    Kohana::$log->add("wdy{$bid}SCAN111",print_r($subscribe->openid, true));
                    $has_subscribe=1;
                ...
                Kohana::$log->add("wdy{$bid}SCAN",print_r(time(), true));
                //if()
                //首次关注积分
                if (ORM::factory('wdy_score')->where('qid', '=', $model_q->id)->where('type', '=', 1)->count_all() == 0&&$has_subscribe==2) {
                    if ($config['goal0'] > 0) {
                        $model_q = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
                        $sid=$model_q->scores->scoreIn($model_q, 1, $config['goal0']);
                        if($config['switch']==1){
                            $this->rsync($bid,$model_q->openid,$access_token,$config['goal0'],$sid,'首次关注加积分');
                        }
                    }
                }
                ...
                if ($model_q->id > $fuser->id&&$has_subscribe==2) {
                    ...
                    if ($config['goal'] > 0&&$fuser->lock!=1) {
                        ...
                        if($config['switch']==1){
                            $this->rsync($bid,$fuser->openid,$access_token,$config['goal'],$goal_result,'上级加积分');
                        }
                    }
                     $fuser = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();//上一级
                    //积分话术
                    ...
                    if ($ffuser->fopenid) {
                        ...
                        if ($fffuser && $config['goal2'] > 0&&$fffuser->lock!=1) {
                            ...
                            if($config['switch']==1){
                                $this->rsync($bid,$fffuser->openid,$access_token,$config['goal2'],$goal2_result,'上上级加积分');
                            }
                        }
                        $fffuser = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $ffuser->fopenid)->find();//上上级
                        ...
                        $content=sprintf($tpl, $fuser->nickname);
                        // $a=strpos($content,'<a');
                        // $b=strpos($content,'/a>');
                        // $c=substr($content,$a,$b-$a+3);
                        // $c =str_replace('"', '\"',$c);
            ...
            if ($model_flag==2){
                $msg['text']['content'] = "您已经参加过活动了，不用再扫了哦。";
                $msg2['text']['content'] = "您的朋友已经参加过活动了，不能再成为您的粉丝了";
                $we->sendCustomMessage($msg2);
            }
            if ($has_subscribe==1){
                $msg['text']['content'] = "您已经关注过公众号了，不用再扫了哦。";
                $msg2['text']['content'] = "您的朋友已经关注过公众号了，不能再成为您的粉丝了";
                $we->sendCustomMessage($msg2);
            }
            ...
            //     $we_result = $we->sendCustomMessage($msg);
            // }
            Kohana::$log->add("$bid:wdy:weixin_scan", print_r($msg,true));
            if($msg){
                $we_result = $we->sendCustomMessage($msg);
            }
            Kohana::$log->add("$bid:wdy:weixin_scan", print_r($we_result,true));
            //扫码后推送网址
            if ($config['text_follow_url']&&$fuser->lock!=1) {
                ...
                $we_result = $we->sendCustomMessage($msg);
                Kohana::$log->add("$bid:wdy:weixin_scan2", print_r($we_result));
            }
        }
        $model_q = ORM::factory('wdy_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        //微信用户上传地理位置信息
         ...
         if(($position >0 && $status=='1')||$status=='0'||!$status){
            ...
            //生成海报
                ...
                if ( ($result['ticket'] = $model_q->ticket) && (time() - $model_q->lastupdate < $ticket_lifetime) ) {
                    ...
                    if($bid==5&&$model_q->openid=='oDt2QjjkFaJbjoBKPO4zVpHj0Qgs'){
                        $model_q->headimgurl = '';
                    }else{
                        $model_q->values($userinfo);
                        $model_q->bid = $bid;
                ...
                if ($tplhead->pic) file_put_contents($default_head_file, $tplhead->pic);
                ...
                if ($uploadresult['media_id'] && !$newticket) {
                    ...
                    Kohana::$log->add("wdy:$bid:head1:", $remote_head);

                    if (!$remote_head) {
                        $remote_head_url = str_replace('/0', '/132', $model_q->headimgurl);
                        $remote_head = curls($remote_head_url);
                        Kohana::$log->add("wdy:$bid:head2:", $remote_head);
                    }
                    ...
                    if (!$remote_head) {
                        ...
                        $remote_head = curls($remote_head_url);
                        Kohana::$log->add("wdy:$bid:head3:", $remote_head);
                    }

                    //获取失败用默认头像
                    if (!$remote_head && $default_head_file) {
                        $remote_head = file_get_contents($default_head_file);
                        Kohana::$log->add("wdy:$bid:head4:", $remote_head);
                    }
                    //写入临时头像文件
                    ...
                    if (!$remote_head || !$remote_qrcode) {
                        ...
                        $we_result = $we->sendCustomMessage($msg);
                        Kohana::$log->add("wdy:$bid:file:remote_head_url get ERROR!", $remote_head_url);
        ...
        if ($newsReply) {
            ...
            Kohana::$log->add("weixin_wdy:$bid:openid图文回复: $openid");
            ob_flush();
            $newsresult = $we->news($newsReply)->reply(array(), true);
            Kohana::$log->add("weixin_wdy:$bid:openid图文回复: ",$newsresult);
            echo $newsresult;
        }
    ...
    private function rsync($bid,$openid,$access_token,$chscore,$sid,$reason){
        ...
        Kohana::$log->add("wdyscore111", print_r($reason, true));
        Kohana::$log->add("wdyscore1", print_r($chscore, true));
        Kohana::$log->add("wdyscore", print_r($openid, true));
        ...
        Kohana::$log->add("wdyscore", print_r($result, true));
        ...
        if($qrcode->yz_score==0){
            ...
            if($a['response']['is_success']=='true'||$qrcodescore0==1){
                ...
                $failscore->save();
                Kohana::$log->add('failscoreid11',print_r($failscore->id,true));
            }
            $qrcode=ORM::factory('wdy_qrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
        }else{
            // if($chscore>0){
            ...
            if($a['response']['is_success']!='true'){
                ...
                $failscore->log=$a['error_response']['code'].$a['error_response']['message'];
                $failscore->save();
                Kohana::$log->add('failscoreid1',print_r($failscore->id,true));
            }
        }
        // $method = 'youzan.crm.fans.points.get';
        // $params =[
