...
//全员分销 api 地址
Class Controller_Api_yty extends Controller_API {

    var $token = 'yuntianyou2017';

    var $FromUserName;
    var $Keyword;
    var $access_token;
    var $baseurl;
    var $cdnurl = 'http://cdn.jfb.smfyun.com/yty/';
    ...
    public function action_get($bid=1)
    {
        ...
        if (!is_numeric($bid)) $bid = ORM::factory('yty_login')->where('user', '=', $bid)->find()->id;
        $config = ORM::factory('yty_cfg')->getCfg($bid);

        if ($this->checkSignature() == true)
            die($_GET['echostr']);
        else
            die($config['name']."@云天佑 by 1nnovator");
    }
    ...
    {
        Database::$default = "wdy";
        $this->baseurl = 'http://'.$_SERVER["HTTP_HOST"].'/yty/';
        set_time_limit(15);
        ...
        $biz = ORM::factory('yty_login')->where('user', '=', $bname)->find();
        $bid = $biz->id;
        ...
        Kohana::$log->add('scene_id',print_r($config['scene_id_yty'],true));
        if (!$this->access_token || !$config['appid'] || !$config['appsecret'] ) die('Not Config!');
        Kohana::$log->add('333','33333');
        if ($bid == $debug_bid) Kohana::$log->add('weixin_yty:config', print_r($config, true));
        if ($biz->expiretime && strtotime($biz->expiretime) < time()) $txtReply = '您的云商系统插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
        ...
        if ($bid == $debug_bid) Kohana::$log->add("weixin_yty:$bid:getRevData", print_r($we->getRevData(), true));
        if(substr($we->checkAuth(), 0,5)=='error'){
            $txtReply = $we->checkAuth();
        }
        if (!$we->checkAuth()) {
            $txtReply = 'appid 和 appsecret 配置不正确，请检查';
            if ($bid == $debug_bid) Kohana::$log->add("weixin_yty:$bid:checkAuth", 'appid 和 appsecret 配置不正确，请检查');
        }
        ...
        if ($bid == $debug_bid) {
            Kohana::$log->add("weixin_yty:$bid:userinfo", var_export($userinfo, true));
        }

        if ($userinfo == false) {
            // die('UserInfo get error!');
            Kohana::$log->add("weixin_yty:$bid:WE", $we->errCode.':'.$we->errMsg);

            if ($we->errCode != 45009) {
                $key = "weixin_yty:$bid:resetAuth";
        ...
        //当前微信用户
        $model_q = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        ...
        // //扫码事件 || 扫码关注
        // $scene_id = $config['scene_id_yty'];

        // if ($userinfo && ($Event == 'SCAN' && $EventKey == $scene_id) || ($we->getRevSceneId() == $scene_id)) {

        //     $fuser = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();
        ...
        //                 if ($bid == $debug_bid) Kohana::$log->add("weixin_yty:$bid:FuserInfo", print_r($fuserinfo, true));
        ...
        //     //已经有上级就直接取来
        //     if($model_q->fopenid){
        //         $fuser = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('openid', '=', $model_q->fopenid)->find();
        //         $fopenid = $fuser->openid;
        //     }
        //     $fromuser = ORM::factory('yty_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();
        //     //扫码后默认推送消息
        //     $msg['touser'] = $openid;
        //     $msg['msgtype'] = 'text';
        ...
        //         $name = $config['title2'];
        //     }
        //     // $id = ORM::factory('yty_qrcode')->where('bid','=',$bid)->where('lv','=',1)->where('id','<=',$model_q->id)->find_all()->count();
        //     //自己关注 没上线
        ...
        if ($userinfo && $Event == 'CLICK' || $haibao==1) {
            ...
            //自定义 key
            if ($EventKey == $config['key_c1_yty'] && $config['value_c1_yty']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c1_yty']);
            }

            else if ($EventKey == $config['key_c2_yty'] && $config['value_c2_yty']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c2_yty']);
            }

            else if ($EventKey == $config['key_c3_yty'] && $config['value_c3_yty']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c3_yty']);
            }

            else if ($EventKey == $config['key_c4_yty'] && $config['value_c4_yty']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c4_yty']);
            }

            // //生成海报
            // else if ($EventKey == $config['key_ytyqrcode'] || $chunv || $EventKey == '生成海报' ||$haibao==1) {
            //     //申请审核？
            //     if ($model_q->lv==0) {
            ...
            //     //自定义过期时间
            //     if ($config['yty_ticket_lifetime']) $ticket_lifetime = 3600*24*$config['yty_ticket_lifetime'];

            //     $qrcode_type = 0;

            //     //if (!$model_q->lastupdate) $model_q->lastupdate = time();

            //     $config['text_ytysend'] = str_replace('{ID}', $model_q->id, $config['text_ytysend']);
            //     $config['text_ytysend'] = str_replace('\n', "\n", $config['text_ytysend']);

            //     // Kohana::$log->add('weixin_yty:tpl_lifetime', time() - $model_q->lastupdate);

            //     if ( ($result['ticket'] = $model_q->ticket) && (time() - $model_q->lastupdate < $ticket_lifetime) ) {

            //         //pass
            //         //海报过期时间文案
            //         $config['text_ytysend'] = str_replace('{TIME}', date('Y-m-d H:i', $model_q->lastupdate+$ticket_lifetime), $config['text_ytysend']);
            //         $msg['text']['content'] = $config['text_ytysend'];
            ...
            //         $result = $we->getQRCode($config['scene_id_yty'], $qrcode_type, $ticket_lifetime);
            ...
            //     //图片合成
            //     //模板
            //     $imgtpl = DOCROOT."yty/tmp/tpl.$bid.jpg";
            //     $tmpdir = '/dev/shm/';

            //     //判断模板文件是否需要从数据库更新
            //     $tpl = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'ytytpl')->find();
            //     if (!$tpl->pic) {
            ...
            //     //默认头像
            //     $tplhead = ORM::factory('yty_cfg')->where('bid', '=', $bid)->where('key', '=', 'ytytplhead')->find()->pic;
            ...
            //     //有海报缓存直接发送
            //     $tpl_key = 'yty:ytytpl:'.$openid.':'.$tpl->lastupdate;
            //     $uploadresult['media_id'] = $mem->get($tpl_key);

            //     if ($uploadresult['media_id'] && !$newticket) {
            //         //pass
            //         // Kohana::$log->add('weixin_yty:tpl_key', $tpl_key);
            //         // Kohana::$log->add('weixin_yty:media_id_cache', print_r($uploadresult, true));
            //     } else {
            ...
            //             $we_result = $we->sendCustomMessage($msg);
            //             Kohana::$log->add("weixin_yty:$bid:file:remote_head_url get ERROR!", $remote_head_url);
            ...
            //                 Kohana::$log->add("weixin_yty:$bid:$newfile upload ERROR!", $we->errCode.':'.$we->errMsg);
            //                 if ($we->errCode == 45009) {
            ...
            //             Kohana::$log->add("weixin_yty:$bid:newfile $newfile gen ERROR!");
            //             Kohana::$log->add("weixin_yty:$bid:imgtplfile:$imgtpl", file_exists($imgtpl));
            //             Kohana::$log->add("weixin_yty:$bid:imgtplfileinfo", print_r($tmp, true));

            //             Kohana::$log->add("weixin_yty:$bid:qrcodefile:$localfile", file_exists($localfile));
            //             Kohana::$log->add("weixin_yty:$bid:headfile:$headfile", file_exists($headfile));
            //         }
            ...
            //资产明细
            else if ($EventKey == $config['key_ytyscore'] || $EventKey == '资产查询') {
                if ($model_q->lv==0) {
                    $msg['text']['content'] = '请先申请成为经销商，完成申请后方可查看';
                    $we->sendCustomMessage($msg);
                    exit;
                }
                if ($model_q->lv==2) {
                    $msg['text']['content'] = "您的审核已经提交，请耐心等待审核通过。\n\n";
                    $msg['text']['content'] .= str_replace('\n', "\n", $config['yty_desc']);
                    $we_result = $we->sendCustomMessage($msg);
                    exit;
                }
                if ($model_q->lv==3) {
                    $msg['text']['content'] = "您已经被取消了分销商资格或审核未通过，请联系管理员。 \n\n";
                    $msg['text']['content'] .= str_replace('\n', "\n", $config['yty_desc']);
                    $we_result = $we->sendCustomMessage($msg);
                    exit;
                }
                $url = $this->baseurl.'index/'. $bid .'?url=home&cksum='. $cksum .'&openid='. base64_encode($model_q->openid);
                $msg['msgtype'] = 'news';
                $news_pic_file = 'score1.jpg';
                $news_pic = $this->cdnurl.$news_pic_file;
                $newsReply[0]['Title'] = $msg['news']['articles'][0]['title'] = '个人中心';
                ...
                $msg['msgtype'] = 'text';
                $txtReply = $msg['text']['content'] = '请将下面的 KEY 填写到「云商系统」商户后台完成配置：'.$EventKey;
            }
            //检查 Auth 是否过期
            if ($we_result === false) {
                if ($bid == $debug_bid) Kohana::$log->add("weixin_yty:$bid:we_result", print_r($we_result, true));
                //$we->resetAuth();
            }
        }
        //默认文字回复
