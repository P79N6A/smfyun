...
Class Controller_Api_Dyb extends Controller_API {

    var $token = 'dingyuebao';

    var $FromUserName;
    var $Keyword;

    var $baseurl = 'http://dyb.smfyun.com/dyb/';
    var $cdnurl = 'http://cdn.jfb.smfyun.com/dyb/';
    ...
    public function action_get($bid=1)
    {
        ...
        if (!is_numeric($bid)) $bid = ORM::factory('dyb_login')->where('user', '=', $bid)->find()->id;
    ...
    {
        $this->baseurl = 'http://'. $_SERVER['HTTP_HOST'] .'/dyb/';
        ...
        //username->bid
        //if (!is_numeric($bid))
        $biz = ORM::factory('dyb_login')->where('user', '=', $bname)->find();
        $bid = $biz->id;
        $config = ORM::factory('dyb_cfg')->getCfg($bid);
        ...
        if ($biz->expiretime && (strtotime($biz->expiretime)+86400) < time()) {
            $die =1;
            $txtReply = '您的积分宝订阅号有赞版插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
        }
        ...
        if($die == 1){
            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = '您的积分宝订阅号有赞版插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
            $we->sendCustomMessage($msg);
            exit;
        }
        //Kohana::$log->add("openid:$bid:fromusername", print_r($fromUsername, true));
        ...
        $model_q = ORM::factory('dyb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        ...
        if ($userinfo && $Event == 'CLICK' || $chunv2) {
            ...
            // }
            $status = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'status')->find()->value;
            $userid = ORM::factory('dyb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->id;
            // if(!$userid){
            //     $msg['touser'] = $openid;
            //     $msg['msgtype'] = 'text';
            ...
            if($userid&&$status==1){//用户存在 并且地理位置打开了
                ...
                $position = 0;
                $u_location = ORM::factory('dyb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->area;
                for ($i=1; $i <=$count ; $i++) {
                    $pro[$i] = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'pro'.$i)->find()->value;
                    $city[$i] = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'city'.$i)->find()->value;
                    $dis[$i] = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'dis'.$i)->find()->value;
         ...
         if(($position >0 && $status=='1')||$status=='0'||!$status||!$userid){
            ...
            //生成海报
                ...
                if ( ($result['ticket'] = $model_q->ticket) &&  (time()<explode('|',$model_q->ticket)[2]) ) {
                    ...
                    // $model_q->save();
                    $fuopenid = ORM::factory('dyb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->fuopenid;
                ...
                //图片合成
                //模板
                $imgtpl = DOCROOT."dyb/tmp/tpl.$bid.jpg";
                $tmpdir = '/dev/shm/';
                ...
                if ($tplhead->pic && !file_exists($default_head_file)) file_put_contents($default_head_file, $tplhead->pic);
                    ...
                    $qrurl =  'http://'.$_SERVER["HTTP_HOST"].'/dyb/qrscan/'.$result['ticket'];
                    ...
                    if (!$remote_head && $default_head_file) {
                        ...
                        Kohana::$log->add("dyb:$bid:head4:", $remote_head);
                    }
            ...
            //积分明细
                ...
                if (!file_exists(DOCROOT."dyb/$news_pic_file")) $news_pic_file = 'news_score.png';
            ...
            //兑换商城
                ...
                if (!file_exists(DOCROOT."dyb/$news_pic_file")) $news_pic_file = 'news_order.jpg';
                $news_pic = $this->cdnurl.$news_pic_file;
            ...
            //排行榜
                ...
                if (!file_exists(DOCROOT."dyb/$news_pic_file")) $news_pic_file = 'news_top.png';
                $news_pic = $this->cdnurl.$news_pic_file;
                ...
                $msg['msgtype'] = 'text';
                $txtReply = $msg['text']['content'] = '请将下面的 KEY 填写到「订阅宝」商户后台完成配置：'.$EventKey;

                //用户少的时候才回复 debug
                // if (ORM::factory('dyb_qrcode')->where('bid', '=', $bid)->count_all() < 10)
                //$we_result = $we->sendCustomMessage($msg);
            }
            ...
            //$msg['text']['content'] = '符合要求'.$u_location.$p_location;
                ...
                $replyfront = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyfront')->find()->value;
                $replyend = ORM::factory('dyb_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyend')->find()->value;
