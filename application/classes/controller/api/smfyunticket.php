...
Class Controller_Api_smfyunticket extends Controller_API {
    //微信开放平台：神码浮云营销系统
    var $token = 'smfyun';
    var $encodingAesKey = 'aCR3CJKZszCBi8DELhIPmJzjA6MFh8lqU5zOWdShQXQ';
    var $appId = 'wx4d981fffa8e917e7';
    var $appserect = '49950b496b4dcccd3fa4ac67ad74ddaf';
    ...
    public function action_get($bid=1)
    {
        ...
        if (!is_numeric($bid)) $bid = ORM::factory('qwt_login')->where('user', '=', $bid)->find()->id;
        $config = ORM::factory('qwt_cfg')->getCfg($bid);

        if ($this->checkSignature() == true)
            die($_GET['echostr']);
        else
            die($config['name']."全网通 by 1nnovator");
    }
    ...
    public function action_post()
    {
        ...
        Kohana::$log->add('smfyunticket:postdata', print_r($encryptMsg, true));
        $timeStamp = $_GET["timestamp"];
        $nonce = $_GET["nonce"];
        require_once Kohana::find_file('vendor', 'oauth/wxBizMsgCrypt');
        ...
        if ($errCode == 0) {
            ...
            if(!$ctoken||(time()+1800>=$ctime)){// 不存在 或在过期范围内
                ...
                if(!$component_access_token){
                    Kohana::$log->add('error_component_access_token:', print_r($res, true));
                }else{
