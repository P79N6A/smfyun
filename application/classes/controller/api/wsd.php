...
//全员分销 api 地址
Class Controller_Api_wsd extends Controller_API {

    var $token = 'weishang2017';

    var $FromUserName;
    var $Keyword;
    var $access_token;
    var $baseurl;
    var $cdnurl = 'http://cdn.jfb.smfyun.com/wsd/';
    ...
    public function action_get($bid=1)
    {
        ...
        if (!is_numeric($bid)) $bid = ORM::factory('wsd_login')->where('user', '=', $bid)->find()->id;
        $config = ORM::factory('wsd_cfg')->getCfg($bid);

        if ($this->checkSignature() == true)
            die($_GET['echostr']);
        else
            die($config['name']."@微商代理 by 1nnovator");
    }
    ...
    {
        Database::$default = "wdy";
        $this->baseurl = 'http://'.$_SERVER["HTTP_HOST"].'/wsd/';
        set_time_limit(15);
        ...
        $biz = ORM::factory('wsd_login')->where('user', '=', $bname)->find();
        $bid = $biz->id;
        ...
        Kohana::$log->add('scene_id',print_r($config['scene_id_wsd'],true));
        if (!$this->access_token || !$config['appid'] || !$config['appsecret'] || !$config['scene_id_wsd']) die('Not Config!');
        Kohana::$log->add('333','33333');
        if ($bid == $debug_bid) Kohana::$log->add('weixin_wsd:config', print_r($config, true));
        if ($biz->expiretime && (strtotime($biz->expiretime)+86400) < time()) {
            $txtReply = '您的微商代理插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
            $die =1;
        }
        ...
        if ($bid == $debug_bid) Kohana::$log->add("weixin_wsd:$bid:getRevData", print_r($we->getRevData(), true));
        if(substr($we->checkAuth(), 0,5)=='error'){
            $txtReply = $we->checkAuth();
        }
        if (!$we->checkAuth()) {
            $txtReply = 'appid 和 appsecret 配置不正确，请检查';
            if ($bid == $debug_bid) Kohana::$log->add("weixin_wsd:$bid:checkAuth", 'appid 和 appsecret 配置不正确，请检查');
        }

        $fromUsername = $we->getRevFrom();
        $toUsername = $we->getRevTo();
        $this->Keyword = $we->getRevContent();
        Kohana::$log->add('wsdfromUsername:', $fromUsername);
        Kohana::$log->add('wsdtoUsername:', $toUsername);
        Kohana::$log->add('wsdkeyword:', $this->Keyword);
        $openid = $this->FromUserName = $fromUsername;
        $userinfo = $we->getUserInfo($openid);
        if($die == 1){
            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = '您的微商代理插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
            $we->sendCustomMessage($msg);
            exit;
        }
        if ($bid == $debug_bid) {
            Kohana::$log->add("weixin_wsd:$bid:userinfo", var_export($userinfo, true));
        }

        if ($userinfo == false) {
            // die('UserInfo get error!');
            Kohana::$log->add("weixin_wsd:$bid:WE", $we->errCode.':'.$we->errMsg);

            if ($we->errCode != 45009) {
                $key = "weixin_wsd:$bid:resetAuth";
        ...
        Kohana::$log->add('wsdevent:', print_r($we->getRevEvent(),true));
        // Kohana::$log->add('eventkey:', $this->Keyword);
        // Kohana::$log->add('ticket:', $this->Keyword);
        //当前微信用户
        $model_q = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        //取消关注 改变关注状态
        $childs = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('fopenid','=',$openid)->find_all();
        ...
        //扫码事件 || 扫码关注
        $scene_id = $config['scene_id_wsd'];

        if ($userinfo && ($Event == 'SCAN' && $EventKey == $scene_id) || ($we->getRevSceneId() == $scene_id)) {

            $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();
            ...
            if ($model_q->id && !$model_q->fopenid && !$model_q->ticket && $fopenid && $fopenid != $openid&&$fuser->lv==1&&$model_q->lv==0) {
                ...
                if ($model_q->id > $fuser->id) {
                    ...
                    if ($fuserinfo['subscribe'] == 0) {
                        if ($bid == $debug_bid) Kohana::$log->add("weixin_wsd:$bid:FuserInfo", print_r($fuserinfo, true));
            ...
            //已经有上级就直接取来
            if($model_q->fopenid){
                $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $model_q->fopenid)->find();
                $fopenid = $fuser->openid;
            }
            $fromuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();
            //扫码后默认推送消息
            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';
            ...
            // $id = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','=',1)->where('id','<=',$model_q->id)->find_all()->count();
            //自己关注 没上线
        ...
        //菜单点击事件
        Kohana::$log->add('wsdkeyword:', print_r($this->Keyword,true));
        if(strpos($this->Keyword,$config['keyword'])!==false){

            $haibao = 1;
        }
        //Kohana::$log->add('wsdhaibao:', print_r($haibao,true));
        if ($userinfo && $Event == 'CLICK' || $haibao==1) {
            Kohana::$log->add('wsdhaibao:', 'aaa');
            Kohana::$log->add('wsdbbb:', $EventKey);
            Kohana::$log->add('wsdccc:', $config['key_wsdqrcode']);

            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';

            //自定义 key
            if ($EventKey == $config['key_c1_wsd'] && $config['value_c1_wsd']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c1_wsd']);
            }

            else if ($EventKey == $config['key_c2_wsd'] && $config['value_c2_wsd']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c2_wsd']);
            }

            else if ($EventKey == $config['key_c3_wsd'] && $config['value_c3_wsd']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c3_wsd']);
            }

            else if ($EventKey == $config['key_c4_wsd'] && $config['value_c4_wsd']) {
                $txtReply = $msg['text']['content'] = str_replace('\n', "\n", $config['value_c4_wsd']);
            }
            ...
            // Kohana::$log->add('wsdbbb:', $EventKey);
            // Kohana::$log->add('wsdccc:', $config['key_wsdqrcode']);
            else if ($EventKey == $config['key_wsdqrcode'] || $chunv || $EventKey == '生成海报' ||$haibao==1) {
                //申请审核？
                Kohana::$log->add('wsdhaibao:', 'bbb');
                if ($model_q->lv==0) {
                ...
                //自定义过期时间
                if ($config['wsd_ticket_lifetime']) $ticket_lifetime = 3600*24*$config['wsd_ticket_lifetime'];

                $qrcode_type = 0;

                //if (!$model_q->lastupdate) $model_q->lastupdate = time();

                $config['text_wsdsend'] = str_replace('{ID}', $model_q->id, $config['text_wsdsend']);
                $config['text_wsdsend'] = str_replace('\n', "\n", $config['text_wsdsend']);

                // Kohana::$log->add('weixin_wsd:tpl_lifetime', time() - $model_q->lastupdate);

                if ( ($result['ticket'] = $model_q->ticket) && (time() - $model_q->lastupdate < $ticket_lifetime) ) {

                    //pass
                    //海报过期时间文案
                    $config['text_wsdsend'] = str_replace('{TIME}', date('Y-m-d H:i', $model_q->lastupdate+$ticket_lifetime), $config['text_wsdsend']);
                    $msg['text']['content'] = $config['text_wsdsend'];
                    ...
                    $result = $we->getQRCode($config['scene_id_wsd'], $qrcode_type, $ticket_lifetime);
                ...
                //图片合成
                //模板
                $imgtpl = DOCROOT."wsd/tmp/tpl.$bid.jpg";
                $tmpdir = '/dev/shm/';

                //判断模板文件是否需要从数据库更新
                $tpl = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'wsdtpl')->find();
                if (!$tpl->pic) {
                ...
                //默认头像
                $tplhead = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'wsdtplhead')->find()->pic;
                $default_head_file = DOCROOT."wsd/tmp/head.$bid.jpg";
                ...
                //有海报缓存直接发送
                $tpl_key = 'wsd:wsdtpl:'.$openid.':'.$tpl->lastupdate;
                $uploadresult['media_id'] = $mem->get($tpl_key);

                if ($uploadresult['media_id'] && !$newticket) {
                    //pass
                    // Kohana::$log->add('weixin_wsd:tpl_key', $tpl_key);
                    // Kohana::$log->add('weixin_wsd:media_id_cache', print_r($uploadresult, true));
                } else {
                    ...
                    if (!$remote_head || !$remote_qrcode) {
                        ...
                        $we_result = $we->sendCustomMessage($msg);
                        Kohana::$log->add("weixin_wsd:$bid:file:remote_head_url get ERROR!", $remote_head_url);
                    ...
                    if (file_exists($newfile)) {
                        ...
                        if (!$uploadresult['media_id']) {
                            Kohana::$log->add("weixin_wsd:$bid:$newfile upload ERROR!", $we->errCode.':'.$we->errMsg);
                            if ($we->errCode == 45009) {
                        ...
                        Kohana::$log->add("weixin_wsd:$bid:newfile $newfile gen ERROR!");
                        Kohana::$log->add("weixin_wsd:$bid:imgtplfile:$imgtpl", file_exists($imgtpl));
                        Kohana::$log->add("weixin_wsd:$bid:imgtplfileinfo", print_r($tmp, true));

                        Kohana::$log->add("weixin_wsd:$bid:qrcodefile:$localfile", file_exists($localfile));
                        Kohana::$log->add("weixin_wsd:$bid:headfile:$headfile", file_exists($headfile));
                    }
            ...
            //资产明细
            else if ($EventKey == $config['key_wsdscore'] || $EventKey == '资产查询') {
                if ($model_q->lv==0) {
                ...
                Kohana::$log->add('wsdddd:', $txtReply);
            }

            //检查 Auth 是否过期
            if ($we_result === false) {
                if ($bid == $debug_bid) Kohana::$log->add("weixin_wsd:$bid:we_result", print_r($we_result, true));
                //$we->resetAuth();
            }
        }
        ...
        //默认文字回复
        if ($txtReply) {
            Kohana::$log->add('wsdeee:', $txtReply);
