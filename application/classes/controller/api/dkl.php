...
Class Controller_Api_dkl extends Controller_API {

    var $token = 'duokelai2017';

    var $FromUserName;
    var $Keyword;
    var $access_token;
    var $baseurl = 'http://jfb.smfyun.com/dkl/';
    var $cdnurl = 'http://cdn.jfb.smfyun.com/dkl/';
    var $cdnurl2 = 'http://jfb.dev.smfyun.com/dkl/';
    var $scorename;
    ...
    public function action_get($bid=1)
    {
        ...
        if (!is_numeric($bid)) $bid = ORM::factory('dkl_login')->where('user', '=', $bid)->find()->id;
    ...
    {
        $this->baseurl = 'http://'. $_SERVER['HTTP_HOST'] .'/dkl/';
        ...
        //username->bid
        //if (!is_numeric($bid))
        $biz = ORM::factory('dkl_login')->where('user', '=', $bname)->find();
        $bid = $biz->id;
        ...
        if ($biz->expiretime && (strtotime($biz->expiretime)+86400) < time()) {
            $die = 1;
            $txtReply = '您的多客来插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
        }
        ...
        $sname = ORM::factory('dkl_cfg')->where('bid','=',$bid)->where('key','=','scorename')->find()->value;
        if($sname){
        ...
        if($die == 1){
            $msg['touser'] = $openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = '您的多客来插件已过期！在有赞后台-概况-微信-插件中心，关闭对应的插件即可取消本提示；如需继续使用，请联系第三方续费。';
            $we->sendCustomMessage($msg);
            exit;
        }
        //取消关注扣积分
        if($config['btnn']==1){
            $child1=ORM::factory('dkl_qrcode')->where('bid','=',$bid)->where('fopenid','=',$openid)->find_all();
            ...
            foreach ($child1 as $a) {
                ...
                if ($chuserinfo['subscribe'] === 0) {
                    ...
                    $qg=ORM::factory('dkl_qgscore')->where('bid','=',$bid)->where('openid','=',$a->openid)->find()->openid;
                    $userobj0=ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $a->openid)->find();
                    $userobj0->subscribe=0;
                    $userobj0->save();
                   // Kohana::$log->add("weixin2:aaaa",print_r($qg,true));
                    if(!$qg){
                        Kohana::$log->add("weixin2:ddddd", 'ddddd');
                        $qg=ORM::factory('dkl_qgscore');
                        $qg->bid=$bid;
                        $qg->openid=$a->openid;
                        $qg->save();
                        $userobj0 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $a->openid)->find();
                        ...
                        $userobj1 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
                        $userobj1->scores->scoreOut($userobj1, 10,$config['goal']);
                        if($config['switch']==1){
                            $this->rsync($bid,$userobj1->openid,$access_token,-$config['goal']);
                        }
                        $fopenid1 =ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->fopenid;
                        $userobj3 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenid1)->find()->id;
                        if($userobj3){
                            $userobj2 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenid1)->find();
            ...
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM dkl_qrcodes Where `bid` = $bid and `fopenid`='$openid'")->execute()->as_array();
            $tempid=array();
            ...
            $child2 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('fopenid','IN',$tempid)->find_all();
            foreach ($child2 as $b) {
                $chuserinfo=array();
                $chuserinfo = $we->getUserInfo($b->openid);
                if ($chuserinfo['subscribe'] === 0) {
                    $qg=ORM::factory('dkl_qgscore')->where('bid','=',$bid)->where('openid','=',$b->openid)->find()->openid;
                    $userobj0=ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $b->openid)->find();
                    $userobj0->subscribe=0;
                    $userobj0->save();
                    if(!$qg){
                        $qg=ORM::factory('dkl_qgscore');
                        ...
                        $fopenid1 =ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $b->openid)->find()->fopenid;
                        $userobj2 = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenid1)->find();
        ...
        //当前微信用户
        $model_q = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
        if($access_token){
        ...
        if ($userinfo && ($Event == 'SCAN' && $EventKey == $scene_id) || ($we->getRevSceneId() == $scene_id)) {
            ...
            //珂心如意
            $fuser = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('ticket', '=', $Ticket)->find();//上一级
            if($fuser->fopenid){
            ...
            if ($model_q->id && !$model_q->fopenid && $fopenid && $fopenid != $openid && $ffopenid!=$openid&&$model_q->lock!=1&&$fuser->lock!=1) {
                ...
                //首次关注积分
                if (ORM::factory('dkl_score')->where('qid', '=', $model_q->id)->where('type', '=', 1)->count_all() == 0) {
                    if ($config['goal0'] > 0) {
                        $model_q = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
                ...
                if ($model_q->id > $fuser->id) {
                    ...
                    if ($config['risk_level1'] > 0 && $config['risk_level2'] > 0) {
                        //直接用户
                        $count2 = ORM::factory('dkl_qrcode', $fuser->id)->scores->where('type', '=', 2)->count_all();
                        //用是否生成海报判断真实下线
                        $count3 = ORM::factory('dkl_qrcode')->where('bid', '=', $fuser->bid)->where('fopenid', '=', $fuser->openid)->where('ticket', '<>', '')->count_all();
                        // //解除锁定
                    ...
                    //上一级推荐人 积分增加处理
                    //珂心如意
                    $ffuser = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fopenidFromQrcode)->find();

                    if ($ffuser->fopenid) {
                        //返利给 $ffuser 的上线 $ffuser->fopenid;
                        $fffuser = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $ffuser->fopenid)->find();//上上级
                        if ($fffuser && $config['goal2'] > 0&&$fffuser->lock!=1) {
                            $fffuser = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $ffuser->fopenid)->find();//上上级
                            $goal2_result = ORM::factory('dkl_score')->scoreIn($fffuser, 3, $config['goal2'], $model_q->id);
                            if($config['switch']==1){
            ...
            //已经有上级就直接取来
                ...
                //$fuser = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $model_q->fopenid)->find();
            ...
            //自定义回复文本消息
            //读取数据库： $replySet = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'text_goal3')->find()->value;
            ...
            if ($model_q->id && $model_q->fopenid && $fopenid){
                $fnickname = ORM::factory('dkl_qrcode')->where('bid','=',$bid)->where('openid','=',$model_q->fopenid)->find()->nickname;
        ...
        if ($userinfo && $Event == 'CLICK' || $chunv2) {
            ...
            $position = 0;
            $u_location = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->area;
            for ($i=1; $i <=$count ; $i++) {
                $pro[$i] = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'pro'.$i)->find()->value;
                $city[$i] = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'city'.$i)->find()->value;
                $dis[$i] = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'dis'.$i)->find()->value;
            ...
            $status = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'status')->find()->value;
         if(($position >0 && $status=='1')||$status=='0'||!$status){
            ...
            //生成海报
                ...
                if ($config['needpay']&&$access_token) {
                    require_once Kohana::find_file('vendor', 'kdt/YZTokenClient');
                    $qr_id = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->needpost;
                    if($qr_id!=0&&$qr_id!=1){
                        ...
                        Kohana::$log->add('wwwresults',print_r($results,true));
                        if($results['response']['qr_trades']) {
                            $qr = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
                            $qr->needpost=1;
                            $qr->save();
                             Kohana::$log->add('111','111');
                        }
                        //Kohana::$log->add('s1:', print_r($results, true));//写入日志，可以删除
                    }
                    $needpost = ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->needpost;
                    if($needpost==1) {
                    }else{
                ...
                //图片合成
                //模板
                $imgtpl = DOCROOT."dkl/tmp/tpl.$bid.jpg";
                $tmpdir = '/dev/shm/';
                Kohana::$log->add("imgtpl", print_r($imgtpl,true));
                //判断模板文件是否需要从数据库更新
                $tpl = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'tpl')->find();
                //Kohana::$log->add("imgtpl", print_r($tpl->pic,true));
                if (!$tpl->pic) {
                ...
                if (!file_exists($imgtpl)) {
                    //Kohana::$log->add("imgtpl", print_r(dirname($imgtpl),true));
                    //umask(0002);
                    //@mkdir(dirname($imgtpl),0777,true);
                    mkdir(dirname($imgtpl),true);
                    //Kohana::$log->add("imgtpl", print_r(mkdir(dirname($imgtpl),0777,true),true));
                    file_put_contents($imgtpl, $tpl->pic);
                    //Kohana::$log->add("imgtpl", print_r(file_put_contents($imgtpl, $tpl->pic),true));
                }
                ...
                //有海报缓存直接发送
                $tpl_key = 'dkl:tpl:'.$openid.':'.$tpl->lastupdate;
                ...
                if ($uploadresult['media_id'] && !$newticket) {
                    ...
                    if (!$remote_head && $default_head_file) {
                        ...
                        Kohana::$log->add("dkl:$bid:head4:", $remote_head);
                    }
            ...
            //积分明细
                ...
                if (!file_exists(DOCROOT."dkl/$news_pic_file")) $news_pic_file = 'news_score.png';
                $news_pic = $this->cdnurl.$news_pic_file;
                Kohana::$log->add("news_pic",print_r($news_pic,'true'));
                ...
                Kohana::$log->add("newsReply",print_r($newsReply,'true'));
                // $we_result = $we->sendCustomMessage($msg);
            }
            ...
            //兑换商城
                ...
                if (!file_exists(DOCROOT."dkl/$news_pic_file")) $news_pic_file = 'news_order.jpg';
                $news_pic = $this->cdnurl.$news_pic_file;

                /*$news_pic_file = 'imgtpl/'.$bid.'/score2.jpg';
                // if (!file_exists(DOCROOT."dkl/$news_pic_file")) $news_pic_file = 'imgtpl/'.'$bid'.'/score1.jpg';
            ...
            //排行榜
                ...
                if (!file_exists(DOCROOT."dkl/$news_pic_file")) $news_pic_file = 'news_top.png';
                $news_pic = $this->cdnurl.$news_pic_file;
                ...
                $msg['msgtype'] = 'text';
                $txtReply = $msg['text']['content'] = '请将下面的 KEY 填写到「多客来」商户后台完成配置：'.$EventKey;

                //用户少的时候才回复 debug
                // if (ORM::factory('dkl_qrcode')->where('bid', '=', $bid)->count_all() < 10)
                //$we_result = $we->sendCustomMessage($msg);
            }
            ...
            //$msg['text']['content'] = '符合要求'.$u_location.$p_location;
                ...
                $replyfront = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyfront')->find()->value;
                $replyend = ORM::factory('dkl_cfg')->where('bid', '=', $bid)->where('key', '=', 'replyend')->find()->value;
        ...
        if ($txtReply) {
            ...
            Kohana::$log->add("dkltxtresult", print_r($txtresult, true));
            echo $txtresult;
        }
