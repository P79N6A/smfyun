...
class Controller_xyba extends Controller_Base {

    public $template = 'weixin/xyb/tpl/atpl';
    public $pagesize = 20;
    //public $access_token;
    public $config;
    public $bid;
    ...
    public function before() {
        ...
        $this->bid = $_SESSION['xyba']['bid'];
        $this->config = $_SESSION['xyba']['config'];
        ...
        if (Request::instance()->action != 'login' && !$this->bid) {
            ...
            header('location:/xyba/login?from='.Request::instance()->action);
            exit;
        }
    }
    ...
    public function action_orders($action='', $id=0) {
        ...
        if ($action == 'ship' && $id) {
            ...
            if ($order->status == 0) {
                ...
                //有单号的情况
                 if ($_REQUEST['remark']) {
                    $_SESSION['xyba']['remark'] = $_REQUEST['remark'];
                    $order->remark = $_REQUEST['remark'];
                    $order->flag=$_REQUEST['flag'];
                    $order->save();
                }
                if ($_REQUEST['shiptype'] && $_REQUEST['shipcode']) {
                    $_SESSION['xyba']['shiptype'] = $_REQUEST['shiptype'];
                    $_SESSION['xyba']['shipcode'] = $_REQUEST['shipcode'];
        ...
        $this->template->content = View::factory('weixin/xyb/admin/orders')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config)
    ...
    public function action_login() {
        $this->template = 'weixin/xyb/tpl/login';
        $this->before();
        ...
        if ($_POST['username'] && $_POST['password']) {
            ...
            if ($biz->id) {
                if ($biz->expiretime && strtotime($biz->expiretime) < time()) {
                    ...
                    $_SESSION['xyba']['bid'] = $biz->id;
                    $_SESSION['xyba']['user'] = $_POST['username'];
                    $_SESSION['xyba']['admin'] = $biz->admin; //超管
                    $_SESSION['xyba']['config'] = ORM::factory('wfb_cfg')->getCfg($biz->id);
        ...
        if ($_SESSION['xyba']['bid']) {
            if (!$_GET['from']) $_GET['from'] = 'orders';
            header('location:/xyba/'.$_GET['from']);
            exit;
        }
    }
