...
class Controller_wsd extends Controller_Base {
    public $template = 'weixin/wsd/tpl/fftpl';
    public $access_token;
    public $config;
    public $openid;
    public $bid;
    ...
    var $baseurl = 'http://dd.smfyun.com/wsd/';
    var $we;
    var $client;
    public function before() {
        ...
        if (Request::instance()->action == 'group_cron') return;
        if (Request::instance()->action == 'trade_get') return;
        if (Request::instance()->action == 'trade_express') return;
        //if (Request::instance()->action == 'index_oauth') return;
        $_SESSION =& Session::instance()->as_array();
        $biz = ORM::factory('wsd_login')->where('id','=',$_SESSION['wsd']['bid'])->find();
        if ($biz->expiretime && strtotime($biz->expiretime)+86400 < time()) die('您的账号已过期');
        $this->config = $_SESSION['wsd']['config'];
        $this->openid = $_SESSION['wsd']['openid'];
        $this->bid = $_SESSION['wsd']['bid']?$_SESSION['wsd']['bid']:$_SESSION['wsda']['bid'];
        $this->uid = $_SESSION['wsd']['uid'];
        $this->access_token = $_SESSION['wsd']['access_token'];


        if ($_GET['debug']) print_r($_SESSION['wsd']);


        //只能通过微信打开
        if (stripos($_SERVER["HTTP_USER_AGENT"], 'MicroMess') === false && !$_SESSION['wsda']['bid']) die('请通过微信访问。');
    }

    public function after() {
        $user = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find()->as_array();

         $firstchild=DB::query(Database::SELECT,"SELECT openid FROM wsd_qrcodes WHERE fopenid='$this->openid'")->execute()->as_array();

        $customer=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->order_by('paid', 'DESC');
        $user['follows'] =$customer->count_all();


        $month = strtotime(date('Y-m-1'));
        $firstchild=DB::query(Database::SELECT,"SELECT openid FROM wsd_qrcodes WHERE fopenid='$this->openid' and jointime>='$month'")->execute()->as_array();

        $user['follows_month']=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->where('jointime','>=',$month)->count_all();
        $user['trades'] = ORM::factory('wsd_score')->where('qid', '=', $user['id'])->where('type', 'IN', array(2,3))->count_all();
    ...
    public function action_group_cron($bid){
        $dl_user = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','=',1)->find_all();
        foreach ($dl_user as $k => $u) {
            $group = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$u->id)->order_by('id','desc')->find();
            echo '<=====================================================><br>';
            echo $group->id.'现有的：'.$group->bottom.'<br>';
            $old_arr = explode(',', $group->bottom);
            // echo '<pre>';
            // var_dump($old_arr);
            $loop_group = $group;
            $str = '';
            $loop_group = ORM::factory('wsd_group')->where('bid','=',$bid)->where('fgid','=',$loop_group->id)->find_all();
            foreach ($loop_group as $key => $value) {
                if(in_array($value->id,$old_arr)){

                }else{
                    echo '缺少1:'.$value->id.'<br>';
                }
                if(in_array($value->bottom,$old_arr)){

                }else{
                    $new_arr = explode(',', $value->bottom);
                    foreach ($new_arr as $n => $a) {
                        if(in_array($a,$old_arr)){
                ...
                $str = $str.','.$value->id.','.$value->bottom;
            }
            echo $group->id.'应该有的：'.$str.'<br>';
        }
    }
    public function action_err_trade($bid){
        $trades = ORM::factory('wsd_trade')->where('bid','=',$bid)->where('fopenid','=',null)->find_all();
        foreach ($trades as $k => $v) {
            echo $k.'<br>';
            $user = ORM::factory('wsd_qrcode')->where('id','=',$v->qid)->where('bid','=',$bid)->find();
            if($user->fopenid==''){
                echo $user->openid.'<br>';
            }
        }
    ...
    public function action_index($bid) {
        //只能通过微信打开
        if (stripos($_SERVER["HTTP_USER_AGENT"], 'MicroMess') === false && !$_SESSION['wsda']['bid']) return $this->action_msg('请通过微信打开！', 'warn');

        $config = ORM::factory('wsd_cfg')->getCfg($bid,1);
        $this->access_token=ORM::factory('wsd_login')->where('id', '=', $bid)->find()->access_token;
        if (!$_GET['openid']) $_SESSION['wsd'] = NULL;
        ...
        if ($config && $_GET['openid']) {
            ...
            $userobj = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();

            $_SESSION['wsd']['config'] = $config;
            $_SESSION['wsd']['openid'] = $openid;
            $_SESSION['wsd']['bid'] = $bid;
            $_SESSION['wsd']['uid'] = $userobj->id;
            $_SESSION['wsd']['access_token'] =$this->access_token;
            Request::instance()->redirect('/wsd/'.$_GET['url']);
        }
    }
    public function action_cron($bid){
        set_time_limit(0);
        $this->config=$config=ORM::factory('wsd_cfg')->getCfg($bid,1);
        $login=ORM::factory('wsd_login')->where('id','=',$bid)->find();
        if(!$login->id) die('没有此商户');
        $day=date('d',time());
        // if($day!=$config['date']) die ('没有到结算日期');
        $qrs=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->find_all();
        foreach ($qrs as $v) {
            $nawtime=time();
            $monthtype='%Y-%m';
            ...
            $score=ORM::factory('wsd_score')->where('bid','=',$bid)->where('qid','=',$v->id)->where('bz','=',$month)->find();
            if(!$score->id){
                $groups=ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$v->id)->find_all();
                $monthjs_pmoney=0;
                ...
                if($result['result_code']=='FAIL'){
                    ...
                    ORM::factory('wsd_score')->scoreOut($v,4, $monthjs_pmoney,'','',$month);
                }
    ...
    public function action_index_oauth($bid, $url='form') {
       ...
       if(!in_array($url,$urls)) die('url不合法'.$url);
        ...
        //     $appId = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'youzan_appid')->find()->value;
        //     $appSecret = ORM::factory('wsd_cfg')->where('bid', '=', $bid)->where('key', '=', 'youzan_appsecret')->find()->value;
        ...
        //     $userobj = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $_GET['open_id'])->find();
        ...
        //     $_SESSION['wsd']['config'] = $config;
        //     $_SESSION['wsd']['openid'] = $_GET['open_id'];
        //     $_SESSION['wsd']['bid'] = $bid;
        //     $_SESSION['wsd']['uid'] = $userobj->id;
        //     Request::instance()->redirect('/wsd/'.$url.'/'.$userobj->openid);
        // }
                //没有 Oauth 授权过才需要
        if ($config) {
    ...
    public function action_memberpage($openid){
        $view = "weixin/wsd/memberpage";
        $this->template = 'tpl/blank';
        self::before();
        if(!$this->bid) die('页面已过期，请重试');
        $config = ORM::factory('wsd_cfg')->getCfg($this->bid,1);
        // $config['buy_money'] = 35;
        $userobj = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $openid)->find();
        if($userobj->lv==1){//正常
            $v = $userobj;
        ...
        if($_POST['tel']){
            $tel = ORM::factory('wsd_qrcode')->where('lv', '=', 1)->where('bid', '=', $this->bid)->where('tel', '=', $_POST['tel'])->find();
            if($tel->id){
                $result['error'] = '对不起，您的手机号已经注册了';
            }else{
                $tel = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $openid)->find();
                $tel->tel = $_POST['tel'];
                $tel->save();
    ...
    public function action_myteam($openid,$bid){
        $view = "weixin/wsd/myteam";
        $this->template = 'tpl/blank';
        self::before();
        $user = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
        $result['follows'] = ORM::factory('wsd_qrcode')->where('lv','=',1)->where('bid','=',$bid)->where('fopenid','=',$openid)->order_by('id','DESC')->find_all();
        $result['num'] = ORM::factory('wsd_qrcode')->where('lv','=',1)->where('bid','=',$bid)->where('fopenid','=',$openid)->count_all();
    ...
    public function action_account_record($uid){
        if(!$this->bid) die('页面已过期，请重试');
        $view = "weixin/wsd/account_record";
        $this->template = 'tpl/blank';
        self::before();
        $this->uid = $uid;
        $user = ORM::factory('wsd_qrcode')->where('bid','=',$this->bid)->where('id','=',$this->uid)->find();
        $records = ORM::factory('wsd_score')->where('bid','=',$this->bid)->where('qid','=',$user->id)->where('score','<',0)->find_all();
        $this->template->content = View::factory($view)
            ->bind('records', $records)
            ->bind('config', $config);
    }
    public function action_order_top($bid){
        $view = "weixin/wsd/now_top";
        $this->template = 'tpl/blank';
        self::before();
        $userobjs = ORM::factory('wsd_qrcode')->where('lv','=',1)->where('bid','=',$bid)->find_all();
        $user = array();
        foreach ($userobjs as $k => $v) {
            if($_POST['start']&&$_POST['end']){
                $now_payment = ORM::factory('wsd_trade')->where('bid','=',$bid)->where('fopenid','=',$v->openid)->where('int_time','<',strtotime($_POST['end']))->where('int_time','>',strtotime($_POST['start']))->select(array('SUM("payment")', 'all_payment'))->find()->all_payment;
            }else{
                $now_payment = ORM::factory('wsd_trade')->where('bid','=',$bid)->where('fopenid','=',$v->openid)->select(array('SUM("payment")', 'all_payment'))->find()->all_payment;
            }
            $user[$k]['id'] = $v->id;
    ...
    public function action_order_detail($openid){
        if(!$this->bid) die('页面已过期，请重试');
        $view = "weixin/wsd/order_detail";
        $this->template = 'tpl/blank';
        self::before();
        // echo $this->uid;
        // exit;
        $this->uid = $uid;
        $user = ORM::factory('wsd_qrcode')->where('bid','=',$this->bid)->where('openid','=',$openid)->find();
        $orders = ORM::factory('wsd_trade')->where('bid','=',$this->bid)->where('fopenid','=',$openid)->order_by('id','DESC')->find_all();
        // echo $orders;
        // var_dump($orders);
    ...
    public function action_code($openid){
        ...
        if($_POST['tel2']){
            $tel = ORM::factory('wsd_qrcode')->where('lv', '=', 1)->where('bid', '=', $this->bid)->where('tel', '=', $_POST['tel2'])->find();
            if($tel->id){
                $result['error'] = '对不起，您的手机号已经注册了';
            }else{
                $tel = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $openid)->find();
    ...
    public function action_commends($mopenid,$bid){//奖品list分享页面
        $fopenid = $mopenid;
        $fuser = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$fopenid)->find();
        if($fuser->lv != 1) die($fuser->nickname.'代理商资格未获得！');
        if(!$fuser->id) die('不合法2');
        $config=ORM::factory('wsd_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/wsd/commends/'.$mopenid.'/'.$bid;
        $this->we = $we = new Wechat($config);

        $callback_url = 'http://'.$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"];
        if (isset($_GET['url'])) $callback_url = urldecode($_GET['url']);
        $signPackage = $we->getJsSign($callback_url);
        $userobj = ORM::factory('wsd_qrcode', $this->uid);
        if($_GET['code']){//静默授权当事人
            $this->template = 'weixin/wsd/tpl/tpl2';
            self::before();
            $token = $we->getOauthAccessToken();
            $openid=$token['openid'];
            $userinfo = $we->getOauthUserinfo($token['access_token'],$openid);
            Kohana::$log->add("1commends_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            ...
            Kohana::$log->add("2commends_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            $user=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
            if(!$user->id){
                $user->bid=$bid;
                $user->openid=$openid;
            ...
            $_SESSION['wsd']['openid'] = $openid;
            $_SESSION['wsd']['bid'] = $bid;
            $_SESSION['wsd']['uid'] = $user->id;

            $this->config = $_SESSION['wsd']['config'];
            $this->openid = $_SESSION['wsd']['openid'];
            $this->bid = $_SESSION['wsd']['bid'];
            $this->uid = $_SESSION['wsd']['uid'];
            ...
            if($user->fopenid||$user->lv==1){
                ...
                $user->headimgurl = $userinfo['headimgurl'];
                $user->nickname = $userinfo['nickname'];
                if($user->lv!=1&&$fopenid != $openid){
                    $user->fopenid = $fopenid;
                }
                $user->save();
            }else{
                $result['title'] = $fuser->nickname.'的推荐商品';
                if($fopenid != $openid){//上线id大于本人id
                    $user->nickname = $userinfo['nickname'];
            ...
            $view = "weixin/wsd/commendsother";//别人直接是url进，不需要加密
            if($fopenid == $openid){
                // echo '自己';
                $status = 2;
                $result['title'] = $user->nickname.'的推荐商品';
                $view = "weixin/wsd/commends";//自己进 跳入shareopenid 需要加密url
            }
            $this->template->title = $fuser->nickname.'推荐商品';
            $result['commends'] = ORM::factory('wsd_setgood')->where('bid','=',$bid)->where('status','=',1)->order_by('id','DESC')->find_all();
            $result['openid'] = $user->openid;
            $shop = ORM::factory('wsd_login')->where('id','=',$bid)->find();
    ...
    public function action_userinfo($lv){
        ...
        $users = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('lv','=',$lv)->where('nickname','=','')->find_all();
    ...
    public function action_shareopenid($mopenid,$gid,$bid){//商品分享页面 自己可以打开 别人也可以打开
        ...
        $config=ORM::factory('wsd_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/wsd/shareopenid/'.$mopenid.'/'.$gid.'/'.$bid;
        $this->we = $we = new Wechat($config);
        ...
        if($_GET['code']){//静默授权当事人
            $this->template = 'weixin/wsd/tpl/tpl';
            self::before();
            $token = $we->getOauthAccessToken();
            $openid=$token['openid'];
            $userinfo = $we->getOauthUserinfo($token['access_token'],$openid);
            Kohana::$log->add("1shareopenid_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            if(!$openid||!$userinfo['nickname']){//当别人打开网页时候 code 失效 openid无法获得 重新授权获得当事人openid和上级fopenid进行绑定
                $auth_url = $we->getOauthRedirect($callback_url, '', 'snsapi_userinfo');
                header("Location:$auth_url");exit;
            }
            Kohana::$log->add("2shareopenid_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            // if($openid=='okxUMwaR4qmy8axy2Acx5dok15e0'){
            //     echo '<pre>';
            //     var_dump($userinfo);
            //     exit;
            ...
            $commend = ORM::factory('wsd_setgood')->where('bid','=',$bid)->where('id','=',$gid)->find();
            $view = "weixin/wsd/share";
    ...
    public function action_shop($mopenid,$bid){//有赞店铺分享页面 自己可以打开 别人也可以打开
        ...
        $config=ORM::factory('wsd_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/wsd/shop/'.$mopenid.'/'.$bid;
        $this->we = $we = new Wechat($config);
        ...
        if($_GET['code']){//静默授权当事人
            ...
            $openid=$token['openid'];
            $userinfo = $we->getOauthUserinfo($token['access_token'],$openid);
            Kohana::$log->add("1shop_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            if(!$openid||!$userinfo['nickname']){//当别人打开网页时候 code 失效 openid无法获得 重新授权获得当事人openid和上级fopenid进行绑定
                $auth_url = $we->getOauthRedirect($callback_url, '', 'snsapi_userinfo');
                header("Location:$auth_url");exit;
            }
            Kohana::$log->add("2shop_userinfo_wsd:$bid:$openid", print_r($userinfo, true));
            // if($openid=='okxUMwaR4qmy8axy2Acx5dok15e0'){
            //     echo '<pre>';
            //     var_dump($userinfo);
            //     exit;
    ...
    public function action_appointment($mopenid,$bid){//推荐代理给别人
        ...
        if($_POST['tel']&&$_POST['openid']){
            $openid = $_POST['openid'];
            $tel = ORM::factory('wsd_qrcode')->where('lv', '=', 1)->where('bid', '=', $bid)->where('tel', '=', $_POST['tel'])->find();
            if($tel->id){
            ...
            $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/wsd/appointment/'.$mopenid.'/'.$bid;
            $this->we = $we = new Wechat($config);
        ...
        $view = "weixin/wsd/appointment";//别人直接是url进，不需要加密
    ...
    public function action_money($out=0, $cksum='') {
        ...
        if ($out == 1 && $cksum_flag == true && ($MONEY >= $this->config['money_out']/100) ) {
            ...
            if ($result_m['result_code'] == 'SUCCESS') {
                ...
                // print_r($result);exit;
                Kohana::$log->add("weixin_wsd:$bid:money", print_r($result, true));
    ...
    public function action_top2() {
        ...
        //计算排名
        $user = ORM::factory('wsd_qrcode', $this->uid)->as_array();

        $rankkey = "wsd:rank3:{$this->bid}:{$this->openid}:$top";
        $result['rank'] = $mem->get($rankkey);
        if (!$result['rank']) {
            $result['rank'] = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('score', '>', $user['score'])->count_all()+1;
            $mem->set($rankkey, $result['rank'], 600);
        }

        $topkey = "wsd:top3:{$this->bid}:$top";
        $users = $mem->get($topkey);
        if (!$users) {
            $usersobj = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('score', 'DESC')->limit($top)->find_all();
    ...
    public function action_order($tid) {
        $view = "weixin/wsd/order";

        $order = ORM::factory('wsd_trade', $tid);
    ...
    public function action_top() {
        ...
        $user = ORM::factory('wsd_qrcode', $this->uid);
        $top = $this->config['rank_wsd'] ? $this->config['rank_wsd'] : 10;

        $result['rank'] = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('lv','=',1)->where('paid', '>', $user->paid)->count_all()+1;

        $usersobj = ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->order_by('paid', 'DESC')->limit($top)->find_all();
        foreach ($usersobj as $userobj) {
            $users[] = $userobj->as_array();
        }
    }
    ...
    public function action_customer($newadd='') {
        ...
        $user = ORM::factory('wsd_qrcode', $this->uid);
         ...
         {
            $month = strtotime(date('Y-m-1'));
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM wsd_qrcodes WHERE fopenid='$user->openid' and jointime>='$month'")->execute()->as_array();
         }
         else
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM wsd_qrcodes WHERE fopenid='$user->openid'")->execute()->as_array();

           if($newadd=='month')
           {
            $customer=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('jointime','>=',$month);
           }
           else
             $customer=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid);
         ...
         if($newadd=='month')
           {
            $totlecustomer=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('jointime','>=',$month)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
           }
         else
           $totlecustomer=ORM::factory('wsd_qrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
    ...
    //     require_once Kohana::find_file('vendor', 'weixin/inc');
    //     require_once Kohana::find_file('vendor', 'weixin/wechat.class');
    //     $bid = ORM::factory('wsd_login')->where('shopid','=',$kdt_id)->find()->id;
    //     $this->bid=$bid;
    //     $this->config = $config = ORM::factory('wsd_cfg')->getCfg($bid);
    ...
    //     }else{
    //         Kohana::$log->add("wsd:$bid:bname", print_r('有赞参数未填', true));
    //     }
    ...
    //         Kohana::$log->add("wsd:$bid", print_r($jsona, true));
    //         $trade=$jsona['trade'];
    ...
    //     }
    // }
    public function action_trade_get($bid,$tid){
        require_once Kohana::find_file('vendor', 'kdt/YZTokenClient');
        $this->access_token=ORM::factory('wsd_login')->where('id', '=', $bid)->find()->access_token;
        if($this->access_token){
            $this->client =$client= new YZTokenClient($this->access_token);
    ...
    public function action_trade_express($bid,$tid){
        $view = "weixin/wsd/trade_express";
        require_once Kohana::find_file('vendor', 'kdt/YZTokenClient');
        $this->access_token=ORM::factory('wsd_login')->where('id', '=', $bid)->find()->access_token;
        if($this->access_token){
            $this->client =$client= new YZTokenClient($this->access_token);
        ...
        $method = 'youzan.logistics.expressbyorderno.get';
        $params = [
            'tid'=>$tid,
        ];
        $result = $client->post($method, $this->methodVersion, $params, $files);
        ...
        $express = $client->post($method, $this->methodVersion, $params, $files);
        // echo '<pre>';
        // var_dump($result);
        // var_dump($express);
        // if(is_array($express)){
        ...
        //     $express = array();
        // }
        foreach ($express['response']['allExpress'] as $k => $v) {
            if($v['id']==$result['response']['express_id']){
                $result['response']['express_id'] = $v['name'];
            }
        }
        $res = json_decode($result['response']['data']);
        // exit;
        $this->template->title = '查看订单';
        $this->template->content = View::factory($view)->bind('res', $res)->bind('result', $result)->bind('tid', $tid);
    }
    public function action_test(){
        $this->template = 'tpl/blank';
        self::before();
        $postStr = file_get_contents("php://input");
        Kohana::$log->add('wsdpostStr', print_r($postStr, true));
        $result11=json_decode($postStr,true);
        if($postStr){
            Kohana::$log->add('bbbbbbbb', 'aaaaaaa');
            $enddata = array('code' => 0,'msg'=>'success');
        ...
        if($sign != $result11['sign']){
            ...
            if($type=='TRADE_ORDER_STATE'){
                ...
                $this->bid=$bid;
                $this->config = $config = ORM::factory('wsd_cfg')->getCfg($bid);
                $expiretime=ORM::factory('wsd_login')->where('id', '=', $bid)->find()->expiretime;
                ...
                if($status=='WAIT_SELLER_SEND_GOODS'||$status=='WAIT_BUYER_CONFIRM_GOODS'||$status=='TRADE_SUCCESS'||$status=='TRADE_CLOSED'){
                    ...
                    $result = $client->post($method,'4.0.0', $params, $files);
                ...
                $posttid=urldecode($msg);
                $jsona=json_decode($posttid,true);
                Kohana::$log->add("wsd{$bid}item", print_r($jsona, true));
                $data=json_decode($jsona['data'],true);
                $item_id=$data['item_id'];
                if ($status=='ITEM_DELETE'||$status=='ITEM_SALE_DOWN') {
                    //商品删除和下架
                    ORM::factory('wsd_setgood')->where('bid', '=', $bid)->where('num_iid','=',$item_id)->delete_all();
                    ORM::factory('wsd_goodsku')->where('bid', '=', $bid)->where('item_id','=',$item_id)->delete_all();
                    ...
                    if($item_id==415069539) return;//商品不刷新
                    $skus=$item['skus'];
                    $type=0;
                    $sql = DB::query(Database::UPDATE,"UPDATE `wsd_goodskus` SET `state` = 3  where `item_id` = $item_id");
                    $sql->execute();
                    if($skus){
                        $type=1;
                        foreach ($skus as $sku) {
                            ...
                            $num=$sku['quantity'];
                            //echo $sku_id."<br>";
                            $sku_num = ORM::factory('wsd_goodsku')->where('sku_id', '=', $sku_id)->where('item_id','=',$item_id)->count_all();
                            //echo $sku_num.'<br>';
                            if($sku_num==0 && $sku_id){
                                //echo "上面<br>";
                                ...
                                //echo "下面<br>";
                                $sql = DB::query(Database::UPDATE,"UPDATE `wsd_goodskus` SET `bid` = $bid ,`item_id` = $item_id,`title` ='$title',`sku_id`=$sku_id, `price`=$price,`state` = 1 , `num`= $num where `sku_id` = $sku_id and  `item_id` = $item_id");
                                $sql->execute();
                            }
                        }
                    }
                    ...
                    if($num_num==0 && $num_iid){
                        $sql = DB::query(Database::INSERT,"INSERT INTO `wsd_setgoods` (`bid`,`num_iid`,`title`,`price`, `pic`,`url`,`status`,`state`,`type`,`num`) VALUES ($bid,$num_iid,'$name' ,$price,'$pic','$url',0,0,$type,$num)");
                        $sql->execute();
                    }else{
                        $sql = DB::query(Database::UPDATE,"UPDATE `wsd_setgoods` SET `bid` = $bid ,`num_iid` = $num_iid,`title` ='$name',`price`=$price, `pic`='$pic',`url`='$url' ,`num` = $num ,`state` = 0 , `type` =$type where `num_iid` = $num_iid ");
                        $sql->execute();
                    }
                    $sql = DB::query(Database::DELETE,"DELETE FROM `wsd_goodskus` where `state` =3 and `bid` = $bid ");
                    $sql->execute();
    ...
    private function tradeImport($trade, $bid, $client, $we, $config,$status) {
        ...
        $wsd_trade = ORM::factory('wsd_trade')->where('tid', '=', $tid)->find();
        //跳过已导入订单
        if ($wsd_trade->id) {
            //更新订单状态
            if ($wsd_trade->status != $trade['order_info']['status']) {
                $wsd_trade->status = $trade['order_info']['status'];
                //echo "$tid status updated.\n";
            }
            //退款订单删返利
            if ($status == 'TRADE_CLOSED'||$trade['order_info']['refund_state'] != 0){
                $qrcods_lv=ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('tid','=',$tid)->find();
                if($qrcods_lv->id) {
                    $qrcods_lv->lv=4;
                    $qrcods_lv->save();
                }
                $wsd_trade->deletedd=1;
                ORM::factory('wsd_score')->where('tid', '=', $wsd_trade->id)->delete_all();
            }
            if($status == 'TRADE_SUCCESS'){
                $score=ORM::factory('wsd_score')->where('bid','=',$bid)->where('tid','=',$wsd_trade->id)->find();
                $score->paydate=time();
                $score->save();
                $wsd_trade->out_time=time();
            }
            $wsd_trade->save();
            //echo "$tid pass.\n";
            return;
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $setgood=ORM::factory('wsd_setgood')->where('bid','=',$bid)->where('num_iid','=',$num_iid)->find();
            if($setgood->status==1){
                $setstatus=1;
                if($setgood->is_profit==0){
                    $trade['payment']+=$order['payment'];
                    Kohana::$log->add("$tid:if_profit", print_r($trade['payment'], true));
                }
            }
        }
        if($setstatus==0){
        ...
        //只处理有下线的订单
        $wsd_qrcode = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $userinfo['weixin_openid'])->find();
        Kohana::$log->add('id', print_r($wsd_qrcode->id, true));
        if (!$wsd_qrcode->id) {
            //echo "$tid no OpenID pass.\n";
            return;
        }
        if(!$wsd_qrcode->receiver_mobile){
            $wsd_qrcode->receiver_mobile=$trade['address_info']['receiver_tel'];
            $wsd_qrcode->save();
             $wsd_qrcode = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $userinfo['weixin_openid'])->find();
        }
        // if (!$wsd_qrcode->lv!=0&&!$wsd_qrcode->lv!=1&&!$wsd_qrcode->lv!=2) {
        //     //echo "$tid no OpenID pass.\n";
        //     return;
        // }
        //只处理用户生成海报时间后付款的订单
        $pay_time = strtotime($trade['order_info']['pay_time'])?strtotime($trade['order_info']['pay_time']):time();
        //是否购买过才能生成海报？
        //是：则判断关注后的订单都有收益
        //否：判断生成海报后的订单才算收益
        $fromtime = $wsd_qrcode->jointime;
        Kohana::$log->add('pay_time', print_r($pay_time, true));
        Kohana::$log->add('fromtime', print_r($fromtime, true));
        if ($pay_time < $fromtime) {
            //echo "$tid Time pass.\n";
            Kohana::$log->add('wsd:之后加入的pass', $tid);
            return;
        }
        $trade['qid'] = $wsd_qrcode->id;
        ...
        $rank=0;
        $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $trade['openid'])->find();
        if($fuser->fopenid||$fuser->lv==1){//有一级
            $rank=1;
            if($fuser->lv==1){
                $ffuser = $fuser;
            }else{
                $ffuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fuser->fopenid)->find();
            }
            $trade['fopenid'] = $ffuser->openid;
            if($ffuser->tid){
                $gointime=ORM::factory('wsd_trade')->where('bid', '=', $bid)->where('tid','=',$ffuser->tid)->find()->int_time;
            }else{
        ...
        foreach ($trade['orders'] as $order) {
            ...
            Kohana::$log->add('item_price', print_r($item_price,true));
            $num_iid=$order['item_id'];
            $setgood=ORM::factory('wsd_setgood')->where('bid','=',$bid)->where('num_iid','=',$num_iid)->find();
            Kohana::$log->add('sku_id', print_r($order['sku_id'],true));
            if($setgood->status==1){
                if($order['sku_id']&&$order['sku_id']!=0&&ORM::factory('wsd_goodsku')->where('sku_id','=',$order['sku_id'])->where('bid','=',$bid)->count_all()>0){
                    $sku_iid=$order['sku_id'];
                    $goodidcof=ORM::factory('wsd_goodsku')->where('sku_id','=',$order['sku_id'])->where('bid','=',$bid)->find();
                    Kohana::$log->add('sku_flag', 'aaa');
                }else{
                    $sku_iid=0;
                    $goodidcof=ORM::factory('wsd_setgood')->where('bid','=',$bid)->where('num_iid','=',$num_iid)->find();
                    Kohana::$log->add('sku_flag', 'bbb');
                }
                Kohana::$log->add('sku_group', print_r($goodidcof->id,true));
                if($ffuser->group_id==0){
                    Kohana::$log->add('sku_group1', print_r($goodidcof->money,true));
                    $money1=$money1+$order['num']*($item_price-$goodidcof->money);
                    Kohana::$log->add('sku_group1', print_r($money1,true));
                    $trade_detail = ORM::factory('wsd_tradedetail');
                    $trade_detail->bid = $bid;
                    $trade_detail->tid = $tid;
                    $trade_detail->skuid = $sku_iid;
                    $trade_detail->iid = $num_iid;
                    $trade_detail->total_fee = $order['total_fee'];//总价
                    $trade_detail->num = $order['num'];//件数
                    $trade_detail->money = $order['num']*($item_price-$goodidcof->money);
                    $trade_detail->save();
                }else{
                    $skumoney=ORM::factory('wsd_smoney')->where('bid','=',$bid)->where('sid','=',$ffuser->group_id)->where('item_id','=',$num_iid)->where('sku_id','=',$sku_iid)->find();
                    Kohana::$log->add('sku_group2', print_r($skumoney->id,true));
                    Kohana::$log->add('sku_group2', print_r($skumoney->money,true));
                    $money1=$money1+$order['num']*($item_price-$skumoney->money);
                    Kohana::$log->add('sku_group2', print_r($money1,true));
                    $trade_detail = ORM::factory('wsd_tradedetail');
                    $trade_detail->bid = $bid;
                    $trade_detail->tid = $tid;
                    $trade_detail->skuid = $sku_iid;
                    ...
                    $trade_detail->total_fee = $order['total_fee'];//总价
                    $trade_detail->num = $order['num'];//件数
                    $trade_detail->money = $order['num']*($item_price-$skumoney->money);
                    $trade_detail->save();
                }
            }
            Kohana::$log->add('money2', print_r($money1,true));
        }
        if($ffuser->lv==1){
            $money1 = $trade['money1'] = number_format($money1, 2,'.',''); //一级
            Kohana::$log->add('money3', print_r($money1,true));
            Kohana::$log->add('money4', print_r($trade['money1'],true));
        }
        //订单完成金额 达到一定值 进行升级
        // $all_payment = ORM::factory('wsd_trade')->select(array('SUM("payment")', 'all_payment'))->where('bid','=',$bid)->where('fopenid','=',$ffuser->openid)->where('status','=','TRADE_BUYER_SIGNED')->find()->all_payment;
        // if($all_payment){
        //     $skus = DB::query(Database::SELECT,"SELECT * FROM wsd_skus WHERE bid=$bid and `money`<=$all_payment")->execute()->as_array();
        ...
        if($wsd_qrcode->lv==1){
            ...
            $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('lv', '=', 1)->where('openid', '=', $wsd_qrcode->fopenid)->find();
        }
        $group=ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$fuser->id)->order_by('id','DESC')->find();
        Kohana::$log->add('trade', print_r($trade,true));
        $trade['gid']=$group->id;
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $wsd_order=ORM::factory('wsd_order')->where('bid','=',$bid)->where('tid','=',$tid)->where('goodid','=',$num_iid)->find();
            if(!$wsd_order->id)//跳过已导入的order
            {
                $wsd_order->bid=$bid;
                $wsd_order->tid=$tid;
                $wsd_order->goodid=$num_iid;
                $wsd_order->title=$title;
                $wsd_order->num=$num;
                $wsd_order->price=$price;
                $wsd_order->save();
            }
        }
        Kohana::$log->add('4444444', '444444');
        //删除重复返利记录
        ORM::factory('wsd_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 1)->delete_all();
        ORM::factory('wsd_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 2)->delete_all();
        ORM::factory('wsd_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 3)->delete_all();
        ...
        //订单上线返利
        if ($money1 > 0) {
            Kohana::$log->add('money6', print_r($money1,true));
            ...
            if ($fuser->id) {
                ...
                $fuser->scores->scoreIn($fuser, 1, $money1, $wsd_qrcode->id, $wsd_trade->id);
                // exit();
                //发消息
                $msg['touser'] = $fuser->openid;
                ...
                $msg['text']['content'] = "您推荐的「{$wsd_qrcode->nickname}」完成一笔订单！\n\n实付金额：$money 元\n销售利润：$money1 元\n\n";
        ...
        $newuser_self_tpl = "
销售利润+{$money1}元
个人销量+{$trade['payment']}元
团队销量+{$trade['payment']}元";
        $newuser_fuser_tpl = "
团队销量+{$trade['payment']}元";
        Kohana::$log->add('lv', print_r($wsd_qrcode->lv,true));
        Kohana::$log->add('payment', print_r($trade['pay_info']['payment'],true));
        Kohana::$log->add('buy_money', print_r($config['buy_money'],true));
        if($trade['tid'] == 'E20180531122911003000005'){
            $has = 1;
        }
        if(($wsd_qrcode->lv==2&&$trade['pay_info']['payment']>=$config['buy_money'])||$has==1){//已经申请了 并且金额也达到了要求
            $new = 1;
            $wsd_qrcode->lv = 1;
            $wsd_qrcode->receiver_mobile=$trade['receiver_tel'];
            $wsd_qrcode->tid=$trade['tid'];
            $wsd_qrcode->save();
            //自己购买之后 成为了代理商
            $msg['touser'] = $wsd_qrcode->openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = $config['text_self'];
            $we->sendCustomMessage($msg);

            $wsd_group = ORM::factory('wsd_group');
            $wsd_group->qid = $wsd_qrcode->id;
            $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $wsd_qrcode->fopenid)->find();
            if($fuser->id) {
                $wsd_group->fqid = $fuser->id;
                // $fgroup = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$fuser->id)->find();
                $wsd_group->fgid = ORM::factory('wsd_group')->where('bid','=',$bid)->where('qid','=',$fuser->id)->order_by('id','DESC')->find()->id;
            }
            $wsd_group->bid = $bid;
            $wsd_group->save();
            $wsd_flag = 1;
            $loop_qrcode = $wsd_qrcode;
            $loop_group = $wsd_group;
            while($wsd_flag == 1){
                ...
                if($loop_qrcode->id){
                    //新代理进来之后 给直属上代理发
                    if($loop_qrcode->openid == $wsd_qrcode->fopenid){
                        $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_direct']).$newuser_self_tpl;
                    }else{//新代理进来之后 给所有上级代理发
                        $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_group']);
                    ...
                    if(!strlen($loop_qrcode->bottom) > 0) {
                        $loop_qrcode->bottom = $wsd_qrcode->id;
                    }else{
                        $loop_qrcode->bottom = $loop_qrcode->bottom.','.$wsd_qrcode->id;
                    }
                    $loop_qrcode->save();
                    if(!strlen($wsd_qrcode->top) > 0) {
                        $wsd_qrcode->top = $loop_qrcode->id;
                    }else{
                        $wsd_qrcode->top = $loop_qrcode->id.','.$wsd_qrcode->top;
                    }
                    $wsd_qrcode->save();
                    if(!strlen($loop_group->bottom) > 0) {
                        $loop_group->bottom = $wsd_group->id;
                    }else{
                        $loop_group->bottom = $loop_group->bottom.','.$wsd_group->id;
                    }
                    $loop_group->save();
                }else{
                    $wsd_flag = 0;
...
订单金额：{$trade['money']}元
您的销售利润+{$money1}元
您的个人销量+{$trade['payment']}元
您的团队销量+{$trade['payment']}元 ";
        $order_fuser_tpl = "
订单名称：{$trade['title']}元
订单金额：{$trade['money']}元
您的团队销量+{$trade['payment']}元 ";
        if($wsd_qrcode->lv==1&&$new!=1){//先给自己发一条
            ...
            $fuser = ORM::factory('wsd_qrcode')->where('bid', '=', $bid)->where('openid', '=', $wsd_qrcode->fopenid)->find();
            if(strlen($wsd_qrcode->top) > 0) {
                $fusers = explode(",",$wsd_qrcode->top);
                for ($i=0; $fusers[$i]; $i++) {
                    $now_user = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('id','=',$fusers[$i])->find();
                    if($now_user->openid == $wsd_qrcode->fopenid){ //发直属上级
                        $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_dirctorder']).$order_fuser_tpl;
                    }else{ //上级的上级
                        $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_order']);
        ...
        if($new ==1 ||($wsd_qrcode->lv != 1 && $wsd_qrcode->fopenid)){
            //发直属上级
            $wsd_fuser = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('openid','=',$wsd_qrcode->fopenid)->find();
            $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_dirctorder']).$order_self_tpl;
            $msg['touser'] = $wsd_fuser->openid;
            $msg['msgtype'] = 'text';
            $msg['text']['content'] = $text;
            if($new!=1)$we->sendCustomMessage($msg);//新代理就不发
            //发上级的上级
            if(strlen($wsd_fuser->top) > 0) {
                $fusers = explode(",",$wsd_fuser->top);
                for ($i=0; $fusers[$i]; $i++) {
                    $now_user = ORM::factory('wsd_qrcode')->where('bid','=',$bid)->where('id','=',$fusers[$i])->find();
                    $text = str_replace('%s', $wsd_qrcode->nickname, $config['text_order']);
                    $text = str_replace('%t', $wsd_fuser->nickname, $text).$order_fuser_tpl;
                    $msg['touser'] = $now_user->openid;
                    $msg['msgtype'] = 'text';
                    $msg['text']['content'] = $text;
