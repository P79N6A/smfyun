...
class Controller_qwtyty extends Controller_Base {
    ...
    public function before() {
        ...
        if (Request::instance()->action == 'home'){
            $qrcode=ORM::factory('qwt_ytyqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find();
            $agent=$qrcode->agent;
            $stock1=DB::query(Database::SELECT,"SELECT SUM(money) as stock from qwt_ytystocks where qid = $qrcode->id and flag = 1")->execute()->as_array();
    ...
    public function after() {
        $user = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find()->as_array();
         $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_ytyqrcodes WHERE fopenid='$this->openid'")->execute()->as_array();
        $customer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->order_by('paid', 'DESC');
        $customer = $customer->reset(FALSE);
        $month = strtotime(date('Y-m-1'));
        $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_ytyqrcodes WHERE fopenid='$this->openid' and jointime>='$month'")->execute()->as_array();
        $user['follows_month']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->where('lv','=',1)->where('jointime','>=',$month)->count_all();
        $user['kehus_month']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->where('lv','!=',1)->where('jointime','>=',$month)->count_all();
        $user['follows']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->where('lv','=',1)->count_all();
        $user['kehus'] =ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $this->openid)->where('lv','!=',1)->count_all();
        $user['trades'] = ORM::factory('qwt_ytyscore')->where('qid', '=', $user['id'])->where('type', 'IN', array(2,3))->count_all();
             //待结算金额
    ...
    public function action_index($bid) {
        ...
        if ($config && $_GET['openid']) {
            ...
            $userobj = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
    ...
    public function action_getopenid($bid){//通过code获取openid
        ...
        var_dump($results);
        echo '</pre>';
        $user = ORM::factory('qwt_ytyqrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
    ...
    public function action_commends($mopenid,$bid){//奖品list分享页面
        ...
        //$userobj = ORM::factory('qwt_ytyqrcode', $this->uid);
        if($_GET['code']){//静默授权当事人
            ...
            $result['commends'] = ORM::factory('qwt_ytysetgood')->where('bid','=',$bid)->where('status','=',1)->find_all();
            $result['openid'] = $user->openid;
            $tpl = ORM::factory('qwt_ytycfg')->where('bid', '=', $bid)->where('key', '=', 'tplhead')->find()->id;
            $this->template->content = View::factory($view)->bind('result', $result)->bind('tpl',$tpl)->bind('config', $this->config)->bind('signPackage', $signPackage)->bind('nickname', $user->nickname);
        }else{//得到code为止
            $auth_url = $we->getOauthRedirect($callback_url, '', 'snsapi_base');
            header("Location:$auth_url");exit;
        }
    ...
    public function action_shareopenid($mopenid,$gid,$bid){//商品分享页面 自己可以打开 别人也可以打开
        ...
        if($_GET['code']){//静默授权当事人
            ...
            $commend = ORM::factory('qwt_ytysetgood')->where('bid','=',$bid)->where('id','=',$gid)->find();
    ...
    public function action_form($openid) {
        ...
        $bid=$this->bid;
        $tpl = ORM::factory('qwt_ytycfg')->where('bid', '=', $bid)->where('key', '=', 'tpl')->find()->id;
        $qrcode=ORM::factory('qwt_ytyqrcode')->where('bid','=',$this->bid)->where('openid','=',$openid)->find();
        if($qrcode->fopenid){
            $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$this->bid)->where('openid','=',$qrcode->fopenid)->find();
            $num=ORM::factory('qwt_ytysku')->where('bid','=',$this->bid)->count_all();
            if($fuser->agent->skus->order>=$num){
                die ('您的上级已是最低等级分销商，您不能成为他的下级分销商');
            }else{
                $order=$fuser->agent->skus->order+1;
                $sku=ORM::factory('qwt_ytysku')->where('bid','=',$this->bid)->where('order','=',$order)->find();
            }
        }else{
            $sku=ORM::factory('qwt_ytysku')->where('bid','=',$this->bid)->where('order','=',1)->find();
        }
        if($_POST['form']['name']&&$_POST['form']['address']&&$_POST['form']['tel']&&$_POST['form']['id_card']&&$_POST['form']['sid']){
            $s=ORM::factory('qwt_ytysku')->where('id','=',$_POST['form']['sid'])->find()->order;
            $userobj = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $openid)->find();
            $agent=ORM::factory('qwt_ytyagent')->where('bid','=',$this->bid)->where('openid','=',$openid)->find();
            $agent->openid =$openid;
            $agent->bid=$this->bid;
            ...
            if($userobj->fopenid){
                $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$this->bid)->where('openid','=',$userobj->fopenid)->find();
                ...
                $msg['text']['content'] = "{$userobj->nickname}申请成为你的下级经销商，请及时处理\n\n<a href=\"$url\">查看我的{$config['title5']}明细</a>";
                if ($config['task_deal_tpl']){
                    $we_result = $this->sendTaskMessage($msg['touser'], $url, '经销商申请', '经销商申请通知','' ,$userobj->nickname.'申请成为你的下级经销商，请及时处理');
                }else{
        ...
        $this->template->content = View::factory($view)->bind('tpl',$tpl)->bind('result', $result)->bind('skus', $sku);
    }
    //默认页面
    public function action_home() {
        ...
        //代理审核
        $result['num2']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',2)->where('fopenid','=',$userobj->openid)->count_all();
        //总收益
    ...
    public function action_money($out=0, $cksum='') {
        ...
        if ($out == 1 && $cksum_flag == true && ($MONEY >= $this->config['money_out']/100) ) {
            ...
            if ($userobj->agent->skus->status==0) die('您的经销商暂不允许提现，请联系管理员！');
    ...
    public function action_top2() {
        ...
        //计算排名
        $user = ORM::factory('qwt_ytyqrcode', $this->uid)->as_array();

        $rankkey = "yty:rank3:{$this->bid}:{$this->openid}:$top";
        $result['rank'] = $mem->get($rankkey);
        if (!$result['rank']) {
            $result['rank'] = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('score', '>', $user['score'])->count_all()+1;
        ...
        if (!$users) {
            $usersobj = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('score', 'DESC')->limit($top)->find_all();
    ...
    public function action_team() {
        ...
        if ($_POST['data']['id']) {
            $id = $_POST['data']['id'];
            $qrcode1 = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('id', '=', $id)->find();
            $fuser1=ORM::factory('qwt_ytyqrcode')->where('bid','=',$bid)->where('openid','=',$qrcode1->fopenid)->find();
            if ($qrcode1->id&&$qrcode1->agent->money*$fuser1->agent->skus->scale/100<=$fuser1->agent->stock) {
                if (isset($_POST['data'])) {
                    $qrcode1->lv = (int)$_POST['data']['lv'];
                    $agent=ORM::factory('qwt_ytyagent')->where('bid','=',$bid)->where('openid','=',$qrcode1->openid)->find();
                    ...
                    if((int)$_POST['data']['lv']==1){
                        ...
                        $front_user = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('id', '<', $qrcode1->id)->order_by('id','desc')->find();
        ...
        $member=0;
        $user = ORM::factory('qwt_ytyqrcode', $this->uid)->as_array();
        $qrcode = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',2)->where('fopenid','=',$user['openid'])->find_all();
        $result['num1']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('fopenid','=',$user['openid'])->count_all();
        $result['num2']=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',2)->where('fopenid','=',$user['openid'])->count_all();
        $totlenum=$result['num2'];
        if($_GET['member']==1){
            $member=1;
            $qrcode = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('fopenid','=',$user['openid'])->find_all();
    ...
    public function action_stock() {
        ...
        //计算排名
         if($_POST['data']){
            ...
            if($stock->money*$fuser->agent->skus->scale/100<=$stock->fqrcode->agent->stock){
                $stock->flag=1;
                $stock->money_all=$stock->qrcode->agent->stock+$stock->money;
                $stock->save();
                $qrcode=$stock->qrcode;
                ...
                $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$bid)->where('openid','=',$qrcode->fopenid)->find();
                ...
                $sqrcodes=ORM::factory('qwt_ytysqrcode')->where('bid','=',$bid)->where('qid','=',$qrcode->id)->find_all();
                foreach ($sqrcodes as $sqrcode) {
                    $sqid=$sqrcode->sqid;
                    Kohana::$log->add("weixin_yty:$bid:sqid", print_r($sqid, true));
                    $sqrcode1=ORM::factory('qwt_ytyqrcode')->where('id','=',$sqid)->find();
                    $money_s=$money*$config['money1']/100;
                     Kohana::$log->add("weixin_yty:$bid:money_s", print_r($money_s, true));
        ...
        $stocks=ORM::factory('qwt_ytystock')->where('bid','=',$this->bid)->where('type','in',array(0,1))->where('qid','=',$user['id'])->order_by('flag', 'ASC')->find_all();
        $result['num1']=ORM::factory('qwt_ytystock')->where('bid','=',$this->bid)->where('type','in',array(0,1))->where('qid','=',$user['id'])->count_all();
        $result['num2']=ORM::factory('qwt_ytystock')->where('bid','=',$this->bid)->where('type','in',array(0,1))->where('fqid','=',$user['id'])->count_all();
        $totlenum= $result['num1'];
        if($_GET['stock']==1){
            $status=2;
            $stocks=ORM::factory('qwt_ytystock')->where('bid','=',$this->bid)->where('type','in',array(0,1))->where('fqid','=',$user['id'])->order_by('flag', 'ASC')->find_all();
            $totlenum= $result['num2'];
        }
        $result['status']=$status;
        $page = max($_GET['page'], 1);
    ...
    public function action_mstock() {
        ...
        $user = ORM::factory('qwt_ytyqrcode', $this->uid)->as_array();
        $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$this->bid)->where('openid','=',$user['fopenid'])->find();
        if($_POST['form']['money']){
            $qrcode=ORM::factory('qwt_ytyqrcode')->where('id','=',$user['id'])->find();
    ...
    public function action_inputmoney() {
        ...
        $inputmoney=ORM::factory('qwt_ytystock')->where('qid','=',$user['id'])->where('bid','=',$this->bid)->where('flag','=',1)->find_all();
        $num=ORM::factory('qwt_ytystock')->where('qid','=',$user['id'])->where('bid','=',$this->bid)->where('flag','=',1)->count_all();
    ...
    public function action_orders() {
        ...
        if($_POST['data']['id']){
            ...
            $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$bid)->where('openid','=',$trade->fopenid)->find();
            if($fuser->agent->stock>=$trade->money1){
                ...
                if ($config['msg_score_tpl']){
                    $we_result = $this->sendScoreMessage($msg['touser'], '好友购买返利', $trade->money1, $fuser->score, $url);
                }else{
                    $we_result = $we->sendCustomMessage($msg);
                }
        ...
        $trades = ORM::factory('qwt_ytytrade')->where('bid','=',$bid)->where('fopenid','=',$userobj->openid)->order_by('flag', 'ASC')->find_all();
        $nums = ORM::factory('qwt_ytytrade')->where('bid','=',$bid)->where('fopenid','=',$userobj->openid)->count_all();
    ...
    public function action_order($tid) {
        ...
        $order = ORM::factory('qwt_ytytrade', $tid);
    ...
    public function action_top() {
        ...
        $this->template->title = '经销商排行';
        $this->template->content = View::factory($view)->bind('users', $users)->bind('result', $result);

        $user = ORM::factory('qwt_ytyqrcode', $this->uid);
        $top = $this->config['rank_yty'] ? $this->config['rank_yty'] : 20;

        $result['rank'] = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('paid', '>', $user->paid)->count_all()+1;

        $usersobj = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->order_by('paid', 'DESC')->limit($top)->find_all();
        foreach ($usersobj as $userobj) {
            $users[] = $userobj->as_array();
        }
    }
    ...
    public function action_customer($newadd='') {
        ...
        $user = ORM::factory('qwt_ytyqrcode', $this->uid);
         ...
         {
            $month = strtotime(date('Y-m-1'));
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_ytyqrcodes WHERE fopenid='$user->openid' and lv != 1 and jointime>='$month'")->execute()->as_array();
         }
         else
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM qwt_ytyqrcodes WHERE fopenid='$user->openid' and lv != 1 ")->execute()->as_array();

           if($newadd=='month')
           {
            $customer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('jointime','>=',$month)->where('lv','!=',1);
           }
           else
             $customer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('lv','!=',1);
         ...
         if($newadd=='month')
           {
            $totlecustomer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('lv','!=',1)->where('jointime','>=',$month)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
           }
         else
           $totlecustomer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','!=',1)->where('fopenid', '=', $user->openid)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
    ...
    public function action_customer1($newadd='') {
        ...
        $user = ORM::factory('qwt_ytyqrcode', $this->uid);
         ...
         if($newadd=='month')
           {
            $totlecustomer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user->openid)->where('jointime','>=',$month)->where('lv','=',1)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
           }
         else
           $totlecustomer=ORM::factory('qwt_ytyqrcode')->where('bid', '=', $this->bid)->where('lv','=',1)->where('fopenid', '=', $user->openid)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
    ...
    public function action_test(){
        ...
        require_once Kohana::find_file('vendor', 'weixin/inc');
        require_once Kohana::find_file('vendor', 'weixin/wechat.class');
        $bid = ORM::factory('qwt_ytylogin')->where('shopid','=',$kdt_id)->find()->id;
        $this->bid=$bid;
        $this->config = $config = ORM::factory('qwt_ytycfg')->getCfg($bid);
    ...
    private function tradeImport($trade, $bid, $client, $we, $config) {
        ...
        $qwt_ytytrade = ORM::factory('qwt_ytytrade')->where('tid', '=', $tid)->find();
        //跳过已导入订单
        if ($qwt_ytytrade->id) {
            //更新订单状态
            if ($qwt_ytytrade->status != $trade['status']) {
                $qwt_ytytrade->status = $trade['status'];
                $qwt_ytytrade->save();

                //echo "$tid status updated.\n";
            }
            Kohana::$log->add('ytytrade', print_r($qwt_ytytrade->id, true));
            //退款订单删返利
            if ($trade['status'] == 'TRADE_CLOSED'||$trade['status'] == 'TRADE_CLOSED_BY_USER'||$trade['refund_state'] != 'NO_REFUND'){
                Kohana::$log->add('ytytrade1', print_r($qwt_ytytrade->id, true));
                if($qwt_ytytrade->flag==1){
                    ORM::factory('qwt_ytyscore')->where('tid', '=', $qwt_ytytrade->id)->delete_all();
                    $fopenid=$qwt_ytytrade->fopenid;
                    $fuser=ORM::factory('qwt_ytyqrcode')->where('bid','=',$bid)->where('openid','=',$fopenid)->find();
                    $fuser->stocks->stockIn($fuser, $type=6,$qwt_ytytrade->money1,$qwt_ytytrade->qrcode->id,1);
                    if($config['money_arrived_tpl']){
                        $money1= $fuser->agent->stock-$qwt_ytytrade->money1;
                        $money2= $fuser->agent->stock;
                        $remark='您的客户'.$qwt_ytytrade->qrcode->nickname.'购买的'.$qwt_ytytrade->title.'已退货。退回给您'.$qwt_ytytrade->money1.'元进货额';
                       $this->stocksuccess($fuser->openid,$money1,$money2,$remark);
                    }else{
                        $msg['touser'] = $fuser->openid;
                        $msg['msgtype'] = 'text';
                        $msg['text']['content'] = '您的客户'.$qwt_ytytrade->qrcode->nickname.'购买的'.$qwt_ytytrade->title.'已退货。退回给您'.$qwt_ytytrade->money1.'元进货额';
                        $this->we->sendCustomMessage($msg);
                    }
                }
            }
        ...
        $qwt_ytyqrcode = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('openid', '=', $userinfo['weixin_openid'])->find();
        Kohana::$log->add('id', print_r($qwt_ytyqrcode->id, true));
        if (!$qwt_ytyqrcode->id) {
            //echo "$tid no OpenID pass.\n";
            return;
        }
        $trade['qid'] = $qwt_ytyqrcode->id;
        ...
        $rank=0;
        $fuser = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('openid', '=', $trade['openid'])->find();
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $goodidcof=ORM::factory('qwt_ytysetgood')->where('goodid','=',$goodid)->find();
            //按照分销商等级 设置比例
            if($fuser->lv==1){
        ...
        $qwt_ytytrade->values($trade);
        $qwt_ytytrade1 = ORM::factory('qwt_ytytrade')->where('tid', '=', $tid)->find();
        if($qwt_ytytrade1->id) return;
        $qwt_ytytrade->save();
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $qwt_ytyorder=ORM::factory('qwt_ytyorder')->where('bid','=',$bid)->where('tid','=',$tid)->where('goodid','=',$goodid)->find();
            if(!$qwt_ytyorder->id)//跳过已导入的order
            {
                $qwt_ytyorder->bid=$bid;
                $qwt_ytyorder->tid=$tid;
                $qwt_ytyorder->goodid=$goodid;
                $qwt_ytyorder->title=$title;
                $qwt_ytyorder->num=$num;
                $qwt_ytyorder->price=$price;
                $qwt_ytyorder->save();
            }
        }
        Kohana::$log->add('4444444', '444444');
        //删除重复返利记录
        ORM::factory('qwt_ytyscore')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 1)->delete_all();
        ORM::factory('qwt_ytyscore')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 2)->delete_all();
        ORM::factory('qwt_ytyscore')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 3)->delete_all();
        Kohana::$log->add('33333', '333333');
        $msg['msgtype'] = 'text';
        $title5=$config['title5'];
        $trade1=ORM::factory('qwt_ytytrade')->where('bid','=',$bid)->where('tid','=',$trade['tid'])->find();
        //订单上线返利
        if ($money1 > 0&&$qwt_ytyqrcode->lv!=1) {
            $fuser = ORM::factory('qwt_ytyqrcode')->where('bid', '=', $bid)->where('lv', '=', 1)->where('openid', '=', $qwt_ytyqrcode->fopenid)->find();
            if ($fuser->id) {
                //echo "$tid money1:$money1 \n";
                if($fuser->agent->stock>=$money1){
                    $trade1->flag=1;
                    $trade1->save();
                    $fuser->scores->scoreIn($fuser, 2, $money, $qwt_ytyqrcode->id, $qwt_ytytrade->id);
                    //发消息
                    $fuser->stocks->stockOut($fuser, $type=3, $money1, $qwt_ytyqrcode->id, 1);
                    $msg['touser'] = $fuser->openid;
                    $cksum = md5($msg['touser'].$config['appsecret'].date('Y-m'));
                    $url = $this->baseurl.'index/'. $bid .'?url=orders&cksum='. $cksum .'&openid='. base64_encode($msg['touser']);
                    $msg['text']['content'] = "您推荐的{$config['title1']}「{$qwt_ytyqrcode->nickname}」完成一笔订单！\n\n实付金额：$money {$title5}\n您扣除进货额：$money1 {$title5}\n获得佣金：$money{$title5}\n\n<a href=\"$url\">查看我的{$config['title5']}明细</a>";
                    ...
                    if($config['money_arrived_tpl']){
                        ...
                        $money2=$fuser->agent->stock;
                        $remark='扣除您的'.$qwt_ytytrade->money1.'元进货额。';
                       $this->stocksuccess($fuser->openid,$money1,$money2,$remark);
                    }else{
                        $msg['touser'] = $fuser->openid;
                        $msg['msgtype'] = 'text';
                        $msg['text']['content'] = '扣除您的'.$qwt_ytytrade->money1.'元进货额。';
                        $this->we->sendCustomMessage($msg);
                    }
                }else{
                    $msg['touser'] = $fuser->openid;
        ...
        }elseif ($money1 > 0&&$qwt_ytyqrcode->lv==1) {
            if ($qwt_ytyqrcode->id) {
                if($qwt_ytyqrcode->agent->stock>=$money1){
                    $trade1->fopenid=$qwt_ytyqrcode->openid;
                    $trade1->flag=1;
                    $trade1->save();
                    $qwt_ytyqrcode->scores->scoreIn($qwt_ytyqrcode, 2, $money, $qwt_ytyqrcode->id, $qwt_ytytrade->id);
                    //发消息
                    $qwt_ytyqrcode->stocks->stockOut($qwt_ytyqrcode, $type=3, $money1, $qwt_ytyqrcode->id, 1);
                    $msg['touser'] = $qwt_ytyqrcode->openid;
                    $cksum = md5($msg['touser'].$config['appsecret'].date('Y-m'));
                    $url = $this->baseurl.'index/'. $bid .'?url=orders&cksum='. $cksum .'&openid='. base64_encode($msg['touser']);
                    ...
                    if($config['money_arrived_tpl']){
                        $money1=$qwt_ytyqrcode->agent->stock+$money1;
                        $money2=$qwt_ytyqrcode->agent->stock;
                        $remark='扣除您的'.$qwt_ytytrade->money1.'元进货额。';
                       $this->stocksuccess($qwt_ytyqrcode->openid,$money1,$money2,$remark);
                    }else{
                        $msg['touser'] = $qwt_ytyqrcode->openid;
                        $msg['msgtype'] = 'text';
                        $msg['text']['content'] = '扣除您的'.$qwt_ytytrade->money1.'元进货额。';
    ...
    private function sendMoneyMessage($openid, $title, $money, $total, $url) {
        ...
        $tplmsg['data']['keyword2']['value'] = '提现';
        $tplmsg['data']['keyword3']['value'] = '提现';
    ...
    private function curl_post_ssl($url, $vars, $second=30, $aHeader=array(), $bid=0) {
        ...
        //证书分布式异步更新
        $file_cert = ORM::factory('qwt_ytycfg')->where('bid', '=', $bid)->where('key', '=', 'qwt_ytyfile_cert')->find();
        $file_key = ORM::factory('qwt_ytycfg')->where('bid', '=', $bid)->where('key', '=', 'qwt_ytyfile_key')->find();
