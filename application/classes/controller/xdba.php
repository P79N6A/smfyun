...
class Controller_xdba extends Controller_Base {

    public $template = 'weixin/xdb/tpl/atpl';
    public $pagesize = 20;
    //public $access_token;
    public $config;
    public $bid;
    ...
    public function before() {
        ...
        $this->bid = $_SESSION['xdba']['bid'];
        $this->config = $_SESSION['xdba']['config'];
        ...
        if (Request::instance()->action != 'login' && !$this->bid) {
            ...
            header('location:/xdba/login?from='.Request::instance()->action);
            exit;
        }
    }
    ...
    public function action_orders($action='', $id=0) {
        ...
        if ($action == 'ship' && $id) {
            ...
            if ($order->status == 0) {
                ...
                //有单号的情况
                 if ($_REQUEST['remark']) {
                    $_SESSION['xdba']['remark'] = $_REQUEST['remark'];
                    $order->remark = $_REQUEST['remark'];
                    $order->flag=$_REQUEST['flag'];
                    $order->save();
                }
                if ($_REQUEST['shiptype'] && $_REQUEST['shipcode']) {
                    $_SESSION['xdba']['shiptype'] = $_REQUEST['shiptype'];
                    $_SESSION['xdba']['shipcode'] = $_REQUEST['shipcode'];
        ...
        $this->template->content = View::factory('weixin/xdb/admin/orders')
            ->bind('pages', $pages)
            ->bind('result', $result)
            ->bind('config', $config)
    ...
    public function action_login() {
        $this->template = 'weixin/xdb/tpl/login';
        $this->before();
        ...
        if ($_POST['username'] && $_POST['password']) {
            ...
            if ($biz->id) {
                if ($biz->expiretime && strtotime($biz->expiretime) < time()) {
                    ...
                    $_SESSION['xdba']['bid'] = $biz->id;
                    $_SESSION['xdba']['user'] = $_POST['username'];
                    $_SESSION['xdba']['admin'] = $biz->admin; //超管
                    $_SESSION['xdba']['config'] = ORM::factory('wfb_cfg')->getCfg($biz->id);
        ...
        if ($_SESSION['xdba']['bid']) {
            if (!$_GET['from']) $_GET['from'] = 'orders';
            header('location:/xdba/'.$_GET['from']);
            exit;
        }
    }
