...
class Controller_qwtdka extends Controller_Base {
    public $template = 'weixin/smfyun/dka/tpl/ftpl';
    ...
    var $baseurl = 'http://dka.smfyun.com/qwtdka/';
    public function before() {
        ...
        if (!$_GET['openid']) {
            if (!$_SESSION['qwtdka']['bid']) die('页面已过期。');
            if (!$_SESSION['qwtdka']['openid']) die('Access Deined..');
        }
        $biz = ORM::factory('qwt_login')->where('id','=',$_SESSION['qwtdka']['bid'])->find();
        $this->config = $_SESSION['qwtdka']['config'];
        $this->openid = $_SESSION['qwtdka']['openid'];
        $this->bid = $_SESSION['qwtdka']['bid'];
        $this->uid = $_SESSION['qwtdka']['uid'];
        $this->yzaccess_token=ORM::factory('qwt_login')->where('id', '=', $this->bid)->find()->yzaccess_token;
        // if (stripos($_SERVER["HTTP_USER_AGENT"], 'MicroMess') === false && !$_SESSION['qwta']['bid']) die('请通过微信访问。');
        $sname = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','scorename')->find()->value;
        if($sname){
        ...
        $config['pstatus'] = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','pstatus')->find()->value;
        $config['ptime'] = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','drawtime')->find()->value;
        ...
        if ($_GET['debug']) print_r($_SESSION['qwtdka']);
    }

    public function after() {
        // if (Request::instance()->action == 'images') return;

        $user = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find()->as_array();
        $user['follows'] = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('fopenid', '=', $user['openid'])->count_all();
...
//入口
    public function action_index($bid) {
        ...
        if (!$_GET['openid']) $_SESSION['qwtdka'] = NULL;

        //OpenId 解密
        if ($config && $_GET['openid']) {
            $openid = base64_decode($_GET['openid']);
            if ($_GET['cksum'] != md5($openid.$config['appsecret'].date('Y-m-d'))) {
                $_SESSION['qwtdka'] = NULL;
                die('Access Deined!');
            }

            $userobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
            $userobj->ip = Request::$client_ip;
            $userobj->save();

            $_SESSION['qwtdka']['config'] = $config;
            $_SESSION['qwtdka']['openid'] = $openid;
            $_SESSION['qwtdka']['bid'] = $bid;
            $_SESSION['qwtdka']['uid'] = $userobj->id;

            if ($bid == 2) {
                // print_r($_SESSION);exit;
            }
            Request::instance()->redirect('/qwtdka/'.$_GET['url']);
        }
    }
    //订单宝部分
    public function action_home() {
        $this->template = 'weixin/smfyun/dka/tpl/fftpl';
        self::before();
        $view = "weixin/smfyun/dka/home";
        ...
        $userobj = ORM::factory('qwt_dkaqrcode', $this->uid);

        //新用户关注收益
        // if ($this->config['money_init'] > 0 && ORM::factory('qwt_dkascore')->where('qid', '=', $this->uid)->where('type', '=', 6)->count_all() == 0) {
        ...
        $result['follows_month'] = ORM::factory('qwt_dkaqrcode')->where('fopenid', '=', $this->openid)->where('jointime', '>', $month)->count_all();
    ...
    public function action_money($out=0, $cksum='') {
        ...
        if ($out == 1 && $cksum_flag == true && ($MONEY >= $this->config['money_out']/100) ) {
            ...
            if ($result_m['result_code'] == 'SUCCESS') {
                ...
                // print_r($result);exit;
                Kohana::$log->add("weixin_qwtdka:$bid:money", print_r($result, true));
    ...
    public function action_ddtop() {
        $this->template = 'weixin/smfyun/dka/tpl/fftpl';
        self::before();
        $view = 'weixin/smfyun/dka/ddtop';

        $this->template->title = '业绩排行';
        $this->template->content = View::factory($view)->bind('users', $users)->bind('result', $result);

        $user = ORM::factory('qwt_dkaqrcode', $this->uid);
        $top = $this->config['rank_qwtdka'] ? $this->config['rank_qwtdka'] : 10;

        //$result['rank'] = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('id2', '>', 0)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('paid', '>=', $user->paid)->count_all();
        $rank1 = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('id2', '>', 0)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('paid', '>', $user->paid)->count_all();
        $rank2 = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('id2', '<', $user->id2)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('paid', '=', $user->paid)->count_all();
        $result['rank'] = $rank1+$rank2+1;
        $usersobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('paid', 'DESC')->order_by('id2', 'asc')->limit($top)->find_all();
    ...
    public function action_ddscore($type=0) {
        $this->template = 'weixin/smfyun/dka/tpl/fftpl';
        self::before();
        $view = "weixin/smfyun/dka/ddscores";
        $userobj = ORM::factory('qwt_dkaqrcode', $this->uid);
    ...
    public function action_ddorders() {
        $this->template = 'weixin/smfyun/dka/tpl/fftpl';
        self::before();
        $view = "weixin/smfyun/dka/ddorders";
        $userobj = ORM::factory('qwt_dkaqrcode', $this->uid);
    ...
    public function action_ddorder($tid) {
        $this->template = 'weixin/smfyun/dka/tpl/fftpl';
        self::before();
        $view = "weixin/smfyun/dka/ddorder";

        $order = ORM::factory('qwt_dkatrade', $tid);
    ...
    private function tradeImport($trade, $bid, $client, $wx, $config) {
        ...
        $qwt_dkatrade = ORM::factory('qwt_dkatrade')->where('tid', '=', $tid)->find();

        //跳过已导入订单
        if ($qwt_dkatrade->id) {

            //更新订单状态
            if ($qwt_dkatrade->status != $trade['status']) {
                $qwt_dkatrade->status = $trade['status'];
                $qwt_dkatrade->save();
            ...
            //退款订单删返利
            if ($trade['status'] == 'TRADE_CLOSED') ORM::factory('qwt_dkadetail')->where('tid', '=', $qwt_dkatrade->id)->delete_all();
            if ($trade['status'] == 'TRADE_CLOSED_BY_USER') ORM::factory('qwt_dkadetail')->where('tid', '=', $qwt_dkatrade->id)->delete_all();
            if ($trade['refund_state'] != 'NO_REFUND') ORM::factory('qwt_dkadetail')->where('tid', '=', $qwt_dkatrade->id)->delete_all();
        ...
        //只处理有下线的订单
        $qwt_dkaqrcode = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $userinfo['weixin_openid'])->find();
        ...
        //是：则判断关注后的订单都有收益
        //否：判断生成海报后的订单才算收益
        $fromtime = $config['haibao_needpay'] ? $qwt_dkaqrcode->subscribe : $qwt_dkaqrcode->jointime;
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $goodidcof=ORM::factory('qwt_dkasetgood')->where('goodid','=',$goodid)->find();
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $num=$order['num'];
            $price=$order['payment'];
            $qwt_dkaorder1=ORM::factory('qwt_dkaorder1')->where('bid','=',$bid)->where('tid','=',$tid)->where('goodid','=',$goodid)->find();
            if(!$qwt_dkaorder1->id)//跳过已导入的order
            {
                $qwt_dkaorder1->bid=$bid;
                $qwt_dkaorder1->tid=$tid;
                $qwt_dkaorder1->goodid=$goodid;
                $qwt_dkaorder1->title=$title;
                $qwt_dkaorder1->num=$num;
                $qwt_dkaorder1->price=$price;
                $qwt_dkaorder1->save();
            }
        }
        echo "aa<br>";
        ...
        //删除重复返利记录
        ORM::factory('qwt_dkadetail')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 1)->delete_all();
        ORM::factory('qwt_dkadetail')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 2)->delete_all();
        ORM::factory('qwt_dkadetail')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 3)->delete_all();
        ...
        if ($money0 > 0) {
            ...
            $qwt_dkaqrcode->details->cashIn($qwt_dkaqrcode, 1, $money0, 0, $qwt_dkatrade->id);

            //发消息
            $msg['touser'] = $qwt_dkaqrcode->openid;
            ...
            if ($config['msg_score_tpl'])
                $wx_result = $this->sendScoreMessage($msg['touser'], '购买返利', $money0, $qwt_dkaqrcode->cash, $url);
            else
        ...
        //订单上线返利
        if ($money1 > 0) {
            $fuser = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $qwt_dkaqrcode->fopenid)->find();
            if ($fuser->id) {
                echo "$tid money1:$money1 \n";
                $fuser->details->cashIn($fuser, 2, $money1, $qwt_dkaqrcode->id, $qwt_dkatrade->id);
                ...
                $msg['text']['content'] = "您推荐的{$config['title1']}「{$qwt_dkaqrcode->nickname}」完成一笔订单！\n\n实付金额：$money\n推广佣金：$money1\n\n<a href=\"$url\">查看我的收益明细</a>";

                if ($config['msg_score_tpl'])
                    $wx_result = $this->sendScoreMessage($msg['touser'], '好友购买返利', $money1, $fuser->cash, $url);
                else
                    $wx_result = $wx->sendCustomMessage($msg);
            }
        }
        ...
        if ($money2 > 0 && $fuser->fopenid) {
            $ffuser = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $fuser->fopenid)->find();
            if ($ffuser->id) {
                echo "$tid money2:$money2 \n";
                $ffuser->details->cashIn($ffuser, 3, $money2, $fuser->id, $qwt_dkatrade->id);
                ...
                if ($config['msg_score_tpl'])
                    $wx_result = $this->sendScoreMessage($msg['touser'], '好友的好友购买返利', $money2, $ffuser->cash, $url);
                else
                    $wx_result = $wx->sendCustomMessage($msg);
            }
        }
    ...
    private function sendScoreMessage($openid, $title, $score, $total, $url, $remark='干的漂亮，请继续加油哦！') {
        ...
        // Kohana::$log->add("weixin_qwtdka:$bid:tplmsg", print_r($tplmsg, true));
    ...
    private function sendMoney($userobj, $money) {
        ...
        if (!$this->wx) {
            ...
            $options['encodingaeskey'] = $this->encodingAesKey;
            $options['appid'] = ORM::factory('qwt_login')->where('id','=',$bid)->find()->appid;
            $this->wx = $wx = new Wxoauth($bid,$options);
        }
        ...
        // Kohana::$log->add('weixin_qwtdka:hongbaopost', print_r($data, true));
        ...
        // Kohana::$log->add('weixin_qwtdka:hongbaoresult', print_r($result, true));
        return $result;
    }
    ...
    private function curl_post_ssl($url, $vars, $second=30, $aHeader=array(), $bid=0) {
        ...
        $cert_file = DOCROOT."qwtdka/tmp/$bid/cert.{$config['appsecret']}.pem";
        $key_file = DOCROOT."qwtdka/tmp/$bid/key.{$config['appsecret']}.pem";
        ...
        // Kohana::$log->add("weixin_qwtdka:$bid:curl_post_ssl:cert_file", $cert_file);
    ...
    public function action_top() {
        ...
        //计算排名
        $user = ORM::factory('qwt_dkaqrcode', $this->uid)->as_array();

        //飘飘管理员
        //if ($user['openid'] == 'oDB2TjizEcKT89gcaaSjI137TK1g') $top = 100;
        //if ($user['openid'] == 'oDB2TjvzpkBAoc2wE2dWOKk1DrE4') $top = 100;

        $rankkey = "qwtdka:rank3:{$this->bid}:{$this->openid}:$top";
        $result['rank'] = $mem->get($rankkey);
        if (!$result['rank']) {
            $result['rank'] = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('score', '>', $user['score'])->count_all()+1;
            $mem->set($rankkey, $result['rank'], 600);
        }

        $topkey = "qwtdka:top3:{$this->bid}:$top";
        $users = $mem->get($topkey);
        if (!$users) {
            $usersobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('score', 'DESC')->limit($top)->find_all();
            foreach ($usersobj as $userobj) {
                $users[] = $userobj->as_array();
            }
    ...
    public function action_score() {
        $this->template = 'weixin/smfyun/dka/tpl/ftpl';
        self::before();
        $view = "weixin/smfyun/dka/scores";
        ...
        //查询积分
        if (time() % 10 == 0) {
            $userobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find();
            $userobj->score = $userobj->scores->select(array('SUM("score")', 'total_score'))->find()->total_score;
            $userobj->save();
            // echo "reCount score.";
        }

        $scores = ORM::factory('qwt_dkaqrcode', $this->uid)->scores->order_by('lastupdate', 'DESC')->find_all();
    }

    //奖品列表
    public function action_items() {
        //$mem = Cache::instance('memcache');
        $view = "weixin/smfyun/dka/items";



        $obj = ORM::factory('qwt_dkaitem')->where('bid', '=', $this->bid)->where('show', '=', 1)->order_by('pri', 'DESC')->find_all();
            foreach($obj as $i) $items[] = $i->as_array();
        $day_limit = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','day_limit')->find()->value;
        $qid = ORM::factory('qwt_dkaqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find()->id;
        //$day = strtotime(date("Y-m-d"));
        $times = ORM::factory('qwt_dkascore')->where('bid','=',$this->bid)->where('type','=',4)->where('qid','=',$qid)->count_all();
    ...
    public function action_item($iid) {
        $this->template = 'tpl/blank';
        self::before();
        $view="weixin/smfyun/dka/xiangqing";
        $item = ORM::factory('qwt_dkaitem')->where('bid', '=', $this->bid)->where('id', '=', $iid)->find();
        $day_limit = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','day_limit')->find()->value;
        $qid = ORM::factory('qwt_dkaqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find()->id;
        //$day = strtotime(date("Y-m-d"));
        $times = ORM::factory('qwt_dkascore')->where('bid','=',$this->bid)->where('type','=',4)->where('qid','=',$qid)->count_all();
        ...
        $user2 = ORM::factory('qwt_dkaqrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find()->as_array();
    ...
    public function action_neworder($iid,$flag=0) {
        $view = "weixin/smfyun/dka/neworder";
        ...
        $item = ORM::factory('qwt_dkaitem', $iid);
        $userobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find();
        if (!$item->id || $item->bid != $this->bid) Request::instance()->redirect('/qwtdka/items');
        ...
        if($kill==1){
            ...
            //2.是否限购
            if ($item->limit > 0) {
                $limit = ORM::factory('qwt_dkaorder')->where('qid', '=', $userobj->id)->where('iid', '=', $iid)->count_all();
            ...
            if ($this->config['risk_level1'] > 0 && $this->config['risk_level2'] > 0) {
                ...
                $count2 = ORM::factory('qwt_dkaqrcode', $userobj->id)->scores->where('type', '=', 2)->count_all();
                //$count3 = ORM::factory('qwt_dkaqrcode', $userobj->id)->scores->where('type', '=', 3)->count_all();
                //用是否生成海报判断下线数量
                $count3 = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $userobj->bid)->where('fopenid', '=', $userobj->openid)->where('ticket', '<>', '')->count_all();
        ...
        if ( ($_POST['data']['name'] && $_POST['data']['address'] && $_POST['data']['tel']) || ($_POST['url'] && $item->url&&$_POST['data']['type']!=5&&$_POST['data']['type']!=6)) {
            $order = ORM::factory('qwt_dkaorder');
            ...
            if($kill== 1){
                $order->score = ORM::factory('qwt_dkacfg')->where('bid','=',$this->bid)->where('key','=','killScore')->find()->value;
            }
            //虚拟产品
            if ($item->url) {
                ...
                if ($url == 'http'){
                    ...
                    $order->url = '/qwtdka/ticket/'.$item->url;
                }
        ...
        if ($_POST['data']['type']==4) {
            ...
            $order->status = 1;
                ...
                $tempmoney=ORM::factory("qwt_dkaitem")->where("id","=",$iid)->find()->price;
        ...
        if ($_POST['data']['type']==6){
            ...
            //gift
            // $wx['appid'] = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key','=','yz_appid')->find()->value;
            // $wx['appsecret'] = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key','=','yz_appsecert')->find()->value;
            $oid = ORM::factory('qwt_dkaitem')->where('bid','=',$this->bid)->where('id','=',$iid)->find()->url; //? iid
            if($this->yzaccess_token){
                $client = new YZTokenClient($this->yzaccess_token);
                ...
                if($present_id==$oid){//找到指定赠品
                    ...
                    if($results['response']['is_success']==true){
                       ...
                       // exit;
                        ...
                        echo $results['error_response']['msg'];
                        exit;
                    }
        ...
        //自动填写旧地址
        $old_order = ORM::factory('qwt_dkaorder')->where('qid', '=', $userobj->id)->order_by('id', 'DESC')->find();
    ...
    public function action_tplcheck(){
        Kohana::$log->add('qwtdka:tplcheck', '进入tplcheck1');//
        // ini_set('max_execution_time', 0);
        set_time_limit(0);
        Kohana::$log->add('qwtdka:tplcheck', '进入tplcheck2');//
        Kohana::$log->add('qwtdka:tplcheck', time());
        $busers = ORM::factory('qwt_buy')->where('switch','=',1)->where('expiretime','>',time())->where('status','=',1)->where('iid', '=', 13)->find_all();

        foreach ($busers as $k => $v) {
            $bid = $v->bid;
            $url = 'http://'.$_SERVER['HTTP_HOST'].'/smfyun/qwt_dka/'.$bid;
            echo 'bid:'.$bid.'<br>';
            $usertime = date('H');//服务器时间

            $btime = ORM::factory('qwt_dkacfg')->where('bid','=',$bid)->where('key','=','start')->find()->value;
            $tplid = ORM::factory('qwt_dkacfg')->where('bid','=',$bid)->where('key','=','tplid')->find()->value;
            echo '商家时间：'.$btime.'<br>';
            if($btime==$usertime&&$tplid){//服务器时间和商家设定时间一直并且模板消息存在
                ...
                $ops = ORM::factory('qwt_dkaqrcode')->where('bid','=',$bid)->where('dka_join','!=',0)->find_all()->as_array();//找出此商家下的 加入了打卡的用户
                $tplid = ORM::factory('qwt_dkacfg')->where('bid','=',$bid)->where('key','=','tplid')->find()->value;
                // echo $bid;
                // echo '<pre>';
                ...
                foreach ($ops as $op) {
                    ...
                    $tplmsg['url'] = 'http://'.$_SERVER['HTTP_HOST'].'/smfyun/user_snsapi_base/'.$bid.'/dka/dka';
                    // echo $tplmsg['url'].'<br>';

                    $tplmsg['data']['first']['value'] = '您好，您有一条打卡提醒';
                    $tplmsg['data']['first']['color'] = '#999999';

                    $tplmsg['data']['keyword1']['value'] = $op->nickname;
                    ...
                    $wx->sendTemplateMessage($tplmsg);
                    // var_dump($tplmsg);
                }
                // echo $res;
            }else{
                Kohana::$log->add('qwtdka:tplcheck:'.$bid, '时间不一致或tplid不存在');//
                echo '时间不一致或tplid不存在<br>';
            }
        }
        exit;
    }
    public function action_dka(){
        ...
        $this->template = 'tpl/blank';
        self::before();
        $view = "weixin/smfyun/dka/dka";
        $bid = $this->bid;
        $openid = $this->openid;
        $Bname = ORM::factory('qwt_login')->where('id', '=', $bid)->find()->user;
        $userobj = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=',$openid)->find();
        $bidname = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'name')->find()->value;
        //递归
        $qid = $userobj->id;
        $conday = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'con_day')->find()->value;//商家定义签到天数上限，超出部分按照每天积分计算
        $userconday = 1;//用户起始进入签到天数
        $rewardpoint = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'reward')->find()->value; //商家自定义连续签到$countday之后一次性增加的积分
        $addpoint = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'add_point')->find()->value;// 商家自定义连续签到$countday之后每天增加的积分
        $point = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'basic_point')->find()->value;//商家自定义每天签到增加的积分
        $explain = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'explain')->find()->value;
        $today = date('y-m-d',time());
        $rangestart = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'start')->find()->value;//打卡开始时间
        $rangeend = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'end')->find()->value;//打卡关闭时间
        ...
        if (isset($_POST['exit'])){
            ...
            ->where('id', '=', $qid)
            ->execute();
            DB::update(ORM::factory('qwt_dkascore')->table_name())
            ->set(array('date' => 0))
            ->where('bid', '=', $bid)
            ->where('qid', '=', $qid)
            ->where('date', '!=', $today)
        ...
        if (isset($_GET['dka'])){
            if(ORM::factory('qwt_dkascore')->where('bid', '=', $bid)->where('qid', '=', $qid)->where('date','=',$today)->where_open()->where('type', '=', 5)->or_where('type', '=', 6)->or_where('type', '=', 7)->where_close()->find()->date){
                ...
                $nstart = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'nstart')->find()->value;//正常分开始时间
                $nend = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'nend')->find()->value;//正常分结束时间
                ...
                if($countday<$conday){
                    ORM::factory('qwt_dkascore')->scoreIn($userobj, 5, $point);//5表示常规签到
                    $content = '今日签到增加积分'.$point.'分,连续签到'.$conday.'天可获得'.$rewardpoint.'分奖励哟';
                }else if($countday>$conday){
                    $add = $addpoint;
                    ORM::factory('qwt_dkascore')->scoreIn($userobj, 6, $add);//6表示超过的连续签到
                    $content = '连续签到'.$countday.'天增加积分'.$add.'分';
                }else if($countday=$conday){
                    $add = $rewardpoint+$point;
                    ORM::factory('qwt_dkascore')->scoreIn($userobj, 7, $add);//7表示恰好当天完成的连续签到
                    $content = '完成连续签到'.$conday.'天,一次性奖励'.$add.'积分,继续连续签到,每天可获得'.$addpoint.'分奖励哟';
                }
                $suserobj1 = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=',$userobj->fopenid)->find();//当前用户上级 王旭文
                if($suserobj1->openid){
                    $goal = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'goal')->find()->value;
                    ORM::factory('qwt_dkascore')->scoreIn($suserobj1, 2, $goal);//2表示下线打卡奖励
                    ...
                    $wx->sendCustomMessage($msg);
                    $suserobj2 = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=',$suserobj1->fopenid)->find();//上上级用户 李静
                    if($suserobj2->openid){
                        $goal2 = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'goal2')->find()->value;
                        ORM::factory('qwt_dkascore')->scoreIn($suserobj2, 3, $goal2);//3表示下线打卡奖励
        ...
        if(isset($_GET['egg'])){
            if(ORM::factory('qwt_dkascore')->where('bid', '=', $bid)->where('qid', '=', $qid)->where('date', '=', $today)->where('type', '=', 8)->find()->date){
                $content = '对不起你今天已经砸过了哟';
            }else{
                $pointstart = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'eggstart')->find()->value;
                $pointend = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'eggsend')->find()->value;
                $pointchance = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'eggchance')->find()->value;
                $rand = rand(1,100);
                if($rand<=$pointchance){
                    $point = rand($pointstart,$pointend);
                    ORM::factory('qwt_dkascore')->scoreIn($userobj, 8, $point);//8表示砸金蛋奖励
            ...
            $con=$content;
            $score=ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $this->openid)->find()->score;
            $arr=array('con'=>$con,'score'=>$score);
            echo json_encode($arr);
            //echo $content;
            exit();
        ...
        if(isset($_GET['shake'])){
            if(ORM::factory('qwt_dkascore')->where('bid', '=', $bid)->where('qid', '=', $qid)->where('date', '=', $today)->where('type', '=', 9)->find()->date){
                $content = '对不起你今天已经摇过了哟';
            }else{
                $pointstart = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'shakestart')->find()->value;
                $pointend = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'shakeend')->find()->value;
                $pointchance = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'shakechance')->find()->value;
                $rand = rand(1,100);
                if($rand<=$pointchance){
                    $point = rand($pointstart,$pointend);
                    ORM::factory('qwt_dkascore')->scoreIn($userobj, 9, $point);//9表示摇一摇奖励
                    $content = '恭喜你获得'.$point.'积分';
                }else{
                    $content = '很遗憾您没有中奖，请明天再来哟';
                }
        ...
        $result['qwtdka'] = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'dka')->find()->id;
        $result['point'] = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find()->score;
        $headimg = ORM::factory('qwt_dkaqrcode')->where('dka_join','!=',0)->where('bid', '=', $bid)->order_by('id', 'DESC')->limit(8)->find_all();
        $bname = ORM::factory('qwt_login')->where('id', '=', $bid)->find()->name;
        $num = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $bid)->where('dka_join', '!=', 0)->count_all();
        $joinstatus = ORM::factory('qwt_dkaqrcode')->where('bid','=',$bid)->where('id', '=', $qid)->find()->dka_join;
        if(!$joinstatus==0){
            $joinstatus=1;
        }else{
            $joinstatus=0;
        }
        $egg = ORM::factory('qwt_dkaqrcode')->where('bid','=',$bid)->where('id', '=', $qid)->find();
        ...
        $config['eggst'] = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'eggst')->find()->value;
        $config['shakest'] = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', 'shakest')->find()->value;
    ...
    public function action_check_location(){
        if (isset($_GET['x'])){
          ...
          echo $content;
          $area = ORM::factory('qwt_dkaqrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find();
          $area->area = $content;
          $area->save();
          exit;
        }
        ...
        $view = "weixin/smfyun/dka/check_location";
        ...
        $wx = new Wxoauth($bid,$options);
        //$wx = new Wechat($wx);
        $count = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'count')->find()->value;
        for ($i=1; $i <=$count ; $i++) {
            $pro[$i] = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'pro'.$i)->find()->value;
            $city[$i] = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'city'.$i)->find()->value;
            $dis[$i] = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'dis'.$i)->find()->value;
            $p_location[$i]= $pro[$i].$city[$i].$dis[$i];
        }
        // $pro = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'pro')->find()->value;
        // $city = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'city')->find()->value;
        // $dis = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'dis')->find()->value;
        $info = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'info')->find()->value;
        $reply = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'reply')->find()->value;
        $isreply = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'isreply')->find()->value;
    ...
    public function action_ticket($cardId) {
        ...
        Kohana::$log->add("qwtdkabid",print_r($this->bid,true));
        $this->template = 'tpl/blank';
        self::before();
        $view = "weixin/smfyun/dka/ticket";
        // $wx['appid'] = $this->config['appid'];
        // $wx['appsecret'] = $this->config['appsecret'];
        $callback_url = 'http://'.$_SERVER["HTTP_HOST"].$_SERVER["REQUEST_URI"];
        if ($_GET['url']) $callback_url = urldecode($_GET['url']);
        Kohana::$log->add("qwtdkacallback",print_r($callback_url,true));
        //$wx = new Wechat($wx);
        $wx = new Wxoauth($this->bid,$options);
        // echo "<pre>";
        // var_dump($callback_url);
        $jsapi = $wx->getJsSign($callback_url);
        Kohana::$log->add("qwtdka1",print_r($jsapi,true));
        // var_dump($jsapi);
        $ticket = $wx->getJsCardTicket();
        Kohana::$log->add("qwtdka2",print_r($ticket,true));
        // var_dump($ticket);
        $sign = $wx->getTicketSignature(array($jsapi["timestamp"], $ticket, $cardId));
        Kohana::$log->add("qwtdka3",print_r($sign,true));
        // var_dump($sign);
        // echo "</pre>";
        $this->template->content = View::factory($view)
                ->bind('cardId', $cardId)
    ...
    public function action_orders() {
        $view = "weixin/smfyun/dka/orders";

        $this->template->title = '我的订单';
        $this->template->content = View::factory($view)->bind('orders', $orders);
        $orders = ORM::factory('qwt_dkaorder')->where('bid', '=', $this->bid)->where('qid', '=', $this->uid)->order_by('id', 'DESC')->find_all();
    }
    ...
    public function action_draw(){
        ...
        //获得奖品设置
        $cfg_score = ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'killScore')->find();
        $cfg_explain=ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'drawexplain')->find();
        $cfg_drawtime=ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'drawtime')->find();
        $cfg_limittime=ORM::factory('qwt_dkacfg')->where('bid', '=', $this->bid)->where('key', '=', 'limitTime')->find();
        $prizeSet = ORM::factory('qwt_dkaprize')->where('bid', '=', $this->bid)->find_all()->as_array();
        $itemset = ORM::factory('qwt_dkaitem')->where('bid', '=', $this->bid)->find_all()->as_array();
        $userdrawtime = ORM::factory('qwt_dkascore')->where('bid', '=', $this->bid)->where('qid', '=', $userobj->id)->where('type', '=', 10)->where('lastupdate', '>', mktime(0,0,0,date('m'),date('d'),date('Y')))->count_all();
        ...
        $prize_1 = ORM::factory('qwt_dkaprize')->where('bid', '=', $this->bid)->where('type', '=', 1)->find()->probability;
        $prize_2 = ORM::factory('qwt_dkaprize')->where('bid', '=', $this->bid)->where('type', '=', 2)->find()->probability;
        $prize_3 = ORM::factory('qwt_dkaprize')->where('bid', '=', $this->bid)->where('type', '=', 3)->find()->probability;
        $prize_4 = ORM::factory('qwt_dkaprize')->where('bid', '=', $this->bid)->where('type', '=', 4)->find()->probability;
        ...
        function getiid($angle,$bid){
            if($angle >= 40 && $angle <= 50){
                    //一等奖
                $result['ptype']=1;
                $result['iid'] = ORM::factory('qwt_dkaprize')->where('bid', '=', $bid)->where('type', '=', 1)->find()->iid;
            }
            else if($angle>= 130 && $angle <= 140){
                //二等奖
                $result['ptype']=2;
                $result['iid'] = ORM::factory('qwt_dkaprize')->where('bid', '=', $bid)->where('type', '=', 2)->find()->iid;
            }
            else if($angle >= 220 && $angle <= 230){
                //三等奖
                $result['ptype']=3;
                $result['iid'] = ORM::factory('qwt_dkaprize')->where('bid', '=', $bid)->where('type', '=', 3)->find()->iid;
            }
            else if($angle >= 310 && $angle <= 315){
                //四等奖
                $result['ptype']=4;
                $result['iid'] = ORM::factory('qwt_dkaprize')->where('bid', '=', $bid)->where('type', '=', 4)->find()->iid;
            }else{
                //未中奖
                $result['ptype']=0;
                $result['iid'] = '0';
        ...
        if($_POST['flag']==1){
            ...
            if($res['limittime']-$userdrawtime>0){
                ...
                if($userobj->score-$res['killscore']>0){// 判断积分够不够
                    $res[1]=true;
                    //更新积分详情表，将积分消耗记录插入
                    $score = ORM::factory('qwt_dkascore');
                    $score->bid = $this->bid;
