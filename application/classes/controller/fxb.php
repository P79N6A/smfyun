...
class Controller_fxb extends Controller_Base {
    public $template = 'weixin/fxb/tpl/fftpl';
    public $access_token;
    public $config;
    public $openid;
    public $bid;
    ...
    public function before() {
        ...
        $biz = ORM::factory('fxb_login')->where('id','=',$_SESSION['fxb']['bid'])->find();
        if ($biz->expiretime && strtotime($biz->expiretime)+86400 < time()) die('您的账号已过期');
        $this->config = $_SESSION['fxb']['config'];
        $this->openid = $_SESSION['fxb']['openid'];
        $this->bid = $_SESSION['fxb']['bid'];
        $this->uid = $_SESSION['fxb']['uid'];
        $this->access_token = $_SESSION['fxb']['access_token'];
        if ($_GET['debug']) print_r($_SESSION['fxb']);
        //只能通过微信打开
        if (stripos($_SERVER["HTTP_USER_AGENT"], 'MicroMess') === false && !$_SESSION['fxba']['bid']) die('请通过微信访问。');
    }

    public function after() {
        $user = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find()->as_array();

         $firstchild=DB::query(Database::SELECT,"SELECT openid FROM fxb_qrcodes WHERE fopenid='$this->openid'")->execute()->as_array();
         ...
         {
              $tempdata = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->find_all()->as_array();
        ...
        $customer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid','IN',$tempiid)->or_where('fopenid', '=', $this->openid)->order_by('paid', 'DESC');
        $user['follows'] =$customer->count_all();


        $month = strtotime(date('Y-m-1'));
        $firstchild=DB::query(Database::SELECT,"SELECT openid FROM fxb_qrcodes WHERE fopenid='$this->openid' and jointime>='$month'")->execute()->as_array();
        ...
        $user['follows_month']=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid','IN',$tempiid)->or_where('fopenid', '=', $this->openid)->where('jointime','>=',$month)->count_all();
        $user['trades'] = ORM::factory('fxb_score')->where('qid', '=', $user['id'])->where('type', 'IN', array(2,3))->count_all();
    ...
    public function action_index($bid) {
        ...
        if ($config && $_GET['openid']) {
            ...
            $userobj = ORM::factory('fxb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $openid)->find();
            ...
            $_SESSION['fxb']['access_token'] =$this->access_token;
            Request::instance()->redirect('/fxb/'.$_GET['url']);
        }
    }
    ...
    public function action_index_oauth($bid, $url='top') {
        ...
        Request::instance()->redirect('/fxb/'.$url);
    }

    //默认页面
    public function action_home() {
        $view = "weixin/fxb/home";
        ...
        $userobj = ORM::factory('fxb_qrcode', $this->uid);

        //新用户关注收益
        // if ($this->config['money_init'] > 0 && ORM::factory('fxb_score')->where('qid', '=', $this->uid)->where('type', '=', 6)->count_all() == 0) {
    ...
    public function action_items() {
        ...
        $userobj->shscore = $result['shscore'] = $userobj->shscores->select(array('SUM("score")', 'total_score'))->find()->total_score;
        if ($userobj->id) $userobj->save();
        $items = ORM::factory('fxb_item')->where('bid', '=', $this->bid)->where('show', '=', 1)->order_by('pri', 'DESC')->find_all();
    ...
    public function action_item($iid){
        ...
        self::before();
        $view="weixin/fxb/xiangqing";
        $item = ORM::factory('fxb_item')->where('bid', '=', $this->bid)->where('id', '=', $iid)->find();
        $day_limit = ORM::factory('fxb_cfg')->where('bid','=',$this->bid)->where('key','=','day_limit')->find()->value;
        $qid = ORM::factory('fxb_qrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find()->id;
        //$day = strtotime(date("Y-m-d"));
        $times = ORM::factory('fxb_shscore')->where('bid','=',$this->bid)->where('type','=',4)->where('qid','=',$qid)->count_all();
        if($times>=$day_limit&&$day_limit!=0){
            $dlimit = 1;
        }else{
            $dlimit = 2;
        }
        $user2 = ORM::factory('fxb_qrcode')->where('bid','=',$this->bid)->where('openid','=',$this->openid)->find()->as_array();
    ...
    public function action_neworder($iid) {
        ...
        //1.积分够不
        $userobj = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('openid', '=', $this->openid)->find();
        if ($item->score > $userobj->shscore) die("该奖品需要 {$item->score} {$this->scorename}，您只有 {$userobj->shscore} {$this->scorename}。");

        //2.是否限购
        if ($item->limit > 0) {
            $limit = ORM::factory('fxb_shorder')->where('qid', '=', $userobj->id)->where('iid', '=', $iid)->count_all();
        ...
        if ($this->config['risk_level1'] > 0 && $this->config['risk_level2'] > 0) {
            ...
            $count2 = ORM::factory('fxb_qrcode', $userobj->id)->scores->where('type', '=', 2)->count_all();
            $count3 = ORM::factory('fxb_qrcode', $userobj->id)->scores->where('type', '=', 3)->count_all();
            //用是否生成海报判断下线数量 （生成海报不一定有积分） 下面计算的是 下线中 没有生成海报的数量
            //$count3 = ORM::factory('fxb_qrcode')->where('bid', '=', $userobj->bid)->where('fopenid', '=', $userobj->openid)->where('ticket', '<>', '')->count_all();
        ...
        if ($_POST['data']['type']==4&&Security::check($_POST['csrf'])==1) {
            ...
            if($this->config['hb_check']==1){
               ...
               $goal_url = '/fxb/orders';
                ...
                //发红包
                $tempname=ORM::factory("fxb_login")->where("id","=",$this->bid)->find()->user;
                $tempmoney=ORM::factory("fxb_item")->where("id","=",$iid)->find()->price;
        ...
        if ($_POST['data']['type']==5){
            ...
            //gift
            //$wx['appid'] = ORM::factory('fxb_cfg')->where('bid', '=', $this->bid)->where('key','=','yz_appid')->find()->value;
            //$wx['appsecret'] = ORM::factory('fxb_cfg')->where('bid', '=', $this->bid)->where('key','=','yz_appsecert')->find()->value;
            $oid = ORM::factory('fxb_item')->where('bid','=',$this->bid)->where('id','=',$iid)->find()->url; //? iid
            $client = new YZTokenClient($this->access_token);
        ...
        //自动填写旧地址
        $old_order = ORM::factory('fxb_shorder')->where('qid', '=', $userobj->id)->order_by('id', 'DESC')->find();
    ...
    public function action_top2() {
        ...
        //计算排名
        $user = ORM::factory('fxb_qrcode', $this->uid)->as_array();

        $rankkey = "fxb:rank3:{$this->bid}:{$this->openid}:$top";
        $result['rank'] = $mem->get($rankkey);
        if (!$result['rank']) {
            $result['rank'] = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('score', '>', $user['score'])->count_all()+1;
        ...
        if (!$users) {
            $usersobj = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('score', 'DESC')->limit($top)->find_all();
    ...
    public function action_shorders($type=0) {
        $view = "weixin/fxb/shorders";
        $userobj = ORM::factory('fxb_qrcode', $this->uid);
        $this->template->title = '我的奖励';
        $this->template->content = View::factory($view)->bind('orders', $orders)->bind('userobj', $userobj);
        $orders = ORM::factory('fxb_shorder')->where('bid', '=', $this->bid)->where('qid', '=', $this->uid)->order_by('id', 'DESC')->find_all();
    }

    //订单明细
    public function action_orders() {
        $view = "weixin/fxb/orders";
        $userobj = ORM::factory('fxb_qrcode', $this->uid);
    ...
    public function action_order($tid) {
        $view = "weixin/fxb/order";

        $order = ORM::factory('fxb_trade', $tid);
    ...
    public function action_top() {
        ...
        $user = ORM::factory('fxb_qrcode', $this->uid);
        $top = $this->config['rank_fxb'] ? $this->config['rank_fxb'] : 10;

        $result['rank'] = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('id2', '>', 0)->where('lock', '<>', 1)->where('lock', '<>', 4)->where('paid', '>', $user->paid)->count_all()+1;

        $usersobj = ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('lock', '<>', 1)->where('lock', '<>', 4)->order_by('paid', 'DESC')->limit($top)->find_all();
        foreach ($usersobj as $userobj) {
            $users[] = $userobj->as_array();
        }
    }
    ...
    public function action_customer($newadd='') {
        ...
        $user = ORM::factory('fxb_qrcode', $this->uid);
         ...
         {
            $month = strtotime(date('Y-m-1'));
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM fxb_qrcodes WHERE fopenid='$user->openid' and jointime>='$month'")->execute()->as_array();
         }
         else
            $firstchild=DB::query(Database::SELECT,"SELECT openid FROM fxb_qrcodes WHERE fopenid='$user->openid'")->execute()->as_array();
           ...
           if($newadd=='month')
           {
            $customer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', 'IN',$tempiid)->or_where('fopenid', '=', $user->openid)->where('jointime','>=',$month);
           }
           else
             $customer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid)->or_where('fopenid', 'IN',$tempiid);
         ...
         if($newadd=='month')
           {
            $totlecustomer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid)->or_where('fopenid', 'IN', $tempiid)->where('jointime','>=',$month)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
           }
         else
           $totlecustomer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid)->or_where('fopenid', 'IN', $tempiid)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();


    }

    // public function action_newadd() {//本月新增
    //     $view = 'weixin/fxb/newadd';
    ...
    //           }
    //         }
    //       $totlecustomer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid);
    //       $totlenum=$totlecustomer->count_all();
    //     //  $totlecustomer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid)->order_by('paid', 'DESC')->find_all();
    ...
    //             'items_per_page'=>500,
    //         ))->render('weixin/fxb/admin/pages');
    //      $totlecustomer=ORM::factory('fxb_qrcode')->where('bid', '=', $this->bid)->where('fopenid', 'IN',$tempid)->or_where('fopenid', '=', $user->openid)->order_by('paid', 'DESC')->limit(500)->offset($offset)->find_all();
    // }
    ...
    public function action_storefuop($bid){// 静默授权
        $config=ORM::factory('fxb_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/fxb/getopenid/'.$bid;
        $we = new Wechat($config);
    ...
    public function action_getopenid($bid){//通过code获取openid
        ...
        var_dump($results);
        echo '</pre>';
        $user = ORM::factory('fxb_qrcode')->where('bid','=',$bid)->where('openid','=',$openid)->find();
        ...
        $_SESSION['fxb']['access_token'] =$this->access_token;
        Request::instance()->redirect('/fxb/commends/'.$openid.'/'.$bid);
    }
    public function action_commends($mopenid,$bid){//奖品list分享页面
        $fopenid = $mopenid;
        $config=ORM::factory('fxb_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/fxb/commends/'.$mopenid.'/'.$bid;
        $we = new Wechat($config);

        //$userobj = ORM::factory('fxb_qrcode', $this->uid);
        if($_GET['code']){//静默授权当事人
            $this->template = 'weixin/fxb/tpl/tpl2';
            self::before();
            $token = $we->getOauthAccessToken();
            $openid=$token['openid'];
            ...
            $view = "weixin/fxb/commendsother";//别人直接是url进，不需要加密
            if($fopenid == $openid){
                // echo '自己';
                $status = 2;
                $result['title'] = $user->nickname.'的推荐商品';
                $view = "weixin/fxb/commends";//自己进 跳入shareopenid 需要加密url
            }
            $this->template->title = $fuser->nickname.'推荐商品';
            $result['commends'] = ORM::factory('fxb_setgood')->where('bid','=',$bid)->where('status','=',1)->find_all();
    ...
    public function action_shareopenid($mopenid,$gid,$bid){//商品分享页面 自己可以打开 别人也可以打开
        ...
        $config=ORM::factory('fxb_cfg')->getCfg($bid,1);
        require Kohana::find_file('vendor', 'weixin/wechat.class');
        $callback_url = 'http://'.$_SERVER['HTTP_HOST'].'/fxb/shareopenid/'.$mopenid.'/'.$gid.'/'.$bid;
        $we = new Wechat($config);
        if($_GET['code']){//静默授权当事人
            $this->template = 'weixin/fxb/tpl/tpl';
            self::before();
            $token = $we->getOauthAccessToken();
            $openid=$token['openid'];
            if(!$openid){//当别人打开网页时候 code 失效 openid无法获得 重新授权获得当事人openid和上级fopenid进行绑定
            ...
            $commend = ORM::factory('fxb_setgood')->where('bid','=',$bid)->where('id','=',$gid)->find();
            $view = "weixin/fxb/share";
    ...
    public function action_test(){
        ...
        if($type=='POINTS'){
            ...
            $bid = ORM::factory('fxb_login')->where('shopid','=',$kdt_id)->find()->id;
            $access_token=ORM::factory('fxb_login')->where('id', '=', $bid)->find()->access_token;
            $expiretime=ORM::factory('fxb_login')->where('id', '=', $bid)->find()->expiretime;
    ...
    private function tradeImport($trade, $bid, $client, $we, $config) {
        ...
        $fxb_trade = ORM::factory('fxb_trade')->where('tid', '=', $tid)->find();

        //跳过已导入订单
        if ($fxb_trade->id) {

            //更新订单状态
            if ($fxb_trade->status != $trade['status']) {
                $fxb_trade->status = $trade['status'];
                $fxb_trade->save();
            ...
            //退款订单删返利
            if ($trade['status'] == 'TRADE_CLOSED') ORM::factory('fxb_score')->where('tid', '=', $fxb_trade->id)->delete_all();
            if ($trade['status'] == 'TRADE_CLOSED_BY_USER') ORM::factory('fxb_score')->where('tid', '=', $fxb_trade->id)->delete_all();
            if ($trade['refund_state'] != 'NO_REFUND') ORM::factory('fxb_score')->where('tid', '=', $fxb_trade->id)->delete_all();
        ...
        //只处理有下线的订单
        $fxb_qrcode = ORM::factory('fxb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $userinfo['weixin_openid'])->find();
        Kohana::$log->add('id', print_r($fxb_qrcode->id, true));
        if (!$fxb_qrcode->id) {
        ...
        //是：则判断关注后的订单都有收益
        //否：判断生成海报后的订单才算收益
        $fromtime = $config['haibao_needpay'] ? $fxb_qrcode->subscribe : $fxb_qrcode->jointime;
        ...
        $rank=0;
        $fuser = ORM::factory('fxb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $trade['openid'])->find();
        if($fuser->fopenid){//有一级
            $rank=1;
            $ffuser = ORM::factory('fxb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fuser->fopenid)->find();
            if($ffuser->fopenid){//有二级
             ...
             foreach ($trade['orders'] as $order) {
                ...
                $goodidcof=ORM::factory('fxb_setgood')->where('goodid','=',$goodid)->find();
        ...
        $fxb_trade->values($trade);
        $fxb_trade1 = ORM::factory('fxb_trade')->where('tid', '=', $tid)->find();
        if($fxb_trade1->id) return;
        $fxb_trade->save();
        ...
        foreach ($trade['orders'] as $order) {
            ...
            $fxb_order=ORM::factory('fxb_order')->where('bid','=',$bid)->where('tid','=',$tid)->where('goodid','=',$goodid)->find();
            if(!$fxb_order->id)//跳过已导入的order
            {
                $fxb_order->bid=$bid;
                $fxb_order->tid=$tid;
                $fxb_order->goodid=$goodid;
                $fxb_order->title=$title;
                $fxb_order->num=$num;
                $fxb_order->price=$price;
                $fxb_order->save();
            }
        }
        Kohana::$log->add('4444444', '444444');
        //删除重复返利记录
        ORM::factory('fxb_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 1)->delete_all();
        ORM::factory('fxb_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 2)->delete_all();
        ORM::factory('fxb_score')->where('bid', '=', $bid)->where('qid', '=', $trade['qid'])->where('tid', '=', $trade['tid'])->where('type', '=', 3)->delete_all();
        ...
        if ($money0 > 0) {
            ...
            Kohana::$log->add('money0', print_r($money0, true));
            $fxb_qrcode->scores->scoreIn($fxb_qrcode, 1, $money0, 0, $fxb_trade->id);

            //发消息
            Kohana::$log->add('openid', print_r($fxb_qrcode->openid, true));
            $msg['touser'] = $fxb_qrcode->openid;
            ...
            if ($config['msg_score_tpl']){
                $we_result = $this->sendScoreMessage($msg['touser'], '购买返利', $money0, $fxb_qrcode->score, $url);
        ...
        //订单上线返利
        if ($money1 > 0) {
            $fuser = ORM::factory('fxb_qrcode')->where('bid', '=', $bid)->where('openid', '=', $fxb_qrcode->fopenid)->find();
            if ($fuser->id) {
                //echo "$tid money1:$money1 \n";
                $fuser->scores->scoreIn($fuser, 2, $money1, $fxb_qrcode->id, $fxb_trade->id);
                ...
                $msg['text']['content'] = "您推荐的{$config['title1']}「{$fxb_qrcode->nickname}」完成一笔订单！\n\n实付金额：$money {$title5}\n推广佣金：$money1 {$title5}\n\n<a href=\"$url\">查看我的{$config['title5']}明细</a>";
        ...
        if ($money2 > 0 && $fuser->fopenid) {
            ...
            if ($ffuser->id) {
                ...
                $ffuser->scores->scoreIn($ffuser, 3, $money2, $fuser->id, $fxb_trade->id);
                ...
                $msg['text']['content'] = "您推荐的{$config['title2']}「{$fxb_qrcode->nickname}」完成一笔订单！\n\n实付金额：$money {$title5}\n推广佣金：$money2 {$title5}\n\n<a href=\"$url\">查看我的{$config['title5']}明细</a>";
        ...
        if ($money3 > 0 && $ffuser->fopenid&&$config['kaiguan_needpay']==1) {
            ...
            if ($fffuser->id) {
                ...
                $fffuser->scores->scoreIn($fffuser, 3, $money3, $fuser->id, $fxb_trade->id);
                ...
                $msg['text']['content'] = "您推荐的{$config['titlen3']}「{$fxb_qrcode->nickname}」完成一笔订单！\n\n实付金额：$money {$title5}\n推广佣金：$money3 {$title5}\n\n<a href=\"$url\">查看我的{$config['title5']}明细</a>";
