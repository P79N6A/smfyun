...
class Controller_crm extends Controller{
    ...
    public function action_oauth1(){
        Request::instance()->redirect('http://crmcs.mclon.com/open/oauth/requestAuthorization?response_type=code&client_id=client001&redirect_uri=http://'.$_SERVER["HTTP_HOST"].'/crm/callback1&state=teststate');
    }
    public function action_callback1(){
        ...
        $url='http://crmcs.mclon.com/open/oauth/requestAccessToken?grant_type=authorization_code&client_id=client001&redirect_uri=http://'.$_SERVER["HTTP_HOST"].'/crm/callback1&code='.$code.'&state='.$state.'&client_secret=FB847641-8EF2-481C-801A-0BF617BBCA6C';
        $data=array();
        $ch=curl_init();
