...
//微代言配置
class Model_Kmi_Cfg extends ORM {
    ...
    public function getCfg($bid, $db=0) {
        ...
        if ( (!$cache = $mem->get($cfg_key)) || $db == 1) {
            $cfgs = ORM::factory('kmi_cfg')->where('bid', '=', $bid)->where('value', '<>', "")->find_all();
    ...
    public function setCfg($bid, $key, $value, $pic="") {
        ...
        $cfg = ORM::factory('kmi_cfg')->where('bid', '=', $bid)->where('key', '=', $key)->find();
    ...
    public function delCfg($bid, $key) {
        ...
        $cfg_key = "kmi:cfg:$bid";
        $mem->delete($cfg_key);
        return ORM::factory('kmi_cfg')->where('bid', '=', $bid)->where('key', '=', $key)->find()->delete();
    }
}
