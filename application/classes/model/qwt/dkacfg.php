...
class Model_qwt_dkaCfg extends ORM {
    ...
    public function getCfg($bid, $db=0) {
        ...
        //从数据库读取配置文件
        $cfg_key = "qwt:dkacfg:$bid";
        $cache = $mem->get($cfg_key);
        if ( (!$cache = $mem->get($cfg_key)) || $db == 1) {
            $cfgs = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('value', '<>', "")->find_all();
            foreach ($cfgs as $c) $cfg[$c->key] = $c->value;
            $mem->set($cfg_key, $cfg, 0);
            if ($cfg) $cache = $cfg;
        }
        $midmerge =array_merge($config['dka'], (array)$cache);
        $mem1 = Cache::instance('memcache');
        $cfg_key1 = "qwt:dkacfg:$bid";
    ...
    public function setCfg($bid, $key, $value, $pic="") {
        ...
        $cfg = ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', $key)->find();
    ...
    public function delCfg($bid, $key) {
        ...
        $cfg_key = "qwt:dkacfg:$bid";
        $mem->delete($cfg_key);
        return ORM::factory('qwt_dkacfg')->where('bid', '=', $bid)->where('key', '=', $key)->find()->delete();
    }
