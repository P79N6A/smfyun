...
//微代言配置
class Model_Qwt_Rwdcfg extends ORM {
    ...
    public function getCfg($bid, $db=0) {
        require Kohana::find_file('vendor', 'weixin/rwd.inc'); //默认配置
        $mem = Cache::instance('memcache');
        //从数据库读取配置文件
        $cfg_key = "qwt:rwdcfg:$bid";
        $cache = $mem->get($cfg_key);
        if ( (!$cache = $mem->get($cfg_key)) || $db == 1) {
            $cfgs = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('value', '<>', "")->find_all();
            foreach ($cfgs as $c) $cfg[$c->key] = $c->value;
            $mem->set($cfg_key, $cfg, 0);
            if ($cfg) $cache = $cfg;
        }
        $midmerge =array_merge($config['rwd'], (array)$cache);
        $mem1 = Cache::instance('memcache');
        $cfg_key1 = "qwt:rwdcfg:$bid";
    ...
    public function setCfg($bid, $key, $value, $pic="") {
        ...
        $cfg = ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', $key)->find();
    ...
    public function delCfg($bid, $key) {
        ...
        $cfg_key = "qwt:rwdcfg:$bid";
        $mem->delete($cfg_key);
        return ORM::factory('qwt_rwdcfg')->where('bid', '=', $bid)->where('key', '=', $key)->find()->delete();
    }
