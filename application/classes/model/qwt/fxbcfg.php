...
class Model_Qwt_Fxbcfg extends ORM {
    ...
    public function getCfg($bid, $db=0) {
        require Kohana::find_file('vendor', 'weixin/fxb.inc'); //默认配置
        $mem = Cache::instance('memcache');
        //从数据库读取配置文件
        $cfg_key = "qwt:fxbcfg:$bid";
        $cache = $mem->get($cfg_key);
        if ( (!$cache = $mem->get($cfg_key)) || $db == 1) {
            $cfgs = ORM::factory('qwt_fxbcfg')->where('bid', '=', $bid)->where('value', '<>', "")->find_all();
            foreach ($cfgs as $c) $cfg[$c->key] = $c->value;
            $mem->set($cfg_key, $cfg, 0);
            if ($cfg) $cache = $cfg;
        }
        $midmerge =array_merge($config['fxb'], (array)$cache);
        $mem1 = Cache::instance('memcache');
        $cfg_key1 = "qwt:fxbcfg:$bid";
    ...
    public function setCfg($bid, $key, $value, $pic="") {
        ...
        $cfg = ORM::factory('qwt_fxbcfg')->where('bid', '=', $bid)->where('key', '=', $key)->find();
    ...
    public function delCfg($bid, $key) {
        ...
        $cfg_key = "qwt:fxbcfg:$bid";
        $mem->delete($cfg_key);
        return ORM::factory('qwt_fxbcfg')->where('bid', '=', $bid)->where('key', '=', $key)->find()->delete();
    }
