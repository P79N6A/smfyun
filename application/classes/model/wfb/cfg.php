...
//微代言配置
class Model_Wfb_Cfg extends ORM {
    ...
    public function getCfg($bid, $db=0) {
        ...
        if ( (!$cache = $mem->get($cfg_key)) || $db == 1) {
            $cfgs = ORM::factory('wfb_cfg')->where('bid', '=', $bid)->where('value', '<>', "")->find_all();
            foreach ($cfgs as $c) $cfg[$c->key] = $c->value;
            $mem->set($cfg_key, $cfg, 0);
            if ($cfg) $cache = $cfg;
        }

        return array_merge($config['wfb'], (array)$cache);
    }
    ...
    public function setCfg($bid, $key, $value, $pic="") {
        ...
        $cfg = ORM::factory('wfb_cfg')->where('bid', '=', $bid)->where('key', '=', $key)->find();
    ...
    public function delCfg($bid, $key) {
        ...
        $cfg_key = "wfb:cfg:$bid";
        $mem->delete($cfg_key);
        return ORM::factory('wfb_cfg')->where('bid', '=', $bid)->where('key', '=', $key)->find()->delete();
    }
